...
def _plot_stress_strain_profiles(state_dict):
    """
    Section + strain + stress block diagram.
    Now:
      • NO headings on strain or stress-block panels.
      • Section title updates for ULS / SLS / Uncracked.
    """
    # ---------------- UNPACK ----------------
    b = state_dict["b"]
    D = state_dict["D"]
    ...
    # Detect state for title switching
    eps_c_abs = abs(eps_c)
    if eps_c_abs > 0.0025:
        section_title = "Section (ULS view)"
    elif eps_c_abs > 0.00015:
    ...
    sigma_c = alpha2 * fc
    sigma_s = abs(fs_t)
    stress_max = max(sigma_c, sigma_s, 1)
    ...
    x_mid_strain = (x0_strain + x1_strain) / 2

    panel_w_stress = 260
    x0_stress = x1_strain + gap
    x1_stress = x0_stress + panel_w_stress

    total_x_max = x1_stress + 40

    def strain_to_x(e):
        return x_mid_strain + (e / eps_max) * (panel_w_strain * 0.4)

    def stress_to_x(s):
        return x0_stress + (s / stress_max) * (panel_w_stress * 0.8)
    ...
    # Compression region for ALL states (blue shading)
    block_depth_sec = min(gamma * c, D)
    ...
    layout_bot = _layout_bars_in_rows(
        nb_bot, b, cover_bot, db_bot, 2*db_bot, 2
    )
    r_bot = db_bot / 2
    y0_bot = D - cover_bot - r_bot
    pitch_bot = db_bot + rowgap_bot

    for x_rel, row_idx in layout_bot:
        ax.add_patch(Circle((x0_sec + x_rel, y0_bot - row_idx*pitch_bot),
                            r_bot, fill=False, edgecolor="tab:blue", linewidth=1.3))

    # -------- TOP REO --------
    layout_top = _layout_bars_in_rows(
        nb_top, b, cover_top, db_top, 2*db_top, 2
    )
    r_top = db_top / 2
    y0_top = cover_top + r_top
    pitch_top = db_top + rowgap_top

    for x_rel, row_idx in layout_top:
        ax.add_patch(Circle((x0_sec + x_rel, y0_top + row_idx*pitch_top),
                            r_top, fill=False, edgecolor="tab:red", linewidth=1.3))

    # NA arrow
    x_na = x0_sec + b + 40
    ax.annotate("", xy=(x_na, c), xytext=(x_na, 0),
                arrowprops=dict(arrowstyle="<->", color="black"))
    ax.text(x_na + 20, c/2, f"NA = {c:.0f} mm", va="center", color="tab:red")

    # d arrow
    x_d = x_na + 80
    ax.annotate("", xy=(x_d, d), xytext=(x_d, 0),
                arrowprops=dict(arrowstyle="<->", color="black"))
    ax.text(x_d + 20, d/2, f"d = {d:.0f} mm", va="center")

    # ---- dynamic title HERE ----
    ax.text(x0_sec + b/2, -0.22*D, section_title,
            ha="center", va="top", fontsize=10)

    ax.set_ylabel("Depth (mm)")

    # =====================================================
    # 2) STRAIN PANEL (NO HEADING)
    # =====================================================
    ax.plot([x_mid_strain, x_mid_strain], [0, D], color="black")

    xvals = [strain_to_x(e) for e in [eps_c, 0, eps_s]]
    ax.plot(xvals, [0, c, d], color="black")

    ax.hlines(c, x0_strain - 10, x1_strain + 10,
              linestyles="--", linewidth=0.8, color="black")

    ax.text(strain_to_x(eps_c), 0, rf"$\varepsilon_c = {eps_c:.4f}$",
            color="tab:red", va="bottom")
    ax.text(strain_to_x(eps_s), d, rf"$\varepsilon_s = {eps_s:.4f}$",
            color="tab:blue", va="top")

    # No “Strain profile” title  
    # No “Strain” x-label

    # =====================================================
    # 3) STRESS-BLOCK PANEL (NO HEADING)
    # =====================================================
    ax.plot([x0_stress, x0_stress], [0, D], color="black")

    # Steel tension arrow
    xt = stress_to_x(sigma_s)
    ax.annotate("", xy=(xt, d), xytext=(x0_stress, d),
                arrowprops=dict(arrowstyle="->", color="tab:blue"))
    ax.text(xt + 8, d, f"T ({sigma_s:.0f} MPa)", color="tab:blue", va="center")

    # Decide rectangular vs triangular
    is_uls = abs(eps_c) > 0.0025
    block_top = 0
    ...
    if is_uls:
        # Rectangular block
        ax.fill_between([x0_stress, xr],
                        [block_top, block_top],
                        [block_bottom, block_bottom],
                        edgecolor="tab:red", facecolor="none", linewidth=1.5)
    else:
        # Triangular block
        ax.fill([x0_stress, x0_stress, xr],
                [block_bottom, block_top, block_top],
                edgecolor="tab:red", facecolor="none", linewidth=1.5)

    # γc arrow
    x_gc = xr + 0.12 * panel_w_stress
    ax.annotate("", xy=(x_gc, block_bottom), xytext=(x_gc, block_top),
                arrowprops=dict(arrowstyle="<->", color="tab:red"))
    ax.text(x_gc + 10, (block_top + block_bottom)/2,
            rf"$\gamma c = {gamma*c:.0f}$ mm", color="tab:red", va="center")

    # α2 fc arrow
    y_alpha = c + 0.05*D
    ax.annotate("", xy=(x0_stress, y_alpha), xytext=(xr, y_alpha),
                arrowprops=dict(arrowstyle="<->", color="tab:red"))
    ax.text((x0_stress+xr)/2, y_alpha + 0.05*D,
            rf"$\alpha_2 f'_c = {sigma_c:.0f}$ MPa", color="tab:red",
            ha="center")

    # 2 internal arrows SLS / 3 arrows ULS
    arrows = [0.25, 0.5, 0.75] if is_uls else [0.33, 0.67]
    for f in arrows:
        ymid = f * block_bottom
        ax.annotate("", xy=(x0_stress + 0.85*bw, ymid),
                    xytext=(x0_stress + 0.15*bw, ymid),
                    arrowprops=dict(arrowstyle="<-", color="tab:red"))
