...
# ============================
# PART 2 — ULS STRAIN + STRESS DIAGRAM (FULL FUNCTION)
# ============================

def _plot_stress_strain_profiles(state_dict):
    """
    Draws the combined ULS diagram containing:
        • Section (ULS view)
        • Strain Profile
        • Stress-block Profile

    All panels share the SAME depth axis, so NA always aligns.
    This version fixes:
        • stretched circles
        • titles overlapping graphics
        • consistent spacing
    """

    # -----------------------------------------
    # Extract values (all from state_dict)
    # -----------------------------------------
    b = state_dict["b"]
    D = state_dict["D"]
    d = state_dict["d"]
    ...
    nb_top = state_dict["nb_top"]
    db_top = state_dict["db_top"]
    cover_top = state_dict["cover_top"]

    nb_bot = state_dict["nb_bot"]
    db_bot = state_dict["db_bot"]
    cover_bot = state_dict["cover_bot"]
    rowgap_bot = state_dict["rowgap_bot"]
    ...
    # Very important: SAME vertical axis for all 3 sections
    ax.set_ylim(D, 0)
    ax.set_xlim(0, b + 650)  # big enough for all 3 panels

    # Keep depth (vertical dimension) true-scale
    ax.set_aspect("equal", adjustable="box")

    ax.axis("off")

    # Precompute panel x-positions
    x_section_left = 0
    x_section_right = b

    x_strain_left = b + 100
    x_strain_mid = x_strain_left + 60

    x_stress_left = x_strain_left + 220
    x_stress_mid = x_stress_left + 60
    ...
    # Concrete outline
    ax.add_patch(Rectangle((x_section_left, 0), b, D,
                           fill=False, edgecolor="black", linewidth=1.4))

    # Compression block
    if a is not None:
        ax.add_patch(
            Rectangle((x_section_left, 0), b, a,
                      facecolor="#c7e3ff", edgecolor="tab:red",
                      linewidth=1.2, alpha=0.8)
        )

        # NA dashed
        ax.hlines(c, x_section_left, x_section_right,
                  linestyles="--", colors="tab:red", linewidth=1.0)

    # --- Top bars ---
    if nb_top > 0:
        r_top = db_top / 2
        y_top = cover_top + r_top
        xs = np.linspace(x_section_left + cover_top + r_top,
                         x_section_right - cover_top - r_top,
                         nb_top)
        for x in xs:
            ax.add_patch(Circle((x, y_top), r_top,
                                fill=False, edgecolor="tab:red", linewidth=1.5))

    # --- Bottom bars ---
    if nb_bot > 0:
        r_bot = db_bot / 2
        y_bot0 = D - cover_bot - r_bot
        # Single-row layout here (multi-row layout handled elsewhere)
        xs = np.linspace(x_section_left + cover_bot + r_bot,
                         x_section_right - cover_bot - r_bot,
                         nb_bot)
        for x in xs:
            ax.add_patch(Circle((x, y_bot0), r_bot,
                                fill=False, edgecolor="tab:blue", linewidth=1.5))

    # Title (raised)
    ax.text((x_section_left + x_section_right) / 2,
            -35,
            "Section (ULS view)",
            ha="center", va="top",
    ...
    # -----------------------------------------
    # Strain Profile (Middle panel)
    # -----------------------------------------

    # NA dashed line repeated
    ax.hlines(c, x_strain_left, x_stress_left,
              linestyles="--", linewidth=1.1)

    # Strain line
    ax.plot([x_strain_mid - 20, x_strain_mid + 20],
            [0, D],
            color="black",
            linewidth=2)

    # Labels
    ax.text(x_strain_mid + 25, 0, rf"$\varepsilon_c = {eps_c:.4f}$",
            color="tab:red", fontsize=10, va="center")

    ax.text(x_strain_mid + 25, D,
            rf"$\varepsilon_s = {eps_s:.4f}$",
            color="tab:blue", fontsize=10, va="center")

    # Title (raised)
    ax.text(x_strain_mid,
            -35,
            "Strain Profile",
            ha="center", va="top",
            fontsize=13, fontweight="bold")
    ...
    # -----------------------------------------
    # Stress-block Profile (Right panel)
    # -----------------------------------------
    # NA dashed
    ax.hlines(c, x_stress_left, x_stress_left + 200,
              linestyles="--", linewidth=1.1)

    # Block depth γc arrow
    ax.annotate("",
                xy=(x_stress_left + 10, gamma * c),
                xytext=(x_stress_left + 10, 0),
                arrowprops=dict(arrowstyle="<->",
                                color="tab:red", linewidth=1.3))
    ax.text(x_stress_left + 18,
            gamma * c / 2,
            rf"$\gamma c = {gamma*c:.0f}\,\text{{mm}}$",
            va="center", color="tab:red")

    # Stress block top label
    ax.text(x_stress_left + 70, -5,
            rf"$\alpha_2 f'_c = {alpha2*fc:.0f}\,\text{{MPa}}$",
            color="tab:red", fontsize=10)

    # Tensile force arrow
    ax.annotate("",
                xy=(x_stress_left + 200, d),
                xytext=(x_stress_left + 80, d),
                arrowprops=dict(arrowstyle="->",
                                color="tab:blue", linewidth=1.5))
    ax.text(x_stress_left + 205, d,
            rf"$T\,({fsy:.0f}\,\text{{MPa}})$",
            va="center", color="tab:blue")

    # Title (raised)
    ax.text(x_stress_left + 100,
            -35,
    ...
    # -----------------------------------------
    # Raise top margin so titles do not collide
    # -----------------------------------------
