...
# ------------------------------------------------------------
#  Shared α2–γ helper (AS 3600 style, teaching)
# ------------------------------------------------------------
def _alpha2_gamma(fc):
    """Return (α2, γ, α2_raw, γ_raw) for a given f'c."""
    alpha2_raw = 0.85 - 0.0015 * fc
    gamma_raw = 0.97 - 0.0025 * fc
    alpha2 = max(0.67, alpha2_raw)
    gamma = max(0.67, gamma_raw)
    return alpha2, gamma, alpha2_raw, gamma_raw


# ------------------------------------------------------------
#  Helper – bar layout and tensile centroid depth
# ------------------------------------------------------------
def _layout_bars_in_rows(n_bars, b, cover, db, min_spacing, n_rows_max=2):
    """
    Return list of (x_rel, row_idx) for bars.
    row_idx = 0 for first row (bottom or top), 1 for second row, etc.
    """
...
def _effective_depth_centroid():
    """
    Effective depth d = depth to the CENTROID of bottom tension reinforcement.

    Uses:
        b, D, nb_bot, db_bot, cover_bot, rowgap_bot  (from shared state)
    """
    b = get_param("b")
    D = get_param("D")
    ...
    # First row depth (to bar centre) from top
...
# ------------------------------------------------------------
#  BENDING CAPACITY CALC (α2–γ stress block)
# ------------------------------------------------------------
def _compute_bending_capacity():
    """
    Compute φMu,cap and intermediates.
    ...
    # Effective depth – use centroid if possible then fallback to user d
    ...
    # Concrete tension properties (for cracking / min steel)
    cb = 0.2
    ...
    # Minimum tension steel (teaching formula)
    kAst = 1.0
    As_min = kAst * (d / D) ** 2 * (fctf / fsy) * b * D

    # Stress-block factors – SINGLE SOURCE OF TRUTH
    alpha2, gamma, _, _ = _alpha2_gamma(fc)
...
# ------------------------------------------------------------
#  THREE-PANEL STRESS–STRAIN DIAGRAM
# ------------------------------------------------------------
def _stress_strain_state(state: str):
    """
    Return dict with:
        b, D, d, c, eps_c, eps_s, gamma, fs_t, fc, fsy, alpha2
    using d = centroid depth of bottom steel.
    """
...
def _plot_stress_strain_profiles(state_dict):
    """
    Three-panel figure: Section (ULS view) + strain + stress-block.
    Uses d = centroid depth to bottom steel.
    """
    b = state_dict["b"]
    D = state_dict["D"]
    d = state_dict["d"]
    ...
    x_mid_strain = 0.5 * (x0_strain + x1_strain)
    ...
    # --- Section (left) ---
    ax.add_patch(Rectangle((x0_sec, 0), b, D, fill=False, linewidth=1.5, edgecolor="black"))
    block_depth = max(0.0, min(gamma * c, D))
    ax.add_patch(Rectangle(
        (x0_sec, 0), b, block_depth,
        facecolor="#c7e3ff", edgecolor="tab:red", linewidth=1.0, alpha=0.8
    ))
    ...
    for x_rel, row_idx in bot_layout:
        x = x0_sec + x_rel
        y = d_row0 - row_idx * row_pitch_bot
        ax.add_patch(Circle((x, y), r_bot, facecolor="none", edgecolor="tab:blue", linewidth=1.3))
    ...
    for x_rel, row_idx in top_layout:
        ...
        ax.add_patch(Circle((x, y), r_top, facecolor="none", edgecolor="tab:red", linewidth=1.3))

    # NA & d arrows
    x_na = x0_sec + b + 40.0
    ax.annotate("", xy=(x_na, c), xytext=(x_na, 0), arrowprops=dict(arrowstyle="<->", linewidth=1.0))
    ax.text(x_na + 20.0, c / 2.0, f"NA = {c:.0f} mm", va="center", fontsize=9, color="tab:red")

    x_d = x_na + 80.0
    ax.annotate("", xy=(x_d, d), xytext=(x_d, 0), arrowprops=dict(arrowstyle="<->", linewidth=1.0))
    ax.text(x_d + 20.0, d / 2.0, f"d ({d:.0f} mm)", va="center", fontsize=9)

    ax.text(x0_sec + b / 2.0, -0.13 * D, "Section (ULS view)", ha="center", va="top", fontsize=10)
    ax.set_ylabel("Depth (mm)")

    # --- Strain panel ---
    ax.plot([x_mid_strain, x_mid_strain], [0, D], color="black", linewidth=1.0)
    y_vals = np.array([0.0, c, d])
    eps_vals = np.array([eps_c, 0.0, eps_s])
    x_vals = [strain_to_x(e) for e in eps_vals]
    ax.plot(x_vals, y_vals, color="black")

    ax.hlines(c, x0_strain - 10.0, x1_strain + 10.0, colors="black", linestyles="--", linewidth=0.8)

    ax.text(strain_to_x(eps_c), 0.0,
            rf"$\varepsilon_c = {eps_c:.4f}$",
            ha="left", va="bottom", color="tab:red")
    ax.text(strain_to_x(eps_s), d,
            rf"$\varepsilon_s = {eps_s:.4f}$",
            ha="left", va="top", color="tab:blue")

    ax.text(x_mid_strain, -0.13 * D, "Strain Profile", ha="center", va="top", fontsize=10)
    ax.text(x_mid_strain, D + 0.14 * D, "Strain", ha="center", va="bottom", fontsize=9)

    # --- Stress-block panel ---
    ax.plot([x0_stress, x0_stress], [0, D], color="black", linewidth=1.0)
    sigma_s = abs(fs_t)
    x_T = stress_to_x(sigma_s)
    ax.annotate("", xy=(x_T, d), xytext=(x0_stress, d),
                arrowprops=dict(arrowstyle="->", linewidth=1.4, color="tab:blue"))
    ax.text(x_T + 0.02 * panel_w_stress, d, f"T ({sigma_s:.0f} MPa)",
    ...
    ax.fill_between([x0_stress, x_block_right],
                    [block_top, block_top],
                    [block_bottom, block_bottom],
                    edgecolor="tab:red", facecolor="none", linewidth=1.5)

    ax.hlines(c, x0_stress - 10.0, x1_stress, colors="black", linestyles="--", linewidth=0.8)

    sigma_c = alpha2 * fc
    y_alpha = c + 0.05 * D
    ax.annotate("", xy=(x0_stress, y_alpha), xytext=(x_block_right, y_alpha),
                arrowprops=dict(arrowstyle="<->", linewidth=1.2, color="tab:red"))
    ax.text((x0_stress + x_block_right) / 2.0, y_alpha + 0.08 * D,
            rf"$\alpha_2 f'_c = {sigma_c:.0f}\ \mathrm{{MPa}}$",
            ha="center", va="top", color="tab:red")

    x_gc = x_block_right + 0.12 * panel_w_stress
    ax.annotate("", xy=(x_gc, block_bottom), xytext=(x_gc, block_top),
                arrowprops=dict(arrowstyle="<->", linewidth=1.2, color="tab:red"))
    val_gammac = gamma * c
    ax.text(x_gc + 0.06 * panel_w_stress,
            0.5 * (block_top + block_bottom),
            rf"$\gamma c = {val_gammac:.0f}\ \mathrm{{mm}}$",
            va="center", color="tab:red")

    for frac in [0.25, 0.5, 0.75]:
        y_mid = block_top + frac * (block_bottom - block_top)
        ax.annotate("", xy=(x0_stress + 0.15 * block_width, y_mid),
                    xytext=(x_block_right - 0.15 * block_width, y_mid),
                    arrowprops=dict(arrowstyle="<-", linewidth=1.0, color="tab:red"))

    ax.text((x0_stress + x1_stress) / 2.0, -0.13 * D,
            "Stress-block Profile (AS3600 α₂–γ)",
            ha="center", va="top", fontsize=10)
    ax.text((x0_stress + x1_stress) / 2.0, D + 0.14 * D,
            "Stress (MPa)", ha="center", va="bottom", fontsize=9)

    return fig


# ------------------------------------------------------------
#  Simple ULS cross-section for step-by-step tab
#  (NO outer axes border – only the concrete outline)
# ------------------------------------------------------------
def _make_cross_section_figure(
    ...
    if show_compression and a is not None and c is not None:
        ...
        ax.add_patch(
            Rectangle((0, 0), b, block_depth,
                      facecolor="#c7e3ff", edgecolor="tab:red",
                      linewidth=1.2, alpha=0.8)
        )
        ax.hlines(c, 0, b, colors="tab:red", linestyles="--", linewidth=1.0)
    ...
    if nb_bot > 0 and db_bot > 0:
        ...
        rowgap_bot = get_param("rowgap_bot") or 25.0
        min_spacing_bot = 2.0 * db_bot
        layout_bot = _layout_bars_in_rows(nb_bot, b, cover_bot, db_bot, min_spacing_bot, 2)
        row_pitch_bot = db_bot + rowgap_bot
        d_row0 = D - cover_bot - db_bot / 2.0
        for x_rel, row_idx in layout_bot:
            ...
            y = d_row0 - row_idx * row_pitch_bot
            ax.add_patch(
                Circle((x, y), r_bot, facecolor="none", edgecolor="tab:blue", linewidth=1.3)
            )
    ...
    if nb_top > 0 and db_top > 0:
        ...
        rowgap_top = get_param("rowgap_top") or 25.0
        min_spacing_top = 2.0 * db_top
        layout_top = _layout_bars_in_rows(nb_top, b, cover_top, db_top, min_spacing_top, 2)
        y_top_base = cover_top + db_top / 2.0
        row_pitch_top = db_top + rowgap_top
        for x_rel, row_idx in layout_top:
            ...
            y = y_top_base + row_idx * row_pitch_top
            ax.add_patch(
                Circle((x, y), r_top, facecolor="none", edgecolor="tab:red", linewidth=1.3)
            )

    # c and z arrows
    if c is not None:
        ax.annotate(
            "", xy=(b + 20.0, c), xytext=(b + 20.0, 0),
            arrowprops=dict(arrowstyle="<->", linewidth=1.0, color="tab:red"),
        )
        ax.text(b + 25.0, c / 2.0, f"c = {c:.0f} mm", va="center", color="tab:red")

    if z is not None and d is not None:
        ax.annotate(
            "", xy=(b + 50.0, d), xytext=(b + 50.0, 0),
            arrowprops=dict(arrowstyle="<->", linewidth=1.0),
        )
        ax.text(b + 55.0, d / 2.0, f"z = {z:.0f} mm", va="center")
...
def _make_uls_stress_block_figure(c, d, gamma_sb, fsy, show_lever_arm=False):
    ...
    ax.annotate(
        "", xy=(1.0, d), xytext=(0.5, d),
        arrowprops=dict(arrowstyle="->", linewidth=1.5, color="tab:blue"),
    )
    ax.text(1.02, d, r"$T = A_{st} f_{sy}$", va="center", color="tab:blue")

    if show_lever_arm:
        z = d - 0.5 * gamma_sb * c
        ax.annotate(
            "", xy=(0.7, d), xytext=(0.7, 0.5 * gamma_sb * c),
            arrowprops=dict(arrowstyle="<->", linewidth=1.2),
        )
        ax.text(0.72, 0.5 * (d + gamma_sb * c / 2.0), "z", va="center")
...
def render_bending():
    ...
    D_local = D or 600.0
    cover_bot_local = cover_bot or 40.0
    db_bot_local = db_bot or 20.0
    ...
    alpha2_sb, gamma_sb, alpha2_raw, gamma_raw = _alpha2_gamma(fc)
    phi_b = get_param("phi_bend", 0.85)
    ku_sb = ku if ku is not None else float("nan")

    Mu_min = 1.2 * Mcr if Mcr not in (None, float("nan")) else float("nan")
    Mu_nom_report = phi_Mu_cap / phi if phi and phi > 0 else float("nan")

    st.markdown("---")

    # ================== ACTIONS & INPUTS (same as before) ==================
    st.subheader("Design Actions for Bending")
    da1, da2, da3 = st.columns(3)
    sync = sync_callbacks

    with da1:
        number_row("Design moment Mu* (kNm)", "bending_Mu_star", 10.0, sync)
    with da2:
        number_row("Axial force N* (kN)", "bending_N_star", 50.0, sync)
    with da3:
        number_row("Prestress force P* (kN)", "bending_P_star", 50.0, sync)
    ...
    with g1:
        ...
        number_row("Width b (mm)", "bending_b", 10.0, sync)
        number_row("Depth D (mm)", "bending_D", 10.0, sync)
        number_row("Span L (mm)", "bending_L", 100.0, sync)

    with g2:
        st.subheader("Materials")
        number_row("Concrete strength f'c (MPa)", "bending_fc", 2.0, sync)
        number_row("Steel yield fsy (MPa)", "bending_fsy", 10.0, sync)
        number_row("Ec (MPa)", "bending_Ec", 1000.0, sync)
        number_row("Es (MPa)", "bending_Es", 10000.0, sync)
    ...
    with r1:
        ...
        number_row("Number of bottom bars nb_bot", "bending_nb_bot", 1, sync)
        number_row("Bottom bar diameter db_bot (mm)", "bending_db_bot", 2.0, sync)
        number_row("Bottom row gap (mm)", "bending_rowgap_bot", 5.0, sync)
        number_row("Bottom cover (mm)", "bending_cover_bot", 5.0, sync)

    with r2:
        st.subheader("Top Longitudinal Reinforcement")
        number_row("Number of top bars nb_top", "bending_nb_top", 1, sync)
        number_row("Top bar diameter db_top (mm)", "bending_db_top", 2.0, sync)
        number_row("Top row gap (mm)", "bending_rowgap_top", 5.0, sync)
        number_row("Top cover (mm)", "bending_cover_top", 5.0, sync)
    ...
    with tab_uls:
        ...
        if phi_Mu_cap > 0 and d and Ast:
            ...
            st.markdown("### 2. Stress-block parameters (AS 3600-like)")
            ...
            st.markdown(
                "### 3. Minimum strength requirements (self-weight / cracking)"
            )
            ...
            with col3_text:
                ...
                Muo_min = Mu_min
                st.markdown(
                    "#### 3.4 Teaching minimum ultimate moment $(M_{uo})_{min}$"
                )
                ...
                st.markdown("#### 3.5 Minimum tensile reinforcement")
                st.latex(
            ...
            with col4_text:
                T = Ast * fsy
                st.markdown("#### 4.1 Neutral axis depth $c$")
                st.latex(
    ...
    # SLS tab left as-is (teaching example)
    with tab_sls:
        ...
        if d and Ast and Ec and Es and b and D:
            ...
            st.markdown("**2. Neutral axis depth dₙ**:")
