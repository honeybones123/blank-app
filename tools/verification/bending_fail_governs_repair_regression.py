...
def main(argv: list[str] | None = None) -> int:
    ...
    blocked_no_repair_visible = bool(
        (
            ...
            or (
                "bending capacity is low" in lower_text
                and "no one-click repair is executable" in lower_text
            )
        )
        and (
            "exhaustive" in lower_text
            or "repair search exhausted" in lower_text
            or "family-owned blocker proof" in lower_text
    ...
    negative_checks = {
        ...
        # An ACTION card is expected to show that its preview passes.  Reject
        # only a pass-only terminal card that has no executable repair CTA.
        "no_pass_terminal": not (
            "pass" in lower_text
