...
def main() -> int:
    ...
    checks: dict[str, bool] = {
        ...
        "module_dependency_set_is_empty": (
    ...
    checks["module_runtime_owns_globals"] = (
        getattr(extracted, "_guidance_item_family", None) is extracted_family
        ...
        is not bridge._guidance_item_coverage_tuple
        and getattr(extracted, "_guidance_items_materially_overlap", None)
        is not bridge._guidance_items_materially_overlap
        and getattr(extracted, "_guidance_update_map", None)
        is not bridge._guidance_update_map
        and getattr(extracted, "_first_actionable_guidance_item", None)
        is not bridge._first_actionable_guidance_item
        and getattr(
            getattr(extracted, "_candidate_cache_key", None),
            "__module__",
            "",
        )
        == "inputs_page_modules.design_guide.candidate_keys"
    ...
    checks["module_collapse_runtime_cases"] = (
        ...
        and collapsed_meta.get("state_fp") == extracted._candidate_cache_key({"D": 650})
        ...
        and uncollapsed_meta.get("state_fp") == extracted._candidate_cache_key({"D": 700})
    )
