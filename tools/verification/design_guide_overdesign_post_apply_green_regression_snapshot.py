...
def _latest_live_lock_artifact(family_id: str) -> Path:
    ...
    candidates = sorted(
        ARTIFACT_DIR.glob(f"{config['artifact_slug']}_live_fuzz_regression_lock_gate_*.json"),
    ...
    for path in candidates:
        try:
            ...
            continue
        if payload.get("family") == config["lock_family_id"] and payload.get("family_10_fuzz_row"):
            return path
    raise AssertionError(f"No strict browser/live lock artifact found for {family_id}")
...
def _compact_family_result(family_id: str, source_path: Path, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        publication_before = dict(row.get("publication_probe_before") or {})
        cta = dict(publication_before.get("cta") or {})
        second_action = (
            str(publication_before.get("outcome_state") or "").upper() == "ACTION"
