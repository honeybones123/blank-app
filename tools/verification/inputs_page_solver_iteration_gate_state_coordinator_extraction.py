...
def build_payload() -> dict[str, Any]:
    ...
    _, _, after_current_eval_helper = _function_segment(
        source,
    ...
    _, _, loop_gate_dispatch = _function_segment(
        source,
    ...
    static_checks = {
        ...
        "helper_preserves_gate_order": (
            ordered_tokens[0] in helper
            and "_prepare_one_click_solver_iteration_gate_after_current_eval_state_coordinator(" in helper
            and all(token in after_current_eval_helper for token in ordered_tokens[1:])
            and [after_current_eval_helper.index(token) for token in ordered_tokens[1:]]
            == sorted(after_current_eval_helper.index(token) for token in ordered_tokens[1:])
        ),
        ...
        "helper_preserves_continue_and_break_flags": all(
            token in after_current_eval_helper
            for token in (
        ...
        "helper_delegates_ready_state_packer": (
            "_build_one_click_solver_iteration_gate_ready_state_coordinator(" in after_current_eval_helper
            and "iteration_gate_scope=locals()" in after_current_eval_helper
        ),
        ...
        "loop_delegates_iteration_gate_state": (
            "_dispatch_one_click_solver_iteration_gate_state_from_iteration_loop_coordinator("
            in loop_body
            and "_prepare_one_click_solver_iteration_gate_state_coordinator("
            in loop_gate_dispatch
        ),
