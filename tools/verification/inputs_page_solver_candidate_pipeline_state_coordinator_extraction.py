...
def build_payload() -> dict[str, Any]:
    ...
    static_checks = {
        ...
        "helper_preserves_pipeline_order": all(token in helper for token in pipeline_ordered_tokens)
        and [helper.index(token) for token in pipeline_ordered_tokens]
        == sorted(helper.index(token) for token in pipeline_ordered_tokens),
        ...
        "helper_delegates_result_state_packing": (
            "_build_one_click_solver_candidate_pipeline_result_state_coordinator(" in helper
            and "candidate_pipeline_scope=locals()" in helper
        ),
