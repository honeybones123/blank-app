"""Verify width wording cutover for _describe_guidance_step."""
...
def _old_width_wording(before: dict[str, Any], after: dict[str, Any], width_key: str) -> str:
...
def _rows() -> list[dict[str, Any]]:
    cases = [
        ("rect_width_increase", {"b": 300.0}, {"b": 350.0}, {"b": 350.0}, "b", True),
        ("web_width_reduce", {"bw": 280.0}, {"bw": 260.0}, {"bw": 260.0}, "bw", True),
        ("i_web_thickness_increase", {"tw": 180.0}, {"tw": 200.0}, {"tw": 200.0}, "tw", True),
        ("bottom_reo_declined", {"bot1_count": 4}, {"bot1_count": 5}, {"bot1_count": 5}, "", False),
        ("shear_reo_declined", {"s_lig": 200.0}, {"s_lig": 250.0}, {"s_lig": 250.0}, "", False),
    ]
    rows: list[dict[str, Any]] = []
    for name, before, after, updates, width_key, should_handle in cases:
        result = build_design_guide_pure_guidance_step_description(
            ...
            resolved_width_key=width_key or None,
        )
        expected = _old_width_wording(dict(before), dict(after), width_key) if should_handle else None
        rows.append(
            {
                "name": name,
...
def _capture() -> dict[str, Any]:
    ...
    rows = _rows()
    source_checks = {
        "page_resolves_width_context": "_resolve_geometry_width_context(after_state)" in segment,
        "page_passes_resolved_width_key": "resolved_width_key=width_key" in segment,
        "page_width_fallback_branch_deleted": "if width_key in updates:" not in segment,
        "risky_bottom_branch_remains": "_bottom_reo_state_label(" in segment,
        "risky_shear_branch_remains": "_shear_state_label(" in segment,
        "controller_helper_accepts_width_key": "resolved_width_key: str | None = None" in controller_source,
    ...
    return {
        "schema": "design_guide_describe_guidance_step_width_wording_cutover.v1",
...
def _checks(payload: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "page_resolves_width_context": bool(source_checks.get("page_resolves_width_context")),
        "page_passes_resolved_width_key": bool(source_checks.get("page_passes_resolved_width_key")),
        "page_width_fallback_branch_deleted": bool(source_checks.get("page_width_fallback_branch_deleted")),
        "risky_reinforcement_branches_remain": bool(source_checks.get("risky_bottom_branch_remains")) and bool(source_checks.get("risky_shear_branch_remains")),
        "controller_helper_accepts_width_key": bool(source_checks.get("controller_helper_accepts_width_key")),
...
def _write(payload: dict[str, Any], checks: dict[str, bool]) -> tuple[Path, Path]:
    ...
    json_path = ARTIFACT_DIR / f"design_guide_describe_guidance_step_width_wording_cutover_{suffix}.json"
    report_path = AUDIT_DIR / f"design_guide_describe_guidance_step_width_wording_cutover_{suffix}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    lines = [
        "# Design Guide Describe Guidance Step Width Wording Cutover",
        "",
...
def main() -> int:
    ...
    print(f"design_guide_describe_guidance_step_width_wording_cutover {payload['status']}")
