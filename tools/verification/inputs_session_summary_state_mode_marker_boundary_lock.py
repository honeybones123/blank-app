...
def _write_report(payload: dict[str, Any], report_path: Path) -> None:
    lines = [
        "# Inputs Session Summary State Mode Marker Boundary Lock",
        "",
        f"## Executive Summary: {payload['decision']}",
        "",
        "This lock covers delegated `_inputs_summary_state_mode` marker assembly inside `_resolved_inputs_summary_state`.",
        "",
        "## Checks",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Boundary",
            "",
            "- Marker payload assembly is owned by `inputs_page_modules.session`.",
            "- `inputs_page.py` still performs the actual `st.session_state` write.",
            "- Widget shear value reads remain page-owned and are passed in as plain data.",
            "- UX probe, derived recompute, normalized shear truth overlay, callbacks, Apply routing, and rendering remain page-owned.",
        ]
    )
...
def main() -> int:
    ...
    checks = {
        ...
        "page_imports_builder": "build_inputs_summary_state_mode_marker_snapshot" in page
        or "build_inputs_summary_state_mode_marker_snapshot" in bridge
        or "build_inputs_summary_state_mode_marker_snapshot" in summary_resolver,
        "summary_delegates_to_module": "build_inputs_summary_state_mode_marker_snapshot(" in summary_window
        and "widget_shear_state={" in summary_window
        and "overlay_count=len(overlay_applied)" in summary_window,
        "session_write_still_page_owned": 'st.session_state["_inputs_summary_state_mode"] = dict(' in summary_window
        and ".marker_payload" in summary_window,
        "delegated_trace_present": "inputs_summary_state_mode_marker_delegated" in summary_window
        ...
        "old_inline_marker_payload_deleted": 'st.session_state["_inputs_summary_state_mode"] = {' not in summary_window
    ...
    decision = (
        "INPUTS_SESSION_SUMMARY_STATE_MODE_MARKER_BOUNDARY_LOCKED"
        if not failures
        else "INPUTS_SESSION_SUMMARY_STATE_MODE_MARKER_BOUNDARY_GAPS_REMAIN"
    )
    payload = {
        "audit": "inputs_session_summary_state_mode_marker_boundary_lock",
        "timestamp": timestamp,
        "decision": decision,
        "checks": checks,
        "failures": failures,
        "locked_surface": "_resolved_inputs_summary_state _inputs_summary_state_mode marker assembly",
        "marker_owner": "inputs_page_modules.session",
        "page_role": "widget shear reader and session marker writer",
    ...
    json_path = VERIFICATION_DIR / f"inputs_session_summary_state_mode_marker_boundary_lock_{timestamp}.json"
    report_path = AUDIT_DIR / f"inputs_session_summary_state_mode_marker_boundary_lock_{timestamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    _write_report(payload, report_path)
    print("inputs_session_summary_state_mode_marker_boundary_lock", "PASS" if not failures else "FAIL")
