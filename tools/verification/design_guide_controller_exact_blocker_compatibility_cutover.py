"""Verify post-click exact-blocker compatibility proof consumes DesignGuideController."""
...
HELPER_NAME = "_stamp_final_publication_post_click_exact_blocker_compatibility_proof"
...
def _controller_sample() -> dict[str, Any]:
    ...
    evidence = {
        "exact_blockers_by_family": {"bending": {"blocked": True, "reason": "synthetic"}},
        "post_click_exact_blockers_by_family": {"bending": {"blocked": True, "reason": "synthetic"}},
    }
    response = run_design_guide_controller_publication_authority(
        DesignGuideControllerRequest(
            item={
                "title": "Post-click exact blocker compatibility",
                "family": "BENDING_FAIL_GOVERNS",
                "action_type": "blocked",
                "candidate_id": "candidate-exact-blocker",
                "post_click_design_guide_state": "exact_blocker",
                "button_contract": {"enabled": False, "action_type": "blocked"},
            },
            debug={
            ...
            final_visible_resolution={
                "render_reason": "exact_blocker_cutover",
                "presentation": {"tone": "blocked", "status": "BLOCKED"},
            },
            guidance_debug={"candidate_search_evidence": evidence},
            publication_reason="exact_blocker_cutover",
            source="exact_blocker_cutover",
        )
    )
    ...
    exact_blocker = {
        ...
        "post_click_design_guide_state": publication.get("post_click_design_guide_state"),
        "blocker_projection_hash": _stable_hash(proof.get("blocker_projection") or {}),
        "evidence_projection_hash": _stable_hash(proof.get("evidence_projection") or {}),
    ...
    return {
        ...
        "exact_blocker_hash": _stable_hash(exact_blocker),
    }
...
def _build_snapshot() -> dict[str, Any]:
    ...
    checks = {
        ...
        "helper_preserves_evidence_override": 'publication_debug["candidate_search_evidence"]' in helper_source,
        ...
        "compatibility_stamp_preserved": "final_publication_post_click_exact_blocker_compatibility_proofs"
        in helper_source,
        ...
        "exact_blocker_hash_stable": sample_a["exact_blocker_hash"] == sample_b["exact_blocker_hash"],
    ...
    return {
        "schema": "design_guide_controller_exact_blocker_compatibility_cutover.v1",
        ...
        "errors": errors,
        "next_slice": "Move compute publication evidence compatibility proof behind DesignGuideController.",
    }


def _write(snapshot: dict[str, Any]) -> tuple[Path, Path]:
    stamp = snapshot["created_at"]
    json_path = ARTIFACT_DIR / f"design_guide_controller_exact_blocker_compatibility_cutover_{stamp}.json"
    md_path = AUDIT_DIR / f"design_guide_controller_exact_blocker_compatibility_cutover_{stamp}.md"
    lines = [
        "# Design Guide Controller Exact-Blocker Compatibility Cutover",
        "",
...
def main() -> int:
    ...
    print(f"design_guide_controller_exact_blocker_compatibility_cutover {snapshot['status']}")
