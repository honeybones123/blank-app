"""Snapshot Design Guide publication resolver outputs for product scenarios."""
...
def _selected_row(rows: list[dict[str, Any]]) -> dict[str, Any]:
    for row in reversed(rows):
        if str(row.get("source") or "") == "render_fast_design_guidance_panel.final_visible_resolution":
            return row
    for row in reversed(rows):
        if str(row.get("source") or "") == "compute_design_guidance_items.final_compute_resolution":
            return row
...
def _run_scenario(label: str, *, base_port: int, index: int, timestamp: str) -> dict[str, Any]:
    ...
    runtime_path = ARTIFACT_DIR / f"design_guide_publication_runtime_{timestamp}_{label.lower()}.jsonl"
    if runtime_path.exists():
        runtime_path.unlink()
    env = dict(os.environ)
    env.pop("CODEX_BROWSER_TEST_MODE", None)
    env.update(dict(spec.get("env") or {}))
    env["DESIGN_GUIDE_PUBLICATION_SNAPSHOT_PATH"] = str(runtime_path)
    command = [
        sys.executable,
    ...
    failures: list[str] = []
    selected_family = str(
        selected.get("selected_family")
        or selected.get("published_family")
    ...
    if not selected:
        failures.append("publication_snapshot_missing")
    if selected_family and selected_family != expected_family:
        failures.append("publication_family_mismatch")
    return {
        "label": label,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--port", type=int, default=10601)
    parser.add_argument(
        "--scenario",
        action="append",
    ...
    report = {
        "schema": "design_guide_publication_snapshot.v1",
        "status": status,
        "results": results,
    }
    output = ARTIFACT_DIR / f"design_guide_publication_snapshot_{timestamp}.json"
