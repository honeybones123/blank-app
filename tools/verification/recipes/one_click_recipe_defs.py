...
REGRESSION_CASES: list[dict[str, Any]] = [
    ...
    {"name": "F_combined_overdesign", "changes": _manual_actions(45.0, 150.0)},
    {
        ...
        "changes": _manual_actions(98.0, 0.0),
    },
    {
        "name": "AB_BLOCKED_INVALID_STATE",
        "changes": {
            **_manual_actions(98.0, 0.0),
            "D": -1.0,
        },
    },
    {
        "name": "AB_COMMIT_ROLLBACK",
        "changes": _manual_actions(300.0, 0.0),
    },
]
...
def _matrix_shear_safe_bending_fail_base() -> dict[str, Any]:
    # Keep shear inside the target band so this recipe exercises the plain
    # bending-failure family rather than the mixed bending/shear-overdesign
    # route.  The previous 1260 kN value made the recipe self-contradictory.
    return _so_base_heavy_links_with_actions(600.0, 1650.0)
...
def _bending_only_overdesign_locked_shear_base_for_mu(mu: float) -> dict[str, Any]:
    changes = _bending_only_overdesign_locked_shear_base()
    changes.update(_manual_actions_with_input_proxies(mu, 0.0))
    return changes
...
DEBUG_CASES.extend(
    _live_fuzz_cases_for_family(
        ...
        [
            # Keep shear inside the target band while the ten recipes vary
            # the bending demand. The previous demand overshot shear failure
            # and silently changed the selected family.
            _so_base_heavy_links_with_actions(600.0 + (index * 10.0), 1250.0 + (index * 5.0))
...
DEBUG_CASES.extend(
    _live_fuzz_cases_for_family(
        "SHEAR_FAIL_GOVERNS",
        [
            _matrix_bending_target_shear_fail_base()
            | {
                "Vu_star": 600.0 + (index * 20.0),
                "Vstar": 600.0 + (index * 20.0),
                "V_u_star": 600.0 + (index * 20.0),
            }
            for index in range(10)
        ],
    )
...
DEBUG_CASES.extend(
    _live_fuzz_cases_for_family(
        ...
        [
            _manual_actions_with_input_proxies(
                300.0 + (index * 20.0),
                400.0 + (index * 25.0),
            )
            for index in range(10)
...
DEBUG_CASES.extend(
    _live_fuzz_cases_for_family(
        ...
        [
            # Hold shear inside its target band. Zero shear makes shear
            # overdesign active too and correctly selects COMBINED_OVERDESIGN.
            # Keep bending close enough to its cleanup boundary that reducing
            # it does not also require a shear change.
            _bending_overdesign_shear_probe_base(
                100.0,
                199.0 + (0.2 * index),
            )
            for index in range(10)
...
DEBUG_CASES.extend(
    _live_fuzz_cases_for_family(
        ...
        [
            # Mu=94 is the proven bending-safe boundary for this section.
            # Increasing Mu moved later recipes to a terminal target-band
            # state before the shear cleanup family could own publication.
            _live_fuzz_rect(
                mu=94,
                vu=65.0 + (0.2 * i),
                b=300,
...
DEBUG_CASES.extend(
    _live_fuzz_cases_for_family(
        ...
        [
            _live_fuzz_rect(mu=291 + 2 * index, vu=80 + 5 * index, b=350, D=420, bot_count=2, bot_dia=16, lig_d=16, lig_legs=3, s_lig=125)
            for index in range(10)
        ],
    )
)
...
DEBUG_CASES.extend(
    [
        {
            "name": "BENDING_FAIL_ZERO_SHEAR_DUCTILITY_DEPTH_REPAIR_REPRO",
            "changes": _live_fuzz_rect(
                mu=400,
                vu=0,
                b=300,
                D=400,
...
DEBUG_CASES.extend(
    _live_fuzz_cases_for_family(
        ...
        [
            _live_fuzz_rect(mu=20, vu=10, b=250, D=320, bot_count=2, bot_dia=16, lig_d=10, s_lig=100, span_m=8.0, sls_mu=500, defl_limit_ratio=250),
            _live_fuzz_rect(mu=40, vu=20, b=275, D=340, bot_count=2, bot_dia=16, lig_d=10, s_lig=100, span_m=8.5, sls_mu=540, defl_limit_ratio=250),
            _live_fuzz_rect(mu=60, vu=30, b=300, D=360, bot_count=3, bot_dia=16, lig_d=10, s_lig=100, span_m=9.0, sls_mu=580, defl_limit_ratio=250),
            _live_fuzz_rect(mu=60, vu=40, b=325, D=380, bot_count=4, bot_dia=16, lig_d=10, s_lig=100, span_m=9.5, sls_mu=620, defl_limit_ratio=250),
            _live_fuzz_rect(mu=100, vu=50, b=350, D=400, bot_count=4, bot_dia=16, lig_d=10, s_lig=100, span_m=10.0, sls_mu=660, defl_limit_ratio=250),
            _live_fuzz_rect(mu=120, vu=60, b=375, D=420, bot_count=4, bot_dia=20, lig_d=10, s_lig=100, span_m=10.5, sls_mu=700, defl_limit_ratio=250),
            _live_fuzz_rect(mu=140, vu=70, b=400, D=440, bot_count=4, bot_dia=20, lig_d=12, s_lig=100, span_m=11.0, sls_mu=740, defl_limit_ratio=250),
            _live_fuzz_rect(mu=160, vu=80, b=425, D=460, bot_count=5, bot_dia=20, lig_d=12, s_lig=100, span_m=11.5, sls_mu=780, defl_limit_ratio=250),
            _live_fuzz_rect(mu=180, vu=90, b=450, D=480, bot_count=5, bot_dia=20, lig_d=12, s_lig=100, span_m=12.0, sls_mu=820, defl_limit_ratio=250),
            _live_fuzz_rect(mu=200, vu=100, b=475, D=500, bot_count=6, bot_dia=20, lig_d=12, s_lig=100, span_m=12.5, sls_mu=860, defl_limit_ratio=250),
        ],
    )
)
