...
def _capture() -> dict[str, Any]:
    ...
    family_visual = _latest("design_guide_family_browser_live_visual_consistency")
    post_apply_readiness = _latest("design_brain_shared_post_apply_readiness_lock")
    ...
    return {
        ...
        "locks": {
            ...
            "current_family_visual": {
                "path": family_visual.get("path"),
                "status": family_visual.get("status"),
            },
            "current_post_apply_readiness": {
                "path": post_apply_readiness.get("path"),
                "status": post_apply_readiness.get("status"),
            },
            "render_bridge_lock": {
...
def _classify(capture: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "current_family_live_apply_visual_passed": (
            (locks.get("current_family_visual") or {}).get("status") == "PASS"
            and (locks.get("current_post_apply_readiness") or {}).get("status") in {"PASS", "LOCKED"}
        ),
