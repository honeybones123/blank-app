...
def _classify(row: dict[str, Any], *, render_bridge_locked: bool) -> dict[str, Any]:
    ...
    if target == "run_design_guide_controller_compute_publication_handoff_trace_only":
        cls = "compute_stage_authority_keep"
        deletion_safe_now = False
        next_step = "compute publication handoff same-object proof"
        rationale = "The controller publication handoff remains the single live pre-publication compute bridge."
    elif reachability == "compatibility stamp":
...
def main() -> int:
    ...
    source_files = (INPUTS_PAGE, ROUTE_COORDINATORS, CONTROLLER)
    input_source = "\n".join(
    ...
    render_resolver_replaced = bool(
        "build_collapsed_guidance_item_from_final_publication(" in input_source
    ...
    expected_callsite_count = 1 if render_resolver_replaced else 12
    ...
    if render_resolver_replaced and classified:
        expected_counts = {
            "compute_stage_authority_keep": 1,
        }
    ...
    print(f"design_guide_post_render_bridge_restamper_readiness {payload['status']}")
