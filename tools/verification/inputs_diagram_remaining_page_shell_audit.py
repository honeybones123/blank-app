...
def run_snapshot() -> dict:
    ...
    runtime_common = _read("inputs_application/page_runtime/common.py")
    runtime_widgets = _read("inputs_application/page_runtime/widgets.py")
    ...
    checks = {
        ...
        and (
            "make_summary_cross_section_figure_fn=make_summary_cross_section_figure" in live_surface
            or '"make_summary_cross_section_figure_fn": make_summary_cross_section_figure' in live_surface
        )
        and (
            "render_plotly_diagram_fn=st.plotly_chart" in live_surface
            or '"render_plotly_diagram_fn": st.plotly_chart' in live_surface
        ),
        ...
        and (
            "cached_make_section_3d_figure_fn=cached_make_section_3d_figure" in live_surface
            or '"cached_make_section_3d_figure_fn": cached_make_section_3d_figure' in live_surface
        )
        and (
            "make_beam_3d_figure_fn=make_beam_3d_figure" in live_surface
            or '"make_beam_3d_figure_fn": make_beam_3d_figure' in live_surface
        )
        and (
            "render_plotly_diagram_fn=st.plotly_chart" in live_surface
            or '"render_plotly_diagram_fn": st.plotly_chart' in live_surface
        ...
        "diagram_figure_wrappers_are_typed_runtime_owned": (
            "def cached_make_section_figure(" in runtime_common
            and "def cached_make_section_3d_figure(" in runtime_widgets
            and "def cached_make_section_figure(" not in root_page
            and "def cached_make_section_3d_figure(" not in root_page
        ),
