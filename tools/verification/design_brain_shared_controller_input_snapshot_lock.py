...
def _inputs_page_controller_request_calls() -> dict[str, Any]:
    ...
    boundary_sources = [
        path for path in CONTROLLER_BOUNDARY_PATHS
        if "design_brain.design_guide_controller" in _read(path)
    ]
    return {
        "request_constructor_alias": "_DesignGuideControllerRequest",
        "call_count": call_count,
        "has_calls": call_count == 0,
        "imports_request_alias": bool(boundary_sources),
        "page_shell_constructor_absent": call_count == 0,
        "controller_boundary_sources": [str(path) for path in boundary_sources],
    }
