...
def main() -> int:
    ...
    runtime_state_cache_source, _ = _function_source(
        runtime_source,
    ...
    if "render_inputs_summary_state_cache_module(" not in runtime_state_cache_source:
        failures.append("runtime_summary_state_cache_missing_module_delegate")
    ...
    transaction_index = workspace_render_source.find(
    ...
    if not (
        0
        <= transaction_index
        < summary_pipeline_index
        < widget_sections_index
    ):
        failures.append(
            "summary_pipeline_call_order_changed:"
            f"transaction={transaction_index}:widget_sections={widget_sections_index}:"
