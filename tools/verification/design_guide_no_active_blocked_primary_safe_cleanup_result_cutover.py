"""Verify safe-cleanup-before-blocker result branch is controller-builder driven."""
...
def _capture() -> dict[str, Any]:
    ...
    return {
        ...
        "bending_route_still_calls_legacy_assembler": f"{BENDING_ASSEMBLER}(" in route_source,
        "bending_route_uses_controller_builder": BENDING_BUILDER_ALIAS + "(" in route_source,
        "safe_route_preserves_trace_event": (
            '"return_no_active_safe_cleanup_candidate_before_blocker"' in route_source
            and "contract_hash=_dg_runtime_trace_hash(safe_cleanup_contract)" in route_source
        ),
        "safe_route_stamps_result_trace": TRACE_HELPER + "(" in route_source,
        "safe_assembler_retained_or_deleted": True,
        "safe_assembler_present": bool(safe_source),
        "bending_assembler_retained": bool(bending_source),
        "verification": {
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    return {
        ...
        == 0,
        "bending_assembler_has_at_most_one_live_call": (capture.get("bending_assembler") or {}).get(
            "live_call_count"
        )
        in (0, 1),
        ...
        is True,
        "bending_route_known_state": (
            capture.get("bending_route_still_calls_legacy_assembler") is True
            or capture.get("bending_route_uses_controller_builder") is True
        ),
        "safe_route_preserves_trace_event": capture.get("safe_route_preserves_trace_event")
        is True,
        "safe_route_stamps_result_trace": capture.get("safe_route_stamps_result_trace") is True,
        "safe_assembler_retained_or_deleted": capture.get(
            "safe_assembler_retained_or_deleted"
        )
        is True,
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Design Guide No-Active Blocked-Primary Safe Cleanup Result Cutover",
        "",
    ...
    lines.extend(
        [
            ...
            "## Decision",
            "",
            "The safe-cleanup-before-blocker branch is controller-builder driven. The old safe "
            "assembler is retained only until a deletion proof confirms it is dead. The bending "
            "cleanup branch remains legacy-driven.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    json_path = (
        ARTIFACT_DIR
        / f"design_guide_no_active_blocked_primary_safe_cleanup_result_cutover_{stamp}.json"
    )
    report_path = (
        AUDIT_DIR
        / f"design_guide_no_active_blocked_primary_safe_cleanup_result_cutover_{stamp}.md"
    )
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_no_active_blocked_primary_safe_cleanup_result_cutover {status}")
