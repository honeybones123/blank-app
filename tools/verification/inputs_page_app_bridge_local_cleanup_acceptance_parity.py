...
def main() -> int:
    ...
    state = _sample_state()
    current_fp = legacy_inputs_page._local_cleanup_acceptance_fingerprint(dict(state))
    other_fp = legacy_inputs_page._local_cleanup_acceptance_fingerprint({**dict(state), "s_lig": 250})
    cases = [
        ("empty_session", {}),
        ("expected_fp_match", {"_design_guide_post_cleanup_acceptance_fp": current_fp}),
        ("accepted_set_match", {"_accepted_fps": {current_fp}}),
        ("acceptance_enabled", {"_design_guide_post_cleanup_acceptance_enabled": True}),
        (
    ...
    original_legacy_st = legacy_inputs_page.st
    original_bridge_st = bridge.st
    shared_storage_identity_match = (
        legacy_inputs_page._DESIGN_GUIDE_POST_CLEANUP_ACCEPTED_FPS
        is DESIGN_GUIDE_POST_CLEANUP_ACCEPTED_FPS
    )
    original_accepted = set(DESIGN_GUIDE_POST_CLEANUP_ACCEPTED_FPS)
    rows: list[dict[str, Any]] = []
    failures: list[str] = []
    try:
        for index, (name, session_values) in enumerate(cases):
            ...
            bridge.st = fake_st
            legacy_value = bool(legacy_inputs_page._local_cleanup_post_apply_acceptance_matches(dict(state)))
            ...
            if not match:
                failures.append(f"{name}_local_cleanup_acceptance_mismatch")
    finally:
        legacy_inputs_page.st = original_legacy_st
