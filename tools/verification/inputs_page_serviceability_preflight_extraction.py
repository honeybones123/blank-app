...
def main() -> int:
    ...
    bridge_helper = _function_source(bridge_source, "_serviceability_governs_preflight_payload")
    module_helper = _function_source(module_source, "_serviceability_governs_preflight_payload")
    cases = _case_results()
    checks = {
        "module_exists": MODULE.exists(),
        "bridge_imports_extracted_helper": "_serviceability_governs_preflight_payload_extracted" in bridge_source,
        "bridge_helper_is_thin_delegate": len(bridge_helper.splitlines()) <= 3,
        "bridge_binds_preflight_dependencies": "_bind_serviceability_preflight_dependencies(globals())" in bridge_helper,
        "bridge_delegates_to_extracted": "_serviceability_governs_preflight_payload_extracted(state)" in bridge_helper,
        "bridge_removed_preflight_body": "SERVICEABILITY_GOVERNS" not in bridge_helper
        and "serviceability_governs_preflight_blocker" not in bridge_helper,
        "module_keeps_preflight_body": "SERVICEABILITY_GOVERNS" in module_helper
        and "serviceability_governs_preflight_blocker" in module_helper,
        "module_has_dependency_binder": "def bind_serviceability_preflight_dependencies" in module_source,
