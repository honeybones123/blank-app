...
FIXED_REPLAYS: list[FixedReplay] = [
    ...
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case000_card_not_recomputed",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_000_card_not_recomputed_after_edit.json",
        "card_not_recomputed_after_edit",
        "Replay exits 0 with the visible card, summary, CTA contract, and payload recomputed from the current edited state.",
        "Manual edits must not leave stale Design Guide card/body/payload state after the fuzz-50 mutation path.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case001_active_fail_post_click_out_of_target",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_001_active_fail_post_click_out_of_target_family_unexplained.json",
        "active_fail_post_click_out_of_target_family_unexplained",
        "Replay exits 0 and any post-click active-fail/out-of-target family is explained by a repair action or exact engineering blocker.",
        "Post-click Design Guide states must not hide active-fail/out-of-target families behind generic terminal guidance.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case003_one_click_no_material_change",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_003_one_click_no_material_change.json",
        "one_click_no_material_change",
        "Replay exits 0 and no visible executor-backed CTA is a no-op payload.",
        "One-click Design Guide actions must make a material validated change or be withheld with exact blocker evidence.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case004_low_util_no_cleanup_or_blocker",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_004_low_util_no_cleanup_or_blocker.json",
        "low_util_no_cleanup_or_blocker",
        "Replay exits 0 and low-util families are resolved by executor-backed cleanup or exact no-further-cleanup proof.",
        "A terminal green card must not hide below-band strength families without exact stop proof.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case013_collapsed_body_contract",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_013_design_guide_collapsed_body_leaking.json",
        "design_guide_collapsed_body_leaking",
        "Replay exits 0 with one valid collapsed card and aligned visible summary/Design Guide governing values.",
        "Collapsed Design Guide cards must not leak body content and visible summary/card truth must share the governing row.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case017_card_not_recomputed",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_017_card_not_recomputed_after_edit.json",
        "card_not_recomputed_after_edit",
        "Replay exits 0 with the visible card, summary, CTA contract, and payload recomputed from the current edited state.",
        "Repeated fuzz-50 manual edit paths must not preserve stale Design Guide publication state.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case018_card_not_recomputed",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_018_card_not_recomputed_after_edit.json",
        "card_not_recomputed_after_edit",
        "Replay exits 0 with the visible card, summary, CTA contract, and payload recomputed from the current edited state.",
        "Repeated fuzz-50 manual edit paths must not preserve stale Design Guide publication state.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case028_ladder_stop_evidence",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_028_ladder_stop_calc_box_evidence_missing.json",
        "ladder_stop_calc_box_evidence_missing",
        "Replay exits 0 and terminal exact-stop details include visible family stop/check evidence.",
        "Green exact-stop cards must keep the expanded engineering reason for why further cleanup stopped.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case035_card_not_recomputed",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_035_card_not_recomputed_after_edit.json",
        "card_not_recomputed_after_edit",
        "Replay exits 0 with the visible card, summary, CTA contract, and payload recomputed from the current edited state.",
        "Repeated fuzz-50 manual edit paths must not preserve stale Design Guide publication state.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case036_active_fail_post_click_no_family",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_036_active_fail_post_click_no_family_in_target.json",
        "active_fail_post_click_no_family_in_target",
        "Replay exits 0 and post-click active-fail families either enter target or publish exact blocker evidence.",
        "A post-click state cannot be accepted if an active-fail family remains unresolved or unexplained.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case040_optimisation_wrong_family",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_040_optimisation_wrong_family.json",
        "optimisation_wrong_family",
        "Replay exits 0 and Design Guide family/badge semantics match the governing visible summary family.",
        "Optimisation cards must not target the wrong family or use a non-governing summary ratio.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case044_card_not_recomputed",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_044_card_not_recomputed_after_edit.json",
        "card_not_recomputed_after_edit",
        "Replay exits 0 with the visible card, summary, CTA contract, and payload recomputed from the current edited state.",
        "Repeated fuzz-50 manual edit paths must not preserve stale Design Guide publication state.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case047_card_not_recomputed",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_047_card_not_recomputed_after_edit.json",
        "card_not_recomputed_after_edit",
        "Replay exits 0 with the visible card, summary, CTA contract, and payload recomputed from the current edited state.",
        "Repeated fuzz-50 manual edit paths must not preserve stale Design Guide publication state.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case048_combined_fail_card_red",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_048_combined_fail_card_not_red_combined.json",
        "combined_fail_card_not_red_combined",
        "Replay exits 0 and combined active-fail cards use red repair semantics with named affected families.",
        "Combined active-fail cards must not be downgraded to cleanup/info semantics.",
    ),
    FixedReplay(
        "Fuzz-50 2026-06-01 Design Guide contract hardening",
        "fuzz50_case049_low_util_no_cleanup_or_blocker",
        "artifacts/verification/live_fuzz/2026-05-31T23-35-44/extracted_failures/case_049_low_util_no_cleanup_or_blocker.json",
        "low_util_no_cleanup_or_blocker",
        "Replay exits 0 and low-util families are resolved by executor-backed cleanup or exact no-further-cleanup proof.",
        "A terminal green card must not hide below-band strength families without exact stop proof.",
    ),
    FixedReplay(
        "Safe candidate vs terminal blocker policy",
        "2026-06-02_safe_combined_cleanup_candidate",
        "artifacts/verification/live_fuzz/2026-06-02T10-15-16/failure_case.json",
        "overdesign_blocker_false_cleanup_candidate_exists",
        "Replay exits 0 and publishable nested combined cleanup evidence is promoted to a fresh executor-backed CTA instead of a disabled blue blocker.",
        "A passing overdesign state must not render terminal BLOCKED when nested evidence contains a safe executable combined bending/shear cleanup candidate.",
        reason="Added after the June 2 sibling fuzz recurrence passed exact replay, previous-fixed, golden, and a one-case fresh fuzz smoke.",
    ),
]
