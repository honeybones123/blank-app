"""Deadness audit for manual no-second-CTA fallback rows.

Audit-only. The live no-second-CTA effect has been cut over to the Design
Brain result maps, but the old literal fallback rows remain as safety if the
...
def _capture() -> dict[str, Any]:
    ...
    latest = {
        "cutover": _latest("design_guide_final_binding_no_second_cta_result_cutover"),
    ...
    fallback_rows = {
        "contract": "if not final_binding_contract_effect:" in binding,
        "item": "if not final_binding_item_effect:" in binding,
        "evidence": "if not final_binding_evidence_effect:" in binding,
        "debug": "if not final_binding_debug_effect:" in binding,
    }
    helper_has_exception_path = (
        "except Exception as exc:" in helper
        and "return {}" in helper
        and '"final_binding_no_second_cta_result_error"' in helper
    )
    ...
    result_effects_are_primary = (
        "final_binding_contract_effect = dict(" in binding
        and "final_binding_item_effect = dict(" in binding
        and "final_binding_evidence_effect = dict(" in binding
        and "final_binding_debug_effect = dict(" in binding
    ...
    return {
        "decision": (
            "FINAL_BINDING_NO_SECOND_CTA_MANUAL_FALLBACK_DELETION_READY"
            if deletion_ready
            else "FINAL_BINDING_NO_SECOND_CTA_MANUAL_FALLBACK_NOT_DELETION_READY"
        ),
        ...
        "next_safe_slice": (
            "no-second-CTA manual fallback rows are already deleted; move to the next proven fallback group"
            if manual_fallback_rows_deleted and deletion_ready
            else "delete only the no-second-CTA manual fallback rows in a separate focused deletion slice"
            if deletion_ready
            else (
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Final Binding No-Second-CTA Manual Fallback Deadness",
        "",
...
def main() -> int:
    ...
    payload = {
        "schema": "design_guide_final_binding_no_second_cta_manual_fallback_deadness_snapshot.v1",
        "status": status,
        "created_at": _stamp(),
        "capture": capture,
        "checks": checks,
    ...
    json_path = ARTIFACT_DIR / f"design_guide_final_binding_no_second_cta_manual_fallback_deadness_{stamp}.json"
    md_path = AUDIT_DIR / f"design_guide_final_binding_no_second_cta_manual_fallback_deadness_{stamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(md_path, payload)
    print(f"design_guide_final_binding_no_second_cta_manual_fallback_deadness {status}")
