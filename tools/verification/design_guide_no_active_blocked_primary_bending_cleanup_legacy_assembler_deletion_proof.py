"""Prove the legacy bending-cleanup-before-blocker assembler can be deleted."""
...
ASSEMBLER = "_assemble_final_visible_bending_cleanup_available_before_blocker_result"
CONTROLLER_BUILDER_ALIAS = (
...
def _capture() -> dict[str, Any]:
    ...
    return {
        ...
        "safe_branch_state": {
            "assembler_present": safe_source is not None,
            "assembler_start_line": safe_start,
            "assembler_end_line": safe_end,
            "assembler_live_call_count": source.count(f"{SAFE_ASSEMBLER}(")
            - (1 if safe_source is not None else 0),
            "controller_builder_call_count": source.count(f"{SAFE_BUILDER_ALIAS}("),
            "state_known": source.count(f"{SAFE_BUILDER_ALIAS}(") > 0,
        },
        "decision": decision,
        "verification": {
            "bending_cutover": _run(
                "tools/verification/design_guide_no_active_blocked_primary_bending_cleanup_result_cutover.py"
            ),
            "safe_deletion": _run(
                "tools/verification/design_guide_no_active_blocked_primary_safe_cleanup_legacy_assembler_deletion_proof.py"
            ),
        },
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        > 0,
        "safe_branch_state_known": (capture.get("safe_branch_state") or {}).get("state_known")
        is True,
        "bending_cutover_passed": (verification.get("bending_cutover") or {}).get("passed")
        is True,
        "safe_deletion_passed": (verification.get("safe_deletion") or {}).get("passed") is True,
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    assembler = capture.get("assembler") or {}
    lines = [
        "# Design Guide No-Active Blocked-Primary Bending Cleanup Legacy Assembler Deletion Proof",
        "",
    ...
    lines.extend(
        [
            ...
            "## Recommendation",
            "",
            "Delete only `_assemble_final_visible_bending_cleanup_available_before_blocker_result(...)` "
...
def main() -> int:
    ...
    json_path = (
        ARTIFACT_DIR
        / f"design_guide_no_active_blocked_primary_bending_cleanup_legacy_assembler_deletion_{stamp}.json"
    )
    report_path = (
        AUDIT_DIR
        / f"design_guide_no_active_blocked_primary_bending_cleanup_legacy_assembler_deletion_{stamp}.md"
    )
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_no_active_blocked_primary_bending_cleanup_legacy_assembler_deletion {status}")
