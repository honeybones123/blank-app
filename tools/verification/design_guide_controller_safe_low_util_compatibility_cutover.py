"""Verify safe-low-util compatibility proof consumes DesignGuideController."""
...
HELPER_NAME = "_stamp_final_publication_safe_low_util_replacement_compatibility_proof"
...
def _controller_sample() -> dict[str, Any]:
    ...
    return {
        ...
        "replacement_hash": _stable_hash(replacement),
    }
...
def _build_snapshot() -> dict[str, Any]:
    ...
    checks = {
        ...
        "compatibility_stamp_preserved": "final_publication_safe_low_util_replacement_compatibility_proofs"
        in helper_source,
        ...
        "replacement_hash_stable": sample_a["replacement_hash"] == sample_b["replacement_hash"],
    ...
    return {
        "schema": "design_guide_controller_safe_low_util_compatibility_cutover.v1",
        ...
        "errors": errors,
        "next_slice": "Move combined cleanup rescue compatibility proof behind DesignGuideController.",
    }


def _write(snapshot: dict[str, Any]) -> tuple[Path, Path]:
    stamp = snapshot["created_at"]
    json_path = ARTIFACT_DIR / f"design_guide_controller_safe_low_util_compatibility_cutover_{stamp}.json"
    md_path = AUDIT_DIR / f"design_guide_controller_safe_low_util_compatibility_cutover_{stamp}.md"
    lines = [
        "# Design Guide Controller Safe-Low-Util Compatibility Cutover",
        "",
...
def main() -> int:
    ...
    print(f"design_guide_controller_safe_low_util_compatibility_cutover {snapshot['status']}")
