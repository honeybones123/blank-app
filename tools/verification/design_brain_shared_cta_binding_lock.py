...
REQUIRED_INPUTS_TOKENS = {
    ...
    "apply_handler_page_owned": "def handle_apply_buttons(",
...
def main() -> int:
    ...
    inputs_token_checks = {
        key: token in inputs_source for key, token in REQUIRED_INPUTS_TOKENS.items()
    }
    ...
    checks = {
        ...
        "inputs_page_declares_final_publication_cta_authority": inputs_token_checks["cta_authority_constant"],
        "inputs_page_delegates_cta_construction_to_final_publication": inputs_token_checks[
            "cta_from_current_state_delegate"
        ],
        "inputs_page_stamps_cta_authority_but_keeps_apply_routing": bool(
            inputs_token_checks["cta_authority_stamper"]
            and inputs_token_checks["contract_authority_marker"]
            and inputs_token_checks["apply_queue_page_owned"]
            and inputs_token_checks["apply_handler_page_owned"]
        ),
        "render_remains_page_owned": inputs_token_checks["render_panel_page_owned"],
        "fallback_cta_shell_non_authoritative": inputs_token_checks["fallback_cta_non_authoritative"],
    ...
    for key, present in inputs_token_checks.items():
        if not present:
    ...
    snapshot = {
        ...
        "inputs_token_checks": inputs_token_checks,
