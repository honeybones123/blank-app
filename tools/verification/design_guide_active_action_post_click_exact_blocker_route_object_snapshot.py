...
This is a self-contained route-object contract.  It proves the current
Design Brain controller route exists, keeps callbacks at the application
boundary, preserves the route gate/result shape, and produces stable plain
data.  Composed locks may depend on this contract, so this verifier must not
read those locks back as a prerequisite.
"""
