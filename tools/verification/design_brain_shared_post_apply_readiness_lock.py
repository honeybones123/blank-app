...
def main() -> int:
    ...
    static_checks = {
        "apply_in_flight_key_exists": "DESIGN_GUIDE_COMPONENT_APPLY_IN_FLIGHT_KEY" in inputs_source,
        "apply_in_flight_clearer_exists": "def _clear_design_guide_component_apply_in_flight(" in inputs_source,
        "apply_completion_clears_in_flight": '_clear_design_guide_component_apply_in_flight("apply_resolved_candidate_completed")' in inputs_source,
        "post_apply_overview_recorded": '"post_apply_overview": dict(_post_apply_route_overview)' in inputs_source,
        "post_apply_all_key_pass_recorded": '"post_apply_all_key_pass": bool(_post_apply_route_overview.get("all_key_pass"))' in inputs_source,
        "post_apply_any_fail_recorded": '"post_apply_any_fail": bool(_post_apply_route_overview.get("any_fail"))' in inputs_source,
        "terminal_post_apply_required_checks_pass_exists": "post_apply_required_checks_pass" in inputs_source,
    ...
    latest_fuzz = dict(family_fuzz.get("latest") or {})
    broad_fuzz = dict(family_fuzz.get("latest_broad_pass") or {})
    if not latest_fuzz:
        blockers.append("no family_10_fuzz_audit artifact found for browser/live post-Apply visual proof")
    elif not latest_fuzz.get("has_post_apply_visual_contract"):
        blockers.append("latest family_10_fuzz_audit lacks post_apply_green_pass_visual_contract evidence")
    if expected_family_count and int(latest_fuzz.get("families_audited") or 0) < expected_family_count:
        blockers.append(
            "latest family_10_fuzz_audit covers "
            f"{latest_fuzz.get('families_audited')} of {expected_family_count} current architecture families"
        )
    if not broad_fuzz:
        blockers.append("no broad passing family_10_fuzz_audit covers all current families with settled-card post-Apply visual proof")
