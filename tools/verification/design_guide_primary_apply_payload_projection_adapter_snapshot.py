...
def _build_snapshot() -> dict[str, Any]:
    ...
    capture = {
        ...
        "latest_locks": {
            "render_bridge": _latest("design_guide_render_bridge_lock"),
            "compute_bridge": _latest("design_guide_compute_resolver_publication_bridge_lock"),
            "independence": _latest("design_guide_independence_lock"),
        },
...
def _checks(capture: dict[str, Any], compile_run: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "combined_truth_probe_still_present": bool((capture.get("source_lines") or {}).get("combined_truth_probe")),
        ...
        "render_bridge_lock_pass": (locks.get("render_bridge") or {}).get("status") == "PASS",
        "compute_bridge_lock_pass": (locks.get("compute_bridge") or {}).get("status") == "PASS",
        "independence_lock_pass": (locks.get("independence") or {}).get("status") == "PASS",
