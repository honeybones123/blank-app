...
COMPUTE_PATHS = {
    "compute_stage_final_visible_resolver": {
        ...
        "target": "DesignGuideController.compute_selection",
        "call_needle": "final_compute_resolution = {",
        "adapter_needle": "handoff_request = DesignGuideControllerComputePublicationHandoffRequest(",
        "publication_reason_marker": '"controller_authority": "DesignGuideController.compute_selection"',
    ...
    "compute_late_evidence_contract_rebound": {
        ...
        "target": "DesignGuideController.compute_publication_handoff",
        "call_needle": "run_design_guide_controller_compute_publication_handoff_trace_only(",
        "adapter_needle": "build_collapsed_guidance_item_from_final_publication(",
        "publication_reason_marker": '"source": "DesignGuideController.compute_publication_handoff.trace_only"',
        "output_mutation_markers": (
    ...
    "post_core_evidence_rebound": {
        ...
        "target": "DesignGuideController.compute_rebound_publication_item",
        "call_needle": "run_design_guide_controller_compute_rebound_publication_item_trace_only(",
        "adapter_needle": "build_collapsed_guidance_item_from_final_publication(",
        "publication_reason_marker": '"memo_owner": "DesignGuideController.compute_rebound_publication_item"',
        "output_mutation_markers": (
...
def _analyze_path(
    ...
    selected_identity_adapter_fields_present = bool(
        adapter_exists
        and (
            "_collapsed_guidance_item_from_final_publication_authority(" in func_source
            or "build_collapsed_guidance_item_from_final_publication(" in func_source
        )
    )
    ...
    blocker_evidence_present = bool(
        ...
        or "build_final_design_guide_compute_publication_handoff_rebound_decision_proof" in func_source
    )
    ...
    return {
        ...
        "cleanup_classification": (
            cleanup_row.get("classification")
            if cleanup_row
            else "CONTROLLER_OWNED_TRACE_ONLY"
        ),
        "matches_cleanup_audit": bool(
            (cleanup_row and cleanup_row.get("classification") == CLASS_C)
            or (cleanup_row is None and start is not None)
        ),
