...
def test_page_names_delegate_to_shared_module() -> None:
    ...
    runtime_source = (ROOT / "deflection_page_runtime.py").read_text(encoding="utf-8")
    section_source = (
        ROOT / "engineering_page_sections" / "deflection_support.py"
    ).read_text(encoding="utf-8")
    neutral_support_source = (ROOT / "deflection_support.py").read_text(
        encoding="utf-8"
    )
    page_source = "\n".join(
        (runtime_source, section_source, neutral_support_source)
    )
