from __future__ import annotations

import ast
import json
from datetime import datetime
from pathlib import Path


ROOT = Path(__file__).resolve().parents[2]
ARTIFACT_DIR = ROOT / "artifacts" / "verification"
AUDIT_DIR = ROOT / "artifacts" / "audits"


def _function_source(source: str, name: str) -> tuple[str, int]:
    tree = ast.parse(source)
    matches: list[tuple[int, int, int]] = []
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and node.name == name:
            matches.append((node.end_lineno - node.lineno + 1, node.lineno, node.end_lineno))
    if not matches:
        return "", 0
    size, start, end = max(matches, key=lambda item: item[0])
    lines = source.splitlines()
    return "\n".join(lines[start - 1 : end]), size


def main() -> int:
    timestamp = datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    json_path = ARTIFACT_DIR / f"inputs_page_geometry_materials_top_section_current_{timestamp}.json"
    report_path = AUDIT_DIR / f"inputs_page_geometry_materials_top_section_current_{timestamp}.md"
    ARTIFACT_DIR.mkdir(parents=True, exist_ok=True)
    AUDIT_DIR.mkdir(parents=True, exist_ok=True)

    shell_source = (ROOT / "inputs_page.py").read_text(encoding="utf-8", errors="ignore")
    widget_module_source = (
        ROOT / "inputs_page_modules" / "widgets" / "render_coordinators.py"
    ).read_text(encoding="utf-8", errors="ignore")
    coordinator_source, coordinator_size = _function_source(
        widget_module_source,
        "render_inputs_geometry_materials_top_section",
    )
    render_inputs_source, _ = _function_source(shell_source, "render_inputs_page")
    widget_owner_source, _ = _function_source(widget_module_source, "render_inputs_widget_sections")

    failures: list[str] = []
    if not coordinator_source:
        failures.append("geometry_materials_top_section_current_coordinator_missing")
    if coordinator_size > 170:
        failures.append(f"geometry_materials_top_section_current_coordinator_too_large:{coordinator_size}")

    for required in [
        "with geometry_slot:",
        "Geometry & Materials",
        "detailed_mode:geometry_recommendation",
        "inputs_sec_shape",
        "Section shape",
        "inputs_D",
        "inputs_L",
        "inputs_cover_side",
        "materials_subsection_fn(sync_callbacks, show_heading=False)",
        'sub_mark("geometry")',
        'mark("top_inputs_widgets")',
        "section_2d_diagram_block_fn()",
        "resolved_inputs_model_state_fn()",
        "_inputs_fast_model_state_debug",
        "fast_model_block_fn(sync_callbacks, model_state=model_state)",
        'sub_mark("materials")',
        'mark("render_diagrams")',
    ]:
        if required not in coordinator_source:
            failures.append(f"coordinator_missing_{required}")

    call_text = "geometry_materials_top_section_fn("
    if call_text not in widget_owner_source:
        failures.append("render_inputs_missing_geometry_materials_top_section_call")

    for stale in [
        "Geometry & Materials",
        "detailed_mode:geometry_recommendation",
        "Section shape",
        "_render_section_2d_diagram_block()",
        "_resolved_inputs_model_state()",
        "_inputs_fast_model_state_debug",
        "_render_fast_model_block(sync_callbacks, model_state=model_state)",
    ]:
        if stale in render_inputs_source:
            failures.append(f"render_inputs_still_owns_{stale}")

    actions_index = widget_owner_source.find("design_actions_section_fn(")
    geometry_index = widget_owner_source.find(call_text)
    reinforcement_index = widget_owner_source.find("col_bot_reo, col_top_reo, col_shear_mat = create_reinforcement_columns_fn()")
    if not (0 <= actions_index < geometry_index < reinforcement_index):
        failures.append(
            "geometry_materials_call_order_changed:"
            f"actions={actions_index}:geometry={geometry_index}:reinforcement={reinforcement_index}"
        )

    payload = {
        "verifier": "inputs_page_geometry_materials_top_section_current_verifier",
        "status": "PASS" if not failures else "FAIL",
        "failures": failures,
        "coordinator_size": coordinator_size,
    }
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    report_path.write_text(
        "\n".join(
            [
                "# Inputs Page Geometry Materials Top Section Current Verifier",
                "",
                f"Status: `{payload['status']}`",
                "",
                f"Coordinator size: `{coordinator_size}`",
                "",
                "## Failures",
                "",
                *(f"- `{failure}`" for failure in failures),
                "",
            ]
        ),
        encoding="utf-8",
    )
    print(payload["status"])
    print(f"json={json_path}")
    print(f"report={report_path}")
    if failures:
        print("failures=" + ";".join(failures))
        return 1
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
