"""Object snapshot for post-click low-bending result item adapter proof."""
...
def _fixture() -> dict[str, Any]:
    ...
    request = {
        ...
        "acceptance_audit": {"post_click_exact_blockers_by_family": exact},
        "final_visible_resolution": {"reason": "fixture"},
    }
    first = build_final_design_guide_post_click_low_bending_resolution_result_item_adapter_proof(**request)
    second = build_final_design_guide_post_click_low_bending_resolution_result_item_adapter_proof(**request)
...
def _capture() -> dict[str, Any]:
    ...
    return {
        "decision": "POST_CLICK_LOW_BENDING_RESULT_ITEM_ADAPTER_OBJECT_PROVEN",
        ...
        "adapted_item_hash_present": bool(first.get("adapted_item_hash")),
        "adapter_surface_hash_present": bool(first.get("adapter_surface_hash")),
        "preserved_title": adapted.get("title_main") == "Design Guide blocker proof complete",
        "preserved_status": adapted.get("status") == "BLOCKED",
        "exact_blockers_projected": bool(
            adapted.get("post_click_exact_blockers_by_family")
            and evidence.get("post_click_exact_blockers_by_family")
        ),
        "action_payload_evidence_projected": bool(
            ((adapted.get("action_payload") or {}).get("candidate_search_evidence") or {}).get(
                "post_click_exact_blockers_by_family"
            )
        ),
        "resolved_candidate_evidence_projected": bool(
            ((adapted.get("resolved_candidate") or {}).get("candidate_search_evidence") or {}).get(
                "post_click_exact_blockers_by_family"
            )
        ),
        ...
        "projection_snapshot": projection_run,
        "raw_payload": first,
    }
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "adapted_item_hash_present": capture.get("adapted_item_hash_present") is True,
        "adapter_surface_hash_present": capture.get("adapter_surface_hash_present") is True,
        "preserved_title": capture.get("preserved_title") is True,
        "preserved_status": capture.get("preserved_status") is True,
        "exact_blockers_projected": capture.get("exact_blockers_projected") is True,
        "action_payload_evidence_projected": capture.get("action_payload_evidence_projected") is True,
        "resolved_candidate_evidence_projected": capture.get("resolved_candidate_evidence_projected") is True,
        ...
        "projection_snapshot_passed": (capture.get("projection_snapshot") or {}).get("passed") is True,
    }
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Post-Click Low-Bending Resolution Result Item Adapter Object Snapshot",
        "",
        ...
        f"- Exact blockers projected: `{capture.get('exact_blockers_projected')}`",
    ...
    lines.extend(
        [
            ...
            "## Next",
            "",
            "Wire this full item adapter trace-only beside the live low-bending result before any branch cutover.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    payload = {
        "schema": "design_guide_post_click_low_bending_resolution_result_item_adapter_object_snapshot.v1",
        "generated_at": _stamp(),
    ...
    json_path = (
        ARTIFACT_DIR
        / f"design_guide_post_click_low_bending_resolution_result_item_adapter_object_{stamp}.json"
    )
    report_path = (
        AUDIT_DIR
        / f"design_guide_post_click_low_bending_resolution_result_item_adapter_object_{stamp}.md"
    )
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str) + "\n", encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_post_click_low_bending_resolution_result_item_adapter_object {payload['status']}")
