...
# Child gates launched during this invocation are available before this
# composed verifier writes its own artifact.  Entries are populated only from
# the child command's stdout and are still validated by current_run_artifact
# once the parent artifact is written.
_CURRENT_RUN_CHILD_ARTIFACTS: list[dict[str, Any]] = []
...
REQUIRED_INPUTS_TOKENS = {
    ...
    "legacy_session_metadata_key": "legacy_non_authoritative",
    ...
    "legacy_session_no_override": "renderer_driving=False",
...
def _run_gate(script: str) -> dict[str, Any]:
    try:
        ...
        _record_child_artifacts(proc.stdout, result, started_epoch=started_epoch)
        return result
...
def _record_child_artifacts(
    stdout: str,
    result: dict[str, Any],
    *,
    started_epoch: float,
) -> None:
    """Record freshly emitted JSON artifacts for same-process gate lookups."""
    ...
    for raw_line in str(stdout or "").splitlines():
        ...
        if start < 0:
            start = lower.find("/")
        end = lower.find(".json", start if start >= 0 else 0)
        if start < 0 or end < 0:
            continue
        path = Path(line[start : end + len(".json")].strip(" \t:=,;"))
        if not path.is_file():
            continue
        ...
        _CURRENT_RUN_CHILD_ARTIFACTS.append(binding)
    if bindings:
        result["nested_artifact_bindings"] = bindings
...
def _active_manifest_payload() -> dict[str, Any]:
    value = str(os.environ.get("DESIGN_BRAIN_VERIFICATION_RUN_MANIFEST") or "").strip()
    if not value:
        return {}
    try:
...
def _latest_artifact(prefix: str) -> dict[str, Any]:
    for binding in reversed(_CURRENT_RUN_CHILD_ARTIFACTS):
        path = Path(str(binding.get("artifact_path") or ""))
        if not path.exists() or not path.name.startswith(f"{prefix}_"):
            continue
        if _file_sha256(path) != binding.get("artifact_sha256"):
            continue
        try:
    ...
    if os.environ.get("DESIGN_BRAIN_VERIFICATION_RUN_MANIFEST"):
        ...
        return {"found": True, "path": str(path), "status": payload.get("status"), "snapshot_hash": payload.get("snapshot_hash"), "failures": payload.get("failures"), "current_run": True}
    candidates = sorted(
...
def _composed_inputs_source() -> str:
    parts = [INPUTS_PAGE, ROUTE_COORDINATORS, APPLY_ROUTING, *CURRENT_RUNTIME_FILES]
    return "\n".join(
