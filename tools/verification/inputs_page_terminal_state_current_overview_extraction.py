...
def _run_cases() -> list[dict[str, Any]]:
    ...
    cases = [
        {
            ...
            "debug": {
                "efficiency_tightening_state": {"classification": "optimal"},
    ...
    for case in cases:
        ...
        bridge_debug = dict(case["debug"])
        bridge_result = bridge._derive_design_guide_terminal_state_from_current_overview(
            bridge_debug,
            dict(case["state"]),
            list(case["items"]),
        )
        bridge_meta = dict(bridge_debug.get("_derived_terminal_state_meta") or {})
        passed = (
            result == case["expected"]
            and meta.get("source") == case["expected_source"]
            and result == bridge_result
            and meta == bridge_meta
        )
        results.append(
            {
                "name": case["name"],
...
def main() -> int:
    ...
    checks = {
        ...
        "module_dependency_set_is_empty": "_TERMINAL_STATE_DEPENDENCIES: tuple[str, ...] = ()" in module_source,
