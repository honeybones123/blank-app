"""Deadness audit for manual consistency-guard fallback rows."""
...
def _capture() -> dict[str, Any]:
    ...
    latest = {
        "cutover": _latest("design_guide_final_binding_consistency_guard_result_cutover"),
    ...
    fallback_rows = {
        "shear": (
            ...
            and "dict(updates) != dict(safe_binding_updates)" in binding
        ),
        "combined": (
            "not final_binding_combined_consistency_guard_result" in binding
            and "combined_binding_evidence_available" in binding
            and "dict(updates) != dict(combined_binding_updates)" in binding
        ),
    }
    helper_has_exception_path = (
        "except Exception as exc:" in helper
        and "return {}" in helper
        and '"final_binding_consistency_guard_result_error"' in helper
    )
    helper_has_typed_fallback = (
        "except Exception as exc:" in helper
        and "_build_final_visible_contract_binding_typed_fallback_payload(" in helper
        and '"final_binding_consistency_guard_result_error"' in helper
        and '"updates_replacement": {}' in helper
        and '"action_type_replacement": ""' in helper
        and '"contract_replacement": {}' in helper
    )
    result_effects_are_primary = (
        "final_binding_safe_consistency_guard_result.get(\"reset_contract\")" in binding
        and "final_binding_combined_consistency_guard_result.get(\"reset_contract\")" in binding
        and "final_binding_safe_consistency_guard_result.get(\"updates_replacement\")" in binding
        and "final_binding_combined_consistency_guard_result.get(\"updates_replacement\")" in binding
    )
    ...
    return {
        "decision": (
            "FINAL_BINDING_CONSISTENCY_GUARD_MANUAL_FALLBACK_DELETION_READY"
            if deletion_ready
            else "FINAL_BINDING_CONSISTENCY_GUARD_MANUAL_FALLBACK_NOT_DELETION_READY"
        ),
        ...
        "next_safe_slice": (
            "consistency-guard manual fallback rows are already deleted; move to the next proven fallback group"
            if manual_fallback_rows_deleted and deletion_ready
            else "delete only the consistency-guard manual fallback rows in a separate focused deletion slice"
            if deletion_ready
            else (
                "either retire the helper empty-payload exception path with a safe typed fallback object, "
                "or move to the enabled contract expected-util/family truth block"
            )
        ),
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Final Binding Consistency Guard Manual Fallback Deadness",
        "",
...
def main() -> int:
    ...
    payload = {
        "schema": "design_guide_final_binding_consistency_guard_manual_fallback_deadness_snapshot.v1",
        "status": status,
        "created_at": _stamp(),
        "capture": capture,
        "checks": checks,
    ...
    json_path = ARTIFACT_DIR / f"design_guide_final_binding_consistency_guard_manual_fallback_deadness_{stamp}.json"
    md_path = AUDIT_DIR / f"design_guide_final_binding_consistency_guard_manual_fallback_deadness_{stamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(md_path, payload)
    print(f"design_guide_final_binding_consistency_guard_manual_fallback_deadness {status}")
