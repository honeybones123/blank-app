"""Focused family contract compliance verifier for MIN_BENDING_REO_GOVERNS."""
...
from design_brain.families.min_bending_reo_contract import (  # noqa: E402
    contract_hash,
    load_min_bending_reo_contract,
)
...
def _write(snapshot: dict[str, Any]) -> tuple[Path, Path]:
    ...
    json_path = ARTIFACT_DIR / f"design_brain_family_contract_compliance_min_bending_reo_{stamp}.json"
    md_path = AUDIT_DIR / f"design_brain_family_contract_compliance_min_bending_reo_{stamp}.md"
    snapshot["artifact"] = str(json_path)
    snapshot["report"] = str(md_path)
    json_path.write_text(json.dumps(snapshot, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    lines = [
        "# MIN_BENDING_REO_GOVERNS Family Contract Compliance",
        "",
...
def main() -> int:
    ...
    bending_owner_run = _run("tools/verification/families/bending_overdesign_governs_lock_verifier.py")
    combined_owner_run = _run("tools/verification/design_brain_family_contract_compliance_combined_overdesign.py")
    proof_chain = [chooser_run, mapping_run, bending_owner_run, combined_owner_run]

    contract = load_min_bending_reo_contract()
    ...
    mapping_case = next(
        (
            ...
            if case.get("case_id") == "old_min_bending_reo_selected"
        ),
        {},
    )
    mapping_coverage = next(
        (
            ...
            if row.get("case_id") == "old_min_bending_reo_selected"
        ),
        {},
    )

    chooser = classify_family_from_raw_flags(
        {"min_bending_reo_fail": True},
        evidence={"case_id": "minimum_bending_reo_maps_to_bending_overdesign_owner"},
    )
    ...
    checks = {
        "compatibility_contract_loads": contract.get("contract_identity", {}).get("family_id") == "MIN_BENDING_REO_GOVERNS",
        "compatibility_contract_hash_present": bool(contract_hash()),
        "classification_contract_excludes_legacy_family_id": (
            "MIN_BENDING_REO_GOVERNS" not in allowed_ids
            and "MIN_BENDING_REO_GOVERNS" not in priority_ids
            and "MIN_BENDING_REO_GOVERNS" not in rules
        ),
        "chooser_maps_min_signal_to_owner_family": selected_owner == "BENDING_OVERDESIGN_GOVERNS",
        ...
        == {"BENDING_OVERDESIGN_GOVERNS", "COMBINED_OVERDESIGN"},
        "bending_overdesign_owner_lock_pass": bending_owner_run["passed"],
    ...
    snapshot = {
        "schema": "design_brain_family_contract_compliance_min_bending_reo.v1",
    ...
    print(f"design_brain_family_contract_compliance_min_bending_reo {snapshot['result']}")
