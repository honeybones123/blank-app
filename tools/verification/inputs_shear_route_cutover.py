"""Prove the shear recommendation route no longer reaches the legacy bridge."""
...
def main() -> None:
    ...
    assert "_legacy_inputs_page._compute_shear_recommendation" not in route
    assert "compute_shear_recommendation_for_page(" in route
    assert "inputs_page_app_contract_bridge" not in runtime
    assert "inputs_page_route_coordinators" not in runtime
    assert "ShearRecommendationRuntime(" in runtime
    ...
    for recipe_id in (
        ...
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(
            ...
            compatibility = bridge._compute_shear_recommendation(
                copy.deepcopy(state)
            )
            replacement = compute_shear_recommendation_for_page(
                copy.deepcopy(state),
                session_state=session_state,
            )
        assert replacement == compatibility, recipe_id
    print(
        "PASS: shear route uses only trace/evaluation/scoring typed services "
        "with exact 3/3 payload parity"
    )
