...
def _assert_bridge_wrapper() -> None:
    ...
    assert line_count <= 7, f"bridge wrapper too large: {line_count} lines"
    assert "_bind_shear_recommendation_ladder_dependencies" not in body
...
def _assert_module_owner() -> None:
    ...
    assert "shear_state_eligible_for_no_links" in source
    assert "def bind_shear_recommendation_ladder_dependencies" not in source
