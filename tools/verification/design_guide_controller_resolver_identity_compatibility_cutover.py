"""Verify resolver identity compatibility proof consumes DesignGuideController.

This is a focused cutover verifier for the compatibility/debug helper only. It
does not exercise browser behaviour and does not change product output.
"""
...
HELPER_NAME = "_stamp_final_publication_resolver_identity_compatibility_proof"
...
def _direct_publication_build_count(source: str) -> int:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
            if node.func.id == "_build_final_design_guide_publication":
                count += 1
    return count
...
def _controller_sample() -> dict[str, Any]:
    ...
    response = run_design_guide_controller_publication_authority(
        DesignGuideControllerRequest(
            item={
                "title": "Resolver identity compatibility",
                "family": "BENDING_FAIL_GOVERNS",
                "action_type": "apply",
                "candidate_id": "candidate-resolver-identity",
                "button_contract": {
                    "enabled": True,
                    "actionable": True,
                    "action_type": "apply",
            ...
            verifier_payload={},
            final_visible_resolution={"render_reason": "resolver_identity_cutover"},
            guidance_debug={},
            publication_reason="resolver_identity_cutover",
            source="resolver_identity_cutover",
        )
    )
    ...
    identity = {
        ...
        "authority_hash": response.publication_hash,
    }
    return {
...
def _build_snapshot() -> dict[str, Any]:
    ...
    checks = {
        ...
        "helper_consumes_controller_hash": "controller_response.controller_hash" in helper_source,
        ...
        "compatibility_stamp_preserved": "final_publication_resolver_identity_compatibility_proofs"
        in helper_source,
        ...
        "identity_hash_stable": sample_a["identity_hash"] == sample_b["identity_hash"],
    ...
    return {
        "schema": "design_guide_controller_resolver_identity_compatibility_cutover.v1",
        ...
        "errors": errors,
        "next_slice": "Move resolution metadata compatibility proof behind DesignGuideController.",
    }


def _write(snapshot: dict[str, Any]) -> tuple[Path, Path]:
    stamp = snapshot["created_at"]
    json_path = ARTIFACT_DIR / f"design_guide_controller_resolver_identity_compatibility_cutover_{stamp}.json"
    md_path = AUDIT_DIR / f"design_guide_controller_resolver_identity_compatibility_cutover_{stamp}.md"
    lines = [
        "# Design Guide Controller Resolver Identity Compatibility Cutover",
        "",
...
def main() -> int:
    ...
    print(f"design_guide_controller_resolver_identity_compatibility_cutover {snapshot['status']}")
