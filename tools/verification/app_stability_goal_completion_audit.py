...
def _latest(prefix: str) -> tuple[Path | None, dict[str, Any]]:
    # Completion is release authority and must not select an old artifact by
    # filename. It is authoritative only inside the active canonical run.
...
def _artifact_matrix() -> dict[str, dict[str, Any]]:
    prefixes = {
        ...
        "release_gate_manifest_check": "release_gate_manifest_check",
...
def _build() -> dict[str, Any]:
    ...
    phase_checks = {
        ...
        "phase_12_exit_gate": {
            ...
            "evidence": [
                ...
                matrix["critical_workflows_lock"]["path"],
                matrix["family_10_fuzz"]["path"],
                matrix["release_gate_manifest_check"]["path"],
                matrix["verifier_retirement_deletion_workflow"]["path"],
                matrix["shared_path_release_lock"]["path"],
                matrix["browser_visual_layout_lock"]["path"],
                matrix["safe_optimal_blocker_publication_lock"]["path"],
                matrix["family_smooth_operation_lock"]["path"],
                matrix["universal_live_family_lock"]["path"],
                matrix["universal_verification_meta_lock"]["path"],
                matrix["shared_bridge_dependency_binding_lock"]["path"],
                matrix["live_bug_registry_contract"]["path"],
                matrix["design_guide_smoothness_completion"]["path"],
            ],
            "notes": (
                "Critical app slices are green only when the universal live family lock and the universal "
                "verification meta-lock are both LOCKED. Full exit gate remains unproven for every "
                "page/workflow until those gates are current."
            ),
        },
    }
    ...
    locked_evidence_checks = {
        ...
        and int(family_summary.get("scenario_trigger_passes") or 0) >= 90,
        "release_gate_manifest_check_pass": _is_pass(matrix["release_gate_manifest_check"]["payload"]),
        "verifier_retirement_deletion_workflow_pass": _is_pass(
            matrix["verifier_retirement_deletion_workflow"]["payload"]
        ),
        "shared_path_release_lock_pass": _is_pass(matrix["shared_path_release_lock"]["payload"]),
        "browser_visual_layout_lock_pass": _is_pass(matrix["browser_visual_layout_lock"]["payload"]),
        "safe_optimal_blocker_publication_lock_pass": _is_pass(
            matrix["safe_optimal_blocker_publication_lock"]["payload"]
        ),
        "family_smooth_operation_lock_pass": _is_pass(matrix["family_smooth_operation_lock"]["payload"])
        and str(matrix["family_smooth_operation_lock"]["payload"].get("smooth_operation_lock") or "") == "LOCKED",
        "universal_live_family_lock_pass": _is_pass(matrix["universal_live_family_lock"]["payload"])
        and str(matrix["universal_live_family_lock"]["payload"].get("universal_lock_status") or "") == "LOCKED",
        "universal_verification_meta_lock_pass": _is_pass(matrix["universal_verification_meta_lock"]["payload"])
        and str(matrix["universal_verification_meta_lock"]["payload"].get("meta_lock_status") or "") == "LOCKED",
        ...
        "shared_bridge_dependency_binding_lock_pass": _is_pass(matrix["shared_bridge_dependency_binding_lock"]["payload"]),
        "legacy_visible_surface_deletion_lock_pass": _is_pass(matrix["legacy_visible_surface_deletion_lock"]["payload"]),
        "live_bug_registry_contract_pass": _is_pass(matrix["live_bug_registry_contract"]["payload"]),
    }
