...
def main() -> int:
    ...
    dependency_block = module_source.split("def bind_recommendation_compound_candidate_dependencies", 1)[0]

    checks: dict[str, bool] = {
        "bridge_wrapper_is_small": (bridge_node.end_lineno or bridge_node.lineno) - bridge_node.lineno + 1 <= 18,
        "bridge_binds_dependencies": "_bind_recommendation_compound_candidate_dependencies(globals())" in bridge_body,
        ...
        "module_has_dependency_binder": "def bind_recommendation_compound_candidate_dependencies" in module_source,
        ...
        call_record.update(
            {
                ...
                "state": dict(state),
                "bottom_rec": dict(bottom_rec),
                "mode_config": dict(mode_config),
                "context": dict(context or {}),
                "compound_trace_log": list(compound_trace_log or []),
                "bound_helper": getattr(extracted, "_evaluate_candidate_fast", None)
    ...
    checks["bridge_runtime_binds_module_globals"] = (
        getattr(extracted, "_evaluate_candidate_fast", None) is bridge._evaluate_candidate_fast
