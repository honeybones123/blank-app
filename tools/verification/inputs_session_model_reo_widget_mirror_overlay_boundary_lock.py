...
def _write_report(payload: dict[str, Any], report_path: Path) -> None:
    lines = [
        "# Inputs Session Model Reo Widget Mirror Overlay Boundary Lock",
        "",
...
def main() -> int:
    ...
    helper = _function_window(page, "_overlay_inputs_reo_widget_mirrors_for_model") or _function_window(
        route,
        "_overlay_inputs_reo_widget_mirrors_for_model",
    )
    latest_parity = _latest_json("inputs_session_model_reo_widget_mirror_overlay_trace_parity")
    latest_cutover = _latest_json("inputs_session_model_reo_widget_mirror_overlay_cutover")
    checks = {
        "trace_parity_latest_pass": bool(latest_parity)
        and latest_parity.get("decision") == "READY_FOR_SESSION_MODEL_REO_WIDGET_MIRROR_OVERLAY_EXTRACTION"
        and not latest_parity.get("failures"),
        "cutover_latest_pass": bool(latest_cutover)
        and latest_cutover.get("decision") == "INPUTS_SESSION_MODEL_REO_WIDGET_MIRROR_OVERLAY_DELEGATED"
        ...
        "old_inline_overlay_scalar_cannot_return": "def _overlay_scalar" not in helper and "_overlay_scalar(" not in helper,
        "old_inline_coord_stale_function_cannot_return": "def _coords_stale_for" not in helper,
        "session_builder_owns_overlay_plan": "def build_inputs_model_reo_widget_mirror_overlay_plan(" in builders
        and "def _coords_stale_for" in builders
        and 'for section in ("bot", "top"):' in builders,
        "session_model_owns_plan_shape": "class InputsModelReoWidgetMirrorOverlayPlan" in models,
    ...
    decision = (
        "INPUTS_SESSION_MODEL_REO_WIDGET_MIRROR_OVERLAY_BOUNDARY_LOCKED"
        if not failures
        else "INPUTS_SESSION_MODEL_REO_WIDGET_MIRROR_OVERLAY_BOUNDARY_LOCK_GAPS_REMAIN"
    )
    payload = {
        "audit": "inputs_session_model_reo_widget_mirror_overlay_boundary_lock",
        "timestamp": timestamp,
        "decision": decision,
        "checks": checks,
        "failures": failures,
    ...
    json_path = VERIFICATION_DIR / f"inputs_session_model_reo_widget_mirror_overlay_boundary_lock_{timestamp}.json"
    report_path = AUDIT_DIR / f"inputs_session_model_reo_widget_mirror_overlay_boundary_lock_{timestamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    _write_report(payload, report_path)
    print("inputs_session_model_reo_widget_mirror_overlay_boundary_lock", "PASS" if not failures else "FAIL")
