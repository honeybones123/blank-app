...
def main() -> int:
    ...
    presence_checks = [
        ...
        _contains(inputs_source, "_pre_render_shell_projection = _build_final_design_guide_direct_shell_card_projection("),
        _contains(inputs_source, "_fallback_shell_projection = _build_final_design_guide_direct_shell_card_projection("),
    ]

    status = "PASS"
    if any(check["found"] for check in absence_checks):
        status = "FAIL"
    if any(not check["found"] for check in presence_checks):
        status = "FAIL"
    ...
    for check in presence_checks:
        lines.append(f"- `{check['needle']}` present: `{check['found']}`")
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
