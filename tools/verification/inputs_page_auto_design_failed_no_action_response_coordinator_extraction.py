"""Verify auto-design failed no-action response coordinator extraction."""
...
def _run_case(module: Any) -> dict[str, Any]:
    ...
    try:
        ...
        module._attach_no_action_visibility_coordinator = lambda **kwargs: (
            ...
            or {
                "user_visible_no_action_reason": "visible no action",
                "one_click_no_action_reason_code": "failed_eval",
            }
        )
        ...
        result = module._return_auto_design_failed_no_action_response_coordinator(
...
def build_payload() -> dict[str, Any]:
    ...
    helper_start, helper_end, helper = _function_segment(
        source,
        "_return_auto_design_failed_no_action_response_coordinator",
    )
    ...
    runtime_checks = {
        ...
        and feedback["reason"] == "solver_failed"
        ...
        "recommendation_envelope_preserved": envelope["status"] == "failed"
        and envelope["commit_audit"] == {"audit": "yes"}
        and envelope["updates"] == {"D": 650}
        and envelope["blocked_reason"] == "solver_failed"
        and envelope["commit_eligible"] is False,
        "latch_reason_preserved": runtime["result"]["latch_reason"] == "run_one_click_auto_design:no_action_failed",
        "returned_payload_shape_preserved": payload["status"] == "no_action"
        and payload["stop_reason"] == "solver_failed"
        and payload["steps"] == ["step-a", "Solver evaluation failed; no changes applied."]
        ...
        and payload["user_visible_no_action_reason"] == "visible no action"
        and payload["one_click_no_action_reason_code"] == "failed_eval",
    }
    static_checks = {
        "helper_present": "def _return_auto_design_failed_no_action_response_coordinator(" in source,
        "helper_preserves_latch_reason": "run_one_click_auto_design:no_action_failed" in helper,
        "helper_preserves_failure_step": "Solver evaluation failed; no changes applied." in helper,
        ...
        "dispatch_delegates_failed_no_action_response": "_return_auto_design_failed_no_action_response_coordinator("
        in terminal_body
        ...
        "dispatch_preserves_other_return_branches": 'if out_status == "blocked":' in dispatch_body
        and 'if out_status == "no_action" and stop_reason == "already_in_band":' in terminal_body
    ...
    return {
        "status": status,
        "surface": "run_one_click_auto_design_failed_no_action_response_coordinator",
        "helper_segment": {
            "function": "_return_auto_design_failed_no_action_response_coordinator",
...
def write_artifacts(payload: dict[str, Any]) -> tuple[Path, Path]:
    ...
    json_path = ARTIFACT_DIR / f"inputs_page_auto_design_failed_no_action_response_coordinator_extraction_{stamp}.json"
    md_path = AUDIT_DIR / f"inputs_page_auto_design_failed_no_action_response_coordinator_extraction_{stamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    lines = [
        "# Auto Design Failed No-Action Response Coordinator Extraction",
        "",
