"""Deadness snapshot for the old fallback-generator boundary-only route-body surface."""
...
def _call_windows(source: str, token: str) -> list[str]:
    ...
    while True:
        ...
        if index < 0:
            break
        windows.append(source[index : index + 1800])
...
def _capture() -> dict[str, Any]:
    ...
    boundary_only_live_windows = [
        window
        for window in live_windows
        if "fallback_variant_generator_handoff=dict(" in window
        and "fallback_variant_generator_dependency_boundary=dict(" not in window
    ]
    dependency_backed_live_windows = [
        window
        for window in live_windows
        if "fallback_variant_generator_dependency_boundary=dict(" in window
        and "residual_fallback_variant_generator_dependency_boundary or {}" in window
    ]
    required = {
        "fallback_dependency_route_shape_readiness": _latest(
            "design_guide_post_click_low_bending_residual_shear_cleanup_"
            "fallback_variant_generator_dependency_route_shape_readiness"
        ),
        "fallback_dependency_trace_wiring": _latest(
            "design_guide_post_click_low_bending_residual_shear_cleanup_"
            "fallback_variant_generator_dependency_boundary_trace_wiring"
        ),
        "route_body_replacement_trace_wiring": _latest(
            "design_guide_post_click_low_bending_residual_shear_cleanup_"
            "route_body_replacement_trace_wiring"
        ),
    }
    return {
        "decision": (
            "RESIDUAL_SHEAR_FALLBACK_GENERATOR_BOUNDARY_ONLY_SURFACE_DEAD_FOR_LIVE_PATH"
        ),
        "total_route_body_replacement_calls": len(windows),
        "live_route_body_replacement_calls": len(live_windows),
        "boundary_only_live_calls": len(boundary_only_live_windows),
        "dependency_backed_live_calls": len(dependency_backed_live_windows),
        ...
        "next_safe_surface": "residual_shear_candidate_evaluator_dependency_boundary",
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    return {
        ...
        "boundary_only_live_calls_zero": capture.get("boundary_only_live_calls") == 0,
        "dependency_backed_live_call_present": (
            capture.get("dependency_backed_live_calls") == 1
        ),
        "optional_controller_param_not_deleted": (
            capture.get("safe_to_delete_optional_controller_param_now") is False
        ),
        "page_generator_not_deleted": capture.get("safe_to_delete_page_generator_now") is False,
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Residual Shear Fallback Generator Boundary-Only Surface Deadness Snapshot",
        "",
        ...
        f"- Boundary-only live calls: `{capture.get('boundary_only_live_calls')}`",
        f"- Dependency-backed live calls: `{capture.get('dependency_backed_live_calls')}`",
...
def main() -> int:
    ...
    payload = {
        "schema": (
            "design_guide_post_click_low_bending_residual_shear_cleanup_"
            "fallback_variant_generator_boundary_only_surface_deadness_snapshot.v1"
        ),
        "created_at": stamp,
    ...
    json_path = ARTIFACT_DIR / (
        "design_guide_post_click_low_bending_residual_shear_cleanup_"
        f"fallback_variant_generator_boundary_only_surface_deadness_{stamp}.json"
    )
    audit_path = AUDIT_DIR / (
        "design_guide_post_click_low_bending_residual_shear_cleanup_"
        f"fallback_variant_generator_boundary_only_surface_deadness_{stamp}.md"
    )
