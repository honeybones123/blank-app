...
def main() -> int:
    ...
    checks = {
        ...
        and (
            "_build_canonical_design_state_pack(_guidance_state_snapshot"
            in module_fields_helper
            or "pack = build_pack(snapshot(" in module_fields_helper
        ),
        ...
        and (
            "set_shared(key, val, source=source)" in module_helper
            or "write_shared(key, val, source=source)" in module_helper
        ),
        "module_exposes_frozen_typed_runtime": "@dataclass(frozen=True)\nclass CanonicalConvenienceResyncRuntime:"
        in module_source,
