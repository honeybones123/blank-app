...
def _assert_runtime_delegate_and_binding() -> None:
    ...
    assert module._guidance_item_family_tag is not bridge._guidance_item_family_tag
    assert (
        module._guidance_item_payload_fingerprint
        is not bridge._guidance_item_payload_fingerprint
    )
    assert module._guidance_item_family_tag.__module__ == module.__name__
    assert module._guidance_item_payload_fingerprint.__module__ == module.__name__


def _assert_behavior_probe() -> None:
    from inputs_page_modules.design_guide import guidance_item_dedupe as module

    action_updates = lambda at, pl, *, state: dict(pl.get("updates") or {})
    state = {"D": 500.0}
    items = [
    ...
    out, meta = module._dedupe_guidance_items_for_display(
        items,
        state,
