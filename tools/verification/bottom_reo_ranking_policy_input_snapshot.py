    ...
    def _capture_keep_top_core(
        ...
        *,
        ...
        for candidate in candidate_list:
            ...
            dedupe_keys[identity] = _safe_value(candidate_key(candidate))
            sort_keys[identity] = _safe_value(sort_key(candidate))

        def _dominates(existing: dict, candidate: dict) -> bool:
            result = bool(dominates(existing, candidate))
            captured.setdefault("dominance_calls", []).append(
                {
                    "existing_identity": _candidate_identity(existing),
