"""Route-shape readiness using the fallback generator dependency boundary."""
...
def _capture() -> dict[str, Any]:
    ...
    dependency_trace_run = _run(
        [
            sys.executable,
            "tools/verification/design_guide_post_click_low_bending_residual_shear_cleanup_fallback_variant_generator_dependency_boundary_trace_wiring_snapshot.py",
        ]
    )
    ...
    return {
        "decision": "RESIDUAL_SHEAR_FALLBACK_VARIANT_GENERATOR_DEPENDENCY_BOUNDARY_ROUTE_SHAPE_READY",
        ...
        "helper_accepts_dependency_boundary": (
            "fallback_variant_generator_dependency_boundary: dict | None" in helper_block
        ),
        "helper_includes_dependency_name": (
            '"fallback_variant_generator_dependency_boundary"' in helper_block
        ),
        "helper_passes_dependency_boundary_to_controller": (
            "fallback_variant_generator_dependency_boundary=dict("
            in helper_block
            and "fallback_variant_generator_dependency_boundary or {}" in helper_block
        ),
        "route_assigns_dependency_boundary_payload": dependency_call >= 0,
        "route_passes_dependency_boundary_to_replacement": (
            "fallback_variant_generator_dependency_boundary=dict(" in route_block
            and "residual_fallback_variant_generator_dependency_boundary or {}" in route_block
        ),
        "dependency_boundary_before_route_body_replacement": (
            dependency_call >= 0 and route_body_call > dependency_call
        ),
        "live_generator_still_injected": (
        ...
        "next_safe_surface": "fallback_variant_generator_dependency_route_shape_cutover",
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    return {
        ...
        "live_generator_still_injected": capture.get("live_generator_still_injected") is True,
        ...
        "page_generator_not_deletable": capture.get("safe_to_delete_page_generator_now") is False,
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Residual Shear Fallback Generator Dependency Route-Shape Readiness",
        "",
...
def main() -> int:
    ...
    payload = {
        "schema": "design_guide_post_click_low_bending_residual_shear_cleanup_fallback_variant_generator_dependency_route_shape_readiness_snapshot.v1",
        "created_at": stamp,
    ...
    json_path = ARTIFACT_DIR / (
        ...
        f"fallback_variant_generator_dependency_route_shape_readiness_{stamp}.json"
    )
    audit_path = AUDIT_DIR / (
        "design_guide_post_click_low_bending_residual_shear_cleanup_"
        f"fallback_variant_generator_dependency_route_shape_readiness_{stamp}.md"
    )
    ...
    print(
        "design_guide_post_click_low_bending_residual_shear_cleanup_fallback_variant_generator_dependency_route_shape_readiness "
        + payload["status"]
    )
