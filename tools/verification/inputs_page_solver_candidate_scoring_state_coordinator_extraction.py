...
def build_payload() -> dict[str, Any]:
    ...
    static_checks = {
        ...
        "helper_preserves_mixed_rank_prefix": (
            "_one_click_mixed_direction_rank_adjustment(cur_eval, peval, mixed_direction_mode, mode_config)" in helper
            and "primary_material_improvement" in helper
        ),
        "helper_preserves_tightening_shear_penalties": (
            "web_crushing_penalty_applied += 1" in helper
            and "_design_optimisation_goal(preview)" in helper
            and "_float_from_state(preview, \"s_lig\", 0.0)" in helper
        ),
        "helper_preserves_sort_key_resolution": helper.count("_resolve_target_band_candidate_sort_key(") == 2,
