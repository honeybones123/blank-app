...
def main(argv: list[str] | None = None) -> int:
    ...
    combined_blocked_no_repair_visible = bool(
        not repair_action_visible
        and not ctas
        and (
            evidence.get("has_no_repair_evidence")
            or "no repair is executable" in text
            or
            "bending and shear repair blocked" in text
            or "combined family" in text
            or "no validated one-click update is available" in text
        )
        and (
            evidence.get("has_no_repair_evidence")
            or "family ladder checked" in text
            or
            "repair search exhausted" in text
            or "safe executable candidates: 0" in text
            or "family-owned blocker proof" in text
            or "exhaustive active-failure one-click repair search" in text
        )
    )
    positive_checks = {
        ...
        "repair_action_visible_or_blocked": repair_action_visible or combined_blocked_no_repair_visible,
        "single_primary_cta_or_blocked": len(ctas) == 1 or combined_blocked_no_repair_visible,
    }
    negative_checks = {
    ...
    report = {
        ...
        "combined_blocked_no_repair_visible": combined_blocked_no_repair_visible,
