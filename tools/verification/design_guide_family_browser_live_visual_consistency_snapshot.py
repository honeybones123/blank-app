...
def _classify_scenario(row: dict[str, Any]) -> dict[str, Any]:
    ...
    for warning in raw_warnings:
        ...
        if (
            warning_text == "pass_design_guide_has_enabled_apply_style_action_button"
            and expected_visual != "pass"
        ):
    ...
    card_attribute_disabled_no_action = False
    for card in list((row.get("design_guide") or {}).get("card_elements") or []):
        attrs = dict(card.get("attrs") or {})
        if (
            str(attrs.get("data-render-cta-enabled") or "").lower() == "false"
            and str(attrs.get("data-render-contract-enabled") or "").lower() == "false"
            and str(attrs.get("data-render-update-count") or "").strip() in {"", "0"}
            and bool(
                str(attrs.get("data-render-blocking-reason") or "").strip()
                or str(attrs.get("data-blocker-reason") or "").strip()
            )
        ):
            card_attribute_disabled_no_action = True
            break
    disabled_no_action_publication = disabled_no_action_publication or card_attribute_disabled_no_action
