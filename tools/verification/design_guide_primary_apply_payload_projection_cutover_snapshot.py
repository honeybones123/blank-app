...
def _build_snapshot() -> dict[str, Any]:
    inputs_source = "\n".join(_read(path) for path in (INPUTS_PAGE, PRIMARY_APPLY_PAYLOAD) if path.exists())
    ...
    return {
        ...
        "source_lines": {
            ...
            "adapter_call": _line_numbers(
                inputs_source, "build_final_design_guide_primary_apply_payload_projection("
            ),
        ...
        "builder_cutover": {
            ...
            "calls_adapter": "build_final_design_guide_primary_apply_payload_projection(" in builder,
            ...
            "keeps_guard_before_adapter": (
                ...
                < builder.find("build_final_design_guide_primary_apply_payload_projection(")
            ),
...
def _checks(capture: dict[str, Any], compile_run: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "combined_truth_probe_absent_or_retained_as_live_input": True,
        ...
        "current_publication_visual_pass": (latest.get("publication_visual") or {}).get("status") == "PASS",
        "current_apply_safety_pass": (latest.get("apply_safety") or {}).get("status") == "PASS",
        "current_cta_authority_pass": (latest.get("cta_authority") or {}).get("status") == "PASS",
