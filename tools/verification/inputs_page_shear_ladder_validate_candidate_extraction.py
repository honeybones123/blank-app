...
def _bind() -> None:
    recommendation_compute._bind_named_recommendation_globals(
        legacy_page=_Legacy(),
        names=recommendation_compute._SHEAR_RECOMMENDATION_NAMES,
    )
...
def main() -> int:
    ...
    checks = {
        ...
        "bridge_binds_recommendation_dependencies": "_bind_named_recommendation_globals(" in bridge_helper,
        ...
        "recommendation_module_binds_lower_level_validator_dependencies": all(
            name in module_source.split("def _bind_named_recommendation_globals", 1)[0]
            for name in (
