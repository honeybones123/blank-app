"""Verify broad direct target-band geometry plan construction is service-backed."""
...
SERVICE_HELPER = "build_direct_target_band_broad_geometry_plan"
...
def _legacy_plan(width_values: list[Any], depth_values: list[Any]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for width in list(width_values or []):
        for depth in list(depth_values or []):
            rows.append({"width": width, "depth": depth})
    return rows


def _parity() -> dict[str, Any]:
    from design_brain.candidate_evaluation import build_direct_target_band_broad_geometry_plan

    cases = [
        ("empty", [], []),
        ("single_width_multi_depth", [300.0], [500.0, 450.0, 400.0]),
        ("multi_width_multi_depth", [400.0, 350.0], [650.0, 600.0]),
    ]
    rows: list[dict[str, Any]] = []
    for name, widths, depths in cases:
        expected = _legacy_plan(widths, depths)
        actual = build_direct_target_band_broad_geometry_plan(width_values=widths, depth_values=depths)
        rows.append(
            {
                "case": name,
...
def _capture() -> dict[str, Any]:
    ...
    source_checks = {
        ...
        "target_no_longer_owns_nested_width_depth_plan_loop": "for width in width_values:" not in target_source
        and "for depth in depth_values:" not in target_source,
        "target_iterates_service_geometry_plan": "for geometry_row in geometry_plan:" in target_source,
        "geometry_state_mutation_remains_page_owned": "_geometry_state_with_updates(base, width=width, depth=depth)" in target_source,
        "bottom_trial_generation_remains_page_owned": all(
    ...
    return {
        "schema": "design_guide_direct_target_broad_geometry_plan_boundary.v1",
        "target": {
            "name": TARGET,
        ...
        "next_safe_slice": "prebuilt_bottom_trial_packaging_boundary_or_direct_target_final_selection_policy_extraction",
    }
...
def _write(payload: dict[str, Any], checks: dict[str, bool]) -> tuple[Path, Path]:
    ...
    json_path = ARTIFACT_DIR / f"design_guide_direct_target_broad_geometry_plan_boundary_{suffix}.json"
    report_path = AUDIT_DIR / f"design_guide_direct_target_broad_geometry_plan_boundary_{suffix}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    lines = [
        "# Design Guide Direct Target Broad Geometry Plan Boundary",
        "",
...
def main() -> int:
    ...
    print(f"design_guide_direct_target_broad_geometry_plan_boundary {status}")
