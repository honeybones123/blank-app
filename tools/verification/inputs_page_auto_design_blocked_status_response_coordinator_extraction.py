"""Verify auto-design blocked status response coordinator extraction."""
...
def _run_case(module: Any) -> dict[str, Any]:
    ...
    try:
        ...
        module._attach_no_action_visibility_coordinator = lambda **kwargs: (
            ...
            or {
                "user_visible_no_action_reason": "visible blocked action",
                "one_click_no_action_reason_code": "blocked_eval",
            }
        )
        ...
        dbg = {
            "current_fail_fingerprint": {"shear": "FAIL"},
            "current_fail_keys": ["shear"],
        }
        result = module._return_auto_design_blocked_status_response_coordinator(
...
def build_payload() -> dict[str, Any]:
    ...
    helper_start, helper_end, helper = _function_segment(
        source,
        "_return_auto_design_blocked_status_response_coordinator",
    )
    ...
    runtime_checks = {
        "trace_run_end_preserved": runtime["traces"][0]["status"] == "blocked"
        ...
        and feedback["reason"] == "solver_blocked"
        ...
        and envelope["blocked_reason"] == "solver_blocked"
        and envelope["commit_eligible"] is False,
        "latch_reason_preserved": runtime["result"]["latch_reason"] == "run_one_click_auto_design:blocked",
        "returned_payload_shape_preserved": payload["status"] == "blocked"
        and payload["stop_reason"] == "solver_blocked"
        ...
        and payload["user_visible_no_action_reason"] == "visible blocked action"
        and payload["one_click_no_action_reason_code"] == "blocked_eval",
    }
    static_checks = {
        "helper_present": "def _return_auto_design_blocked_status_response_coordinator(" in source,
        "helper_preserves_latch_reason": "run_one_click_auto_design:blocked" in helper,
        ...
        "dispatch_delegates_blocked_status_response_dispatch": (
            "_dispatch_auto_design_blocked_status_response_from_final_response_coordinator("
            in dispatch_body
        ),
        "blocked_status_response_dispatch_delegates_response": (
            "_return_auto_design_blocked_status_response_coordinator(" in blocked_status_dispatch
            and "final_response_scope[" in blocked_status_dispatch
        ),
        "run_delegates_final_tail": "_finish_auto_design_post_current_eval_and_dispatch_coordinator(" in run_body,
        "dispatch_preserves_other_return_branches": 'if out_status == "failed":' in dispatch_body
        and 'if out_status == "no_action" and stop_reason == "already_in_band":' in dispatch_body
    ...
    return {
        "status": status,
        "surface": "run_one_click_auto_design_blocked_status_response_coordinator",
        "helper_segment": {
            "function": "_return_auto_design_blocked_status_response_coordinator",
        ...
        "next_safe_slice": "extract already-in-band pass final return branch from run_one_click_auto_design",
    }
...
def write_artifacts(payload: dict[str, Any]) -> tuple[Path, Path]:
    ...
    json_path = ARTIFACT_DIR / f"inputs_page_auto_design_blocked_status_response_coordinator_extraction_{stamp}.json"
    md_path = AUDIT_DIR / f"inputs_page_auto_design_blocked_status_response_coordinator_extraction_{stamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    lines = [
        "# Auto Design Blocked Status Response Coordinator Extraction",
        "",
