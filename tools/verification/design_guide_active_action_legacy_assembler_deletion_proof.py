"""Prove the legacy active-action final-visible assembler can be deleted."""
...
ASSEMBLER = "_assemble_final_visible_active_action_result"
...
def _capture() -> dict[str, Any]:
    ...
    return {
        ...
        "decision": decision,
        "verification": {
            "active_action_cutover": _run(
                "tools/verification/design_guide_active_action_result_cutover.py"
            ),
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "active_action_cutover_passed": (verification.get("active_action_cutover") or {}).get(
            "passed"
        )
        is True,
        "inventory_audit_passed_or_deleted_state": (
            decision == "DELETED"
            or (verification.get("remaining_inventory") or {}).get("passed") is True
        ),
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    assembler = capture.get("assembler") or {}
    lines = [
        "# Design Guide Active-Action Legacy Assembler Deletion Proof",
        "",
    ...
    lines.extend(
        [
            ...
            "## Recommendation",
            "",
            "Delete only `_assemble_final_visible_active_action_result(...)` when decision is "
            "`READY_TO_DELETE`. Once deleted, update the inventory audit to accept zero remaining "
            "final-visible assemblers.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    stamp = _stamp()
    json_path = ARTIFACT_DIR / f"design_guide_active_action_legacy_assembler_deletion_{stamp}.json"
    report_path = AUDIT_DIR / f"design_guide_active_action_legacy_assembler_deletion_{stamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_active_action_legacy_assembler_deletion {status}")
