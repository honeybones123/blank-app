...
PACKAGE_INIT = ROOT / "design_brain" / "families" / "shear_fail_governs" / "__init__.py"
...
def main() -> int:
    ...
    if not PACKAGE_INIT.exists():
        failures.append("missing_shear_fail_governs_package_init")

    source = PACKAGE_INIT.read_text(encoding="utf-8", errors="replace") if PACKAGE_INIT.exists() else ""
    if "inputs_page" in source:
        failures.append("shear_fail_governs_imports_inputs_page")
    if "def evaluate_shear_fail_governs" not in source:
        failures.append("missing_evaluate_shear_fail_governs")

    engine_hits = _line_hits(ROOT / "design_brain" / "engine.py", "evaluate_shear_fail_governs")
    inputs_hits = _line_hits(ROOT / "inputs_page.py", "evaluate_shear_fail_governs")
    if engine_hits:
        failures.append("engine_calls_new_shear_fail_governs_api")
    if inputs_hits:
        failures.append("inputs_page_calls_new_shear_fail_governs_api")

    result_summary: dict[str, Any] = {}
    if not failures:
        from design_brain.families.shear_fail_governs import evaluate_shear_fail_governs
        from design_brain.shared.schemas import FamilyResult

        result = evaluate_shear_fail_governs(
            {
                "summary": {
                    "statuses": {"shear": "FAIL", "bending": "PASS"},
                    "utils": {"shear": 1.18, "bending": 0.72},
                },
                "evidence": {
        ...
        if result_summary.get("family_id") != "SHEAR_FAIL_GOVERNS":
            failures.append("family_id_mismatch")
        if result_summary.get("is_applicable") is not True:
            failures.append("synthetic_shear_fail_context_not_applicable")
    ...
    output = {
        "schema": "design_brain_shear_fail_governs_api_check.v1",
        "status": status,
    ...
    output_path = ARTIFACT_DIR / f"design_brain_shear_fail_governs_api_check_{stamp}.json"
    output_path.write_text(json.dumps(output, indent=2, sort_keys=True), encoding="utf-8")
    report_path = AUDIT_DIR / f"design_brain_shear_fail_governs_api_check_{stamp}.md"
    report_lines = [
        "# Design Brain SHEAR_FAIL_GOVERNS API Check",
        "",
        f"Status: {status}",
        "",
        "## Result Summary",
