...
def main() -> int:
    from design_brain.families.shear_fail_governs import run_shear_fail_governs_ladder_runtime
    from design_brain.families.shear_fail_governs.runtime import (
        shear_fail_governs_contract_lane_order,
    )

    deleted_name = "evaluate_" + "shear_fail_governs"
    failures: list[str] = []
    ...
    checks = {
        ...
        "runtime_exported": "run_shear_fail_governs_ladder_runtime" in source,
        ...
        "contract_lane_order_available": bool(shear_fail_governs_contract_lane_order()),
    }
    ...
    output = {
        "schema": "design_brain_shear_fail_governs_runtime_export_check.v1",
        "status": status,
    ...
    report_path.write_text(
        "\n".join(
            [
                "# Design Brain SHEAR_FAIL_GOVERNS Runtime Export Check",
                "",
                f"Status: {status}",
                "",
