"""Speed check for COMBINED_BENDING_SHEAR_FAIL product-path migration."""
...
def _latest_trace_report(started_at: float) -> Path | None:
    reports = sorted(
        ARTIFACT_DIR.glob("inputs_pre_widget_trace_*.jsonl"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    for path in reports:
...
def main(argv: list[str] | None = None) -> int:
    ...
    speed = {
        "schema": "combined_family_speed_check.v1",
        ...
        "candidate_evaluation_count": (
            evidence.get("family_ladder_candidate_count")
            or evidence.get("combined_fail_contract_ladder_candidate_count")
            or (evidence.get("candidate_search_evidence") or {}).get(
                "combined_fail_contract_ladder_candidate_count"
            )
        ),
        ...
        "speed_isolation": {
            "family_selected_early": bool(
                evidence.get("family_early_dispatch_used")
                or (evidence.get("candidate_search_evidence") or {}).get("family_early_dispatch_used")
            ),
    ...
    path = ARTIFACT_DIR / f"combined_family_speed_check_{time.strftime('%Y-%m-%dT%H-%M-%S')}.json"
