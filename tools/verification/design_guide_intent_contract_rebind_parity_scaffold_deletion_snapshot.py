"""Deletion snapshot for post-cutover intent-contract parity scaffolding."""
...
def _window(lines: list[str], center: int, *, before: int = 80, after: int = 120) -> str:
    if center < 0:
        return ""
...
def _capture() -> dict[str, Any]:
    ...
    post_proof_context = "\n".join(lines[proof_line : min(len(lines), proof_line + 150)]) if proof_line >= 0 else ""
    ...
    return {
        "decision": "INTENT_CONTRACT_REBIND_PARITY_SCAFFOLD_DELETED_AFTER_CUTOVER",
        ...
        "source_checks": {
            "proof_object_still_drives_tail": all(
                token in context
                for token in (
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Intent Contract Rebind Parity Scaffold Deletion Snapshot",
        "",
...
def main() -> int:
    ...
    payload = {
        "schema": "design_guide_intent_contract_rebind_parity_scaffold_deletion_snapshot.v1",
        "status": status,
        "created_at": stamp,
        "capture": capture,
        "checks": checks,
    ...
    json_path = ARTIFACT_DIR / f"design_guide_intent_contract_rebind_parity_scaffold_deletion_{stamp}.json"
    report_path = AUDIT_DIR / f"design_guide_intent_contract_rebind_parity_scaffold_deletion_{stamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_intent_contract_rebind_parity_scaffold_deletion_snapshot {status}")
