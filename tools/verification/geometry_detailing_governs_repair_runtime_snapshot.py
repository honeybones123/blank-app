...
def _static_checks() -> dict[str, Any]:
    ...
    checks = {
        ...
        "inputs_imports_family_api": "_evaluate_geometry_detailing_governs" in inputs_source,
        "inputs_has_guidance_adapter": "def _geometry_detailing_governs_guidance_item" in inputs_source,
        "inputs_early_dispatch_geometry": 'early_governing_state == "GEOMETRY_DETAILING_GOVERNS"' in inputs_source,
        "inputs_uses_existing_apply_binding": re.search(
            r"geometry_detailing_family_early_dispatch.*?_design_guide_apply_button_contracts_to_items",
            inputs_source,
            flags=re.S,
        )
        is not None,
        "inputs_does_not_move_render_apply_ownership": "geometry_detailing_family_early_dispatch" in inputs_source
        and "apply_resolved_candidate" in inputs_source,
    }
    return {
        "checks": checks,
