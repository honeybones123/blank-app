...
APPROVED_PAGE_SHELL_FUNCTIONS = {
    "render_inputs_calculation_explainer_trace_coordinator": "trace_only_page_shell",
}
...
def main() -> int:
    ...
    live_surface = source + "\n" + (_read(ROUTE_COORDINATORS) if ROUTE_COORDINATORS.exists() else "")
    ranges = _function_ranges(live_surface)
    page_surfaces = [
        {
            "name": name,
    ...
    checks = {
        "approved_page_shell_function_present": all(name in ranges for name in APPROVED_PAGE_SHELL_FUNCTIONS),
        "page_delegates_to_calculation_trace_module": all(token in live_surface for token in APPROVED_PAGE_SHELL_TOKENS),
    ...
    next_safe_slice = (
        "Mark Calculations & Explainers as LOCKED for `inputs_page.py` modularisation, then start Widgets Phase 0."
        if not failures
