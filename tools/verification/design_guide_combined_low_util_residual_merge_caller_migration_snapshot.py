"""Proof snapshot for residual active-action combined low-util caller migration."""
...
CALLER = "_merge_residual_cleanup_for_final_visible_active_action"
...
def _capture() -> dict[str, Any]:
    ...
    return {
        ...
        "caller_preserves_exception_guard": "except Exception:" in caller_source,
        "caller_preserves_merge_contract_update": 'contract["updates"] = dict(updates)' in caller_source,
        "remaining_direct_helper_calls": direct_hits,
        "remaining_direct_helper_call_count": len(direct_hits),
        "maximum_remaining_direct_helper_call_count": 3,
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    return {
        ...
        "caller_preserves_exception_guard": bool(capture.get("caller_preserves_exception_guard")),
        "caller_preserves_merge_contract_update": bool(capture.get("caller_preserves_merge_contract_update")),
...
def _write_report(payload: dict[str, Any], report_path: Path) -> None:
    ...
    lines = [
        "# Combined Low-Util Residual Merge Caller Migration Snapshot",
        "",
...
def main() -> int:
    ...
    payload = {
        "status": status,
        "snapshot": "design_guide_combined_low_util_residual_merge_caller_migration",
    ...
    stamp = _stamp()
    json_path = ARTIFACT_DIR / f"design_guide_combined_low_util_residual_merge_caller_migration_{stamp}.json"
    report_path = AUDIT_DIR / f"design_guide_combined_low_util_residual_merge_caller_migration_{stamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    _write_report(payload, report_path)
    print(f"design_guide_combined_low_util_residual_merge_caller_migration {status}")
