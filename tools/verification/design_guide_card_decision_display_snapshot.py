"""Snapshot resolved Design Guide card decision/display fields for product scenarios."""
...
def _selected_row(rows: list[dict[str, Any]]) -> dict[str, Any]:
    for row in reversed(rows):
        if str(row.get("source") or "") == "build_design_guide_card_decision_display_fields":
            return row
...
def _run_scenario(label: str, *, base_port: int, index: int, timestamp: str) -> dict[str, Any]:
    ...
    runtime_path = ARTIFACT_DIR / f"design_guide_card_decision_display_runtime_{timestamp}_{label.lower()}.jsonl"
    if runtime_path.exists():
        runtime_path.unlink()
    env = dict(os.environ)
    env.pop("CODEX_BROWSER_TEST_MODE", None)
    env.update(dict(spec.get("env") or {}))
    env["DESIGN_GUIDE_CARD_DECISION_DISPLAY_SNAPSHOT_PATH"] = str(runtime_path)
    command = [
        sys.executable,
    ...
    if not selected:
        failures.append("decision_display_snapshot_missing")
    evidence_family = str(evidence.get("selected_family_id") or "")
    if evidence_family and evidence_family != expected_family:
        failures.append("evidence_family_mismatch")
    return {
        "label": label,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--port", type=int, default=10521)
    parser.add_argument(
        "--scenario",
        action="append",
    ...
    report = {
        "schema": "design_guide_card_decision_display_snapshot.v1",
        "status": status,
        "results": results,
    }
    output = ARTIFACT_DIR / f"design_guide_card_decision_display_snapshot_{timestamp}.json"
