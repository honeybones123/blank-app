...
CALLSITES = {
    "early_shear_cleanup_direct_action_shell": {
        ...
        "expected_classification": "deleted_or_rehomed_zero_shell_callsite",
    },
...
def _classify_callsite(window: str, projection_var: str) -> tuple[str, str]:
    ...
    return (
        "deleted_or_rehomed_zero_shell_callsite",
        "Historical shell callsite assignment is absent from the live Inputs shell.",
    )
...
def build_snapshot() -> dict[str, Any]:
    ...
    summary = {
        ...
        "deleted_or_rehomed_zero_shell_callsite": sum(
            1
            for row in callsite_rows
            if row["classification"] == "deleted_or_rehomed_zero_shell_callsite"
        ),
    ...
    return {
        ...
        "recommended_next_slice": (
            "Fallback-shell helper retired; all tracked direct-shell callsites are absent from the live Inputs shell."
        ),
