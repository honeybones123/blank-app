...
RETAINED_SURFACES: tuple[dict[str, Any], ...] = (
    ...
    {
        "surface": "direct shell projection callsites",
        "token": "_build_final_design_guide_direct_shell_card_projection(",
        "path": INPUTS_PAGE,
        "classification": "KEEP_RENDER_SHELL_CONSUMER",
        "owner": "page/UI render shell",
        "reason": "latest callsite classifier marks all tracked callsites cutover-complete direct-shell consumers and safe_delete_now is zero",
        "delete_now": False,
    },
)
        ...
        rows.append(
            {
                ...
                "present": bool(lines),
                "line_numbers": lines,
                "classification": spec["classification"] if lines else "MISSING_RETAINED_SURFACE",
                "owner": spec["owner"],
                "reason": spec["reason"],
...
def _snapshot(command_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if compute_capture.get("controller_shell_retained_non_authoritative") is not True:
        blockers.append("controller compute resolver fallback shell is not proven retained/non-authoritative")
