...
def _commit_number_input_like_user(
    ...
    *,
    ...
    def _wait_target() -> tuple[dict[str, Any], bool, dict[str, Any]]:
        if mu is not None and vu is not None:
            return _wait_for_settled_preclick_state(page, mu=mu, vu=vu, timeout_s=reconcile_timeout_s)
        if mu is not None:
            state, ok = _wait_for_partial_state(page, mu=mu, timeout_s=reconcile_timeout_s)
            return state, ok, {"settle_wait_time_ms": 0, "poll_cycles": 0, "stability_multiple_cycles": False}
        if vu is not None:
            state, ok = _wait_for_partial_state(page, vu=vu, timeout_s=reconcile_timeout_s)
            return state, ok, {"settle_wait_time_ms": 0, "poll_cycles": 0, "stability_multiple_cycles": False}
    ...
    for method_name, method in commit_methods:
        ...
        try:
            ...
            pass
        time.sleep(0.5)
        last_state, matched, meta = _wait_target()
        ...
        if before_sig is not None and after_sig is not None and before_sig != after_sig:
            meta = dict(meta)
            meta["commit_method"] = f"{method_name}:rerun_only"
            return last_state, True, f"{method_name}:rerun_only", meta
...
def _apply_live_inputs(page, *, mu: float, vu: float) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for _ in range(1):
        ...
        if matched or _input_dom_matches(page, MU_LABEL, mu):
            if not matched:
                pre_settle_meta["mu"]["dom_committed_without_published_state"] = True
            break
    else:
        raise RuntimeError(
            f"Mu edit did not reconcile into shared/published state. "
            f"expected Mu={mu}, probe={dict(last_state.get('summary_state_probe') or {})}, "
    ...
    for _ in range(1):
        ...
        if matched or _input_dom_matches(page, VU_LABEL, vu):
            if not matched:
                pre_settle_meta["vu"]["dom_committed_without_published_state"] = True
...
def _page_cycle_click_page(page, slug: str, label: str, *, timeout_s: float = 20.0) -> dict[str, Any]:
    ...
    def _timed_out(selector: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "selector": selector,
            "classification": PAGE_CYCLE_NAVIGATION_TIMEOUT_CLASS,
    ...
    *,
    ...
    for index, (slug, page_label) in enumerate(page_sequence):
        ...
        if slug == "inputs":
            ...
            missing_controls = sorted(key for key, present in controls.items() if not present)
            if missing_controls:
