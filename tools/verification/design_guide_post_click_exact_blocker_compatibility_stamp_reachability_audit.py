"""Audit reachability for post-click exact blocker compatibility stamp rows."""
...
HELPER = "_stamp_final_publication_post_click_exact_blocker_compatibility_proof"
TOKENS = (
    HELPER,
    "final_publication_post_click_exact_blocker_compatibility_proofs",
    "final_publication_post_click_exact_blocker_compatibility_proof_hash",
    "final_publication_post_click_exact_blocker_rows_compatibility_only",
    "final_publication_post_click_exact_blocker_remaining_truth_narrowed",
)
PRODUCT_ROOTS = (
    ROOT / "inputs_page.py",
    ROOT / "design_brain",
...
def _capture() -> dict[str, Any]:
    ...
    return {
        ...
        "next_safe_step": (
            "Delete the product post-click exact blocker compatibility stamp helper/call if "
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Post-Click Exact Blocker Compatibility Stamp Reachability Audit",
        "",
...
def main() -> int:
    ...
    json_path = (
        ARTIFACT_DIR
        / f"design_guide_post_click_exact_blocker_compatibility_stamp_reachability_{stamp}.json"
    )
    report_path = (
        AUDIT_DIR
        / f"design_guide_post_click_exact_blocker_compatibility_stamp_reachability_{stamp}.md"
    )
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_post_click_exact_blocker_compatibility_stamp_reachability {status}")
