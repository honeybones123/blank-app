"""Proof snapshot for bending-restore combined low-util caller migration."""
...
ANCHOR = "_bending_restore_orchestration = _run_design_guide_combined_low_util_orchestration("
...
def _window(source_lines: list[str], anchor: str, before: int = 35, after: int = 70) -> dict[str, Any]:
    for index, line in enumerate(source_lines, start=1):
        if anchor in line:
...
def _capture() -> dict[str, Any]:
    ...
    bending_window = _window(source_lines, ANCHOR)
    bending_text = str(bending_window.get("text") or "")
    return {
        "bending_restore_window": {
            key: value
            for key, value in bending_window.items()
            if key != "text"
        },
        "bending_restore_uses_controller_wrapper": CONTROLLER_TOKEN in bending_text,
        "bending_restore_old_helper_call_count": bending_text.count(f"{THIN_ADAPTER}("),
        "bending_restore_preserves_exception_guard": "except Exception:" in bending_text,
        "bending_restore_updates_debug_trace": "debug_trace.update(" in bending_text,
        "bending_restore_reads_controller_item": 'get("item")' in bending_text,
        "bending_restore_preserves_normalisation": (
            "_bending_restore_combined = normalise_final_visible_design_guide_item(" in bending_text
        ),
        "bending_restore_preserves_display_truth_apply": (
            "_design_guide_apply_display_truth_to_items(" in bending_text
        ),
        "remaining_direct_helper_calls": direct_hits,
        "remaining_direct_helper_call_count": len(direct_hits),
        "maximum_remaining_direct_helper_call_count": 2,
...
def _write_report(payload: dict[str, Any], report_path: Path) -> None:
    ...
    window = dict(capture.get("bending_restore_window") or {})
    lines = [
        "# Combined Low-Util Bending Restore Caller Migration Snapshot",
        "",
        f"Status: `{payload.get('status')}`",
        f"Snapshot hash: `{payload.get('snapshot_hash')}`",
        "",
        "## Bending Restore Window",
        "",
...
def main() -> int:
    ...
    payload = {
        "status": status,
        "snapshot": "design_guide_combined_low_util_bending_restore_caller_migration",
    ...
    stamp = _stamp()
    json_path = ARTIFACT_DIR / f"design_guide_combined_low_util_bending_restore_caller_migration_{stamp}.json"
    report_path = AUDIT_DIR / f"design_guide_combined_low_util_bending_restore_caller_migration_{stamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    _write_report(payload, report_path)
    print(f"design_guide_combined_low_util_bending_restore_caller_migration {status}")
