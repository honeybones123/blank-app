...
def main() -> int:
    ...
    json_path = ARTIFACT_DIR / (
        f"inputs_page_post_cleanup_terminal_residual_width_selection_{timestamp}.json"
    )
    report_path = AUDIT_DIR / (
        f"inputs_page_post_cleanup_terminal_residual_width_selection_{timestamp}.md"
    )
    ...
    def run_case(
        ...
        *,
        ...
        result = inputs_page.render_design_guide_post_cleanup_terminal_residual_width_selection(
            guidance_debug=dict(debug or {}),
            dg_overview=dict(overview or {}),
            guidance_disp_state=dict(state or {}),
        )
        item, contract, updates, match_state, rendered_as_residual = result
        case = {
            "name": name,
            "item": item,
            "contract": contract,
            ...
            "rendered_as_residual": rendered_as_residual,
            "events": list(events),
        }
        cases.append(case)
        return case
    ...
    try:
        ...
        expect(
            ...
            and case["rendered_as_residual"] is True
            and case["events"][0]["overview"] == {"source": "debug"},
            f"case={case}",
        )
        ...
        expect(
            ...
            and case["rendered_as_residual"] is False
    ...
    report_path.write_text(
        "\n".join(
            [
                "# Inputs Page Post Cleanup Terminal Residual Width Selection Verifier",
                "",
