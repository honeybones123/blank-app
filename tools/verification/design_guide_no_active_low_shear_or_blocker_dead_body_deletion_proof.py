"""Dead-body/deletion proof for the no-active low-shear/blocker page route.

This verifier supports both states:
...
def _capture() -> dict[str, Any]:
    ...
    dead_body_tokens = {
        "zero_shear_gate": "final_zero_shear_post_click_accepted" in source,
        "low_shear_probe": "resolve_low_shear_target_cleanup_probe_fn(" in source,
        "combined_green_audit": "post_click_accepted_green_audit_fn(" in source,
    }
    route_deleted = node is None
    return {
        "decision": (
            "DELETION_COMPLETE"
            if route_deleted
        ...
        "page_route": {
            ...
            "dead_body_tokens_present": dead_body_tokens,
        },
        "latest": {
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        "page_route_present_or_deleted": route.get("present") is True
        or route.get("deleted") is True,
        ...
        "old_body_state_matches_decision": (
            ...
            or (
                ...
                and (
                    route.get("old_body_after_delegating_return_present") is False
                    or route.get("deleted") is True
                )
            )
        ),
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    route = dict(capture.get("page_route") or {})
    lines = [
        "# Design Guide No-Active Low-Shear/Blocker Dead Body Deletion Proof",
        "",
...
def main() -> int:
    ...
    json_path = (
        ARTIFACT_DIR
        / f"design_guide_no_active_low_shear_or_blocker_dead_body_deletion_proof_{stamp}.json"
    )
    report_path = (
        AUDIT_DIR
        / f"design_guide_no_active_low_shear_or_blocker_dead_body_deletion_proof_{stamp}.md"
    )
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_no_active_low_shear_or_blocker_dead_body_deletion_proof {status}")
