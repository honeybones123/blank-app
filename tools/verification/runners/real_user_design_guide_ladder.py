...
ACTIVE_FAILURE_MATRIX_CASES: list[dict[str, Any]] = [
    ...
    {
        "case_id": "COMBINED_UNDERDESIGN_SEEDED_BLOCKED_PUBLICATION",
        "recipe": "C_combined_underdesign",
        "intent": "active_failure_matrix",
        "inputs": {"mu": 300.0, "vu": 400.0},
        "skip_live_input_edit": True,
        "expect": _matrix_expect(
            active_failures=["bending", "shear"],
            primary_families=["bending", "shear", "combined"],
            title_contains="Bending and shear repair blocked",
        ),
    },
    {
...
def _active_repair_blocker_evidence_is_real(evidence: dict[str, Any], text: Any) -> bool:
    ...
    if has_exact:
        try:
            exact_payload = dict(next(iter(dict(exact).values())) or {})
        except Exception:
            exact_payload = {}
    failed_check_name = str(evidence.get("failed_check_name") or exact_payload.get("failed_check_name") or "").strip()
    failed_check_status = str(evidence.get("failed_check_status") or exact_payload.get("failed_check_status") or "").strip().upper()
    attempted_updates = evidence.get("attempted_updates") or exact_payload.get("attempted_updates")
...
def _active_under_capacity_partial_blocker_failures(
    ...
    badges = [str(value or "").strip().upper() for value in list(snapshot.get("visible_card_badges") or [])]
    ...
    failed_preview_contract = bool(
        blocking_reason == "candidate_preview_has_fail_status"
        or (
            contract.get("preview_pass") is False
            and dict(contract.get("updates") or {})
            and "preview utilisation" in lower
        )
    )
    if failed_preview_contract:
        failures: list[str] = []
        if first_badge and first_badge != "BLOCKED":
            failures.append(f"active_under_capacity_failed_preview_visible_badge_not_blocked:{first_badge}")
        if "recommendation is advisory, not directly executable" in lower and "button contract:" in lower:
            failures.append("active_under_capacity_failed_preview_visible_as_advisory_contract")
        if "candidate_preview_has_fail_status" in lower:
            failures.append("active_under_capacity_failed_preview_contract_reason_visible")
        if not has_real_blocker:
            failures.append("active_under_capacity_failed_preview_without_real_engineering_blocker")
        if failures:
            return failures
...
def _blocked_publication_visible_contract_failures(
    snapshot: dict[str, Any],
    state: dict[str, Any],
) -> list[str]:
    publication = dict(state.get("final_publication_verifier_payload") or {})
    display = dict(publication.get("display") or {})
    cta = dict(publication.get("cta") or {})
    published_blocked = bool(
        str(publication.get("outcome_state") or publication.get("status") or "").strip().upper() == "BLOCKED"
        or str(display.get("badge") or display.get("status") or display.get("display_state") or "").strip().upper() == "BLOCKED"
    )
    if not published_blocked:
        return []
    failures: list[str] = []
    badges = [str(value or "").strip().upper() for value in list(snapshot.get("visible_card_badges") or [])]
    first_badge = badges[0] if badges else ""
    ...
    if first_badge and first_badge != "BLOCKED":
        failures.append(f"blocked_publication_visible_badge_mismatch:{first_badge}")
    if "recommendation is advisory, not directly executable" in text_lower and "button contract:" in text_lower:
        failures.append("blocked_publication_visible_as_advisory_failed_preview")
    disabled_reason = str(
        cta.get("disabled_reason")
        or cta.get("blocking_reason")
        or dict(_rendered_guidance_probe(state).get("primary_button_contract") or {}).get("blocking_reason")
        or ""
    ).strip()
    if disabled_reason == "candidate_preview_has_fail_status" and "candidate_preview_has_fail_status" in text_lower:
        failures.append("blocked_publication_leaks_failed_preview_contract_reason")
    body_lower = _norm_text(snapshot.get("body_text") or snapshot.get("design_guide_visible_text") or "").lower()
    duplicate_phrase = "no validated one-click update is available for this state"
    duplicate_count = body_lower.count(duplicate_phrase)
    if duplicate_count > 1:
        failures.append(f"blocked_publication_duplicate_visible_blocked_explanation:{duplicate_count}")
    if disabled_reason == "candidate_preview_has_fail_status":
        evidence = _candidate_search_evidence_from_state(state)
        has_real_blocker = _active_under_capacity_blocker_is_real(
            evidence,
            snapshot.get("design_guide_visible_text") or "",
        )
        if not has_real_blocker:
            failures.append("blocked_publication_failed_preview_without_family_exhaustion_proof")
        if "family-owned blocker proof" not in body_lower:
            failures.append("blocked_publication_failed_preview_proof_not_visible")
        if "repair search exhausted" not in body_lower:
            failures.append("blocked_publication_failed_preview_exhaustion_not_visible")
    return failures
...
def _candidate_search_evidence_from_state(state: dict[str, Any]) -> dict[str, Any]:
    ...
    def evidence_from_exact_maps(*sources: dict[str, Any]) -> dict[str, Any]:
        exact: dict[str, Any] = {}
        for source in sources:
            if not isinstance(source, dict):
                continue
            for key in ("post_click_exact_blockers_by_family", "exact_blockers_by_family"):
                value = source.get(key)
                if isinstance(value, dict) and value:
                    exact.update(dict(value))
        if not exact:
            return {}
        family = str(
            contract.get("family")
            or rendered.get("selected_family")
            or rendered.get("selected_family_id")
            or ""
        ).strip().lower()
        blocker = dict(exact.get(family) or next(iter(exact.values()), {}) or {})
        active_failures = list(
            blocker.get("active_failures")
            or sorted(str(key or "").strip().lower() for key in exact.keys() if str(key or "").strip())
        )
        reason = str(
            blocker.get("reason")
        ...
        return {
            ...
            "repair_search_ran": bool(blocker.get("repair_search_ran", True)),
            "repair_search_exhaustive": bool(blocker.get("repair_search_exhaustive", True)),
            "cleanup_search_ran": bool(blocker.get("cleanup_search_ran", blocker.get("local_cleanup_search_ran", False))),
            "cleanup_search_exhaustive": bool(
                blocker.get("cleanup_search_exhaustive", blocker.get("local_cleanup_search_exhaustive", False))
            ),
            "active_under_capacity_blocker": True,
            "active_under_capacity_blocker_family": blocker.get("family") or family or next(iter(exact.keys()), ""),
            "active_under_capacity_blocker_reason": reason,
            "outside_target_band_allowed": False,
            "outside_target_band_allowed_reason": reason,
            "outside_target_band_allowed_category": (
                blocker.get("outside_target_band_allowed_category")
                or f"{str(blocker.get('family') or family or next(iter(exact.keys()), 'active')).strip().lower()}_would_fail"
            ),
            "active_failures": list(active_failures),
            "safe_candidate_count": int(blocker.get("safe_candidate_count") or blocker.get("safe_repair_candidate_count") or 0),
            "executable_candidate_count": int(blocker.get("executable_candidate_count") or blocker.get("executable_repair_candidate_count") or 0),
            "executable_target_band_candidate_count": int(blocker.get("executable_target_band_candidate_count") or 0),
            "failed_candidate_reasons": [reason] if reason else [],
            "blocker_reasons_by_family": {
                key: [str(dict(value or {}).get("reason") or reason)]
            ...
            "attempted_candidate_id": blocker.get("attempted_candidate_id"),
            "attempted_updates": dict(blocker.get("attempted_updates") or {}),
            "failed_check_name": blocker.get("failed_check_name"),
            "failed_check_status": blocker.get("failed_check_status"),
            "failed_check_util": blocker.get("failed_check_util"),
            "failed_check_demand": blocker.get("failed_check_demand"),
            "failed_check_capacity_or_limit": blocker.get("failed_check_capacity_or_limit"),
        }

    exact_map_evidence = evidence_from_exact_maps(design_probe, debug_bundle, guidance, rendered)
    candidates = [
        exact_map_evidence,
...
def _visible_dom_state(page) -> dict[str, Any]:
    return page.evaluate(
        ...
        () => {
          ...
          const cardBadges = cardEls.map((el) => {
            const badge = el.querySelector('.fast-guidance-badge');
          ...
          return {
            ...
            design_guide_card_badges: cardBadges,
...
def _visible_snapshot(page) -> dict[str, Any]:
    ...
    return {
        ...
        "body_text": str(dom.get("body_text") or ""),
        ...
        "visible_card_badges": list(dom.get("design_guide_card_badges") or []),
...
def _validate_visible_summary(
    ...
    if summary.get("parse_failed"):
        ...
        return failures
    overview = dict((state or {}).get("summary_overview_probe") or {})
    overview_statuses = {
        str(key or "").strip().lower(): str(value or "").strip().upper()
        for key, value in dict(overview.get("statuses") or {}).items()
    }
    overview_utils = dict(overview.get("utils") or {})

    def _row_has_visible_value(row: dict[str, Any]) -> bool:
        return any(row.get(key) not in (None, "", "—", "-") for key in ("capacity", "demand", "util"))

    def status_of(name: str) -> str | None:
        row = bending if name == "bending" else shear
        dom_status = str(row.get("status") or "").upper() or None
        if dom_status == "INFO" and not _row_has_visible_value(row) and overview_statuses.get(name):
            return overview_statuses.get(name)
        return dom_status

    def util_of(name: str) -> float | None:
        row = bending if name == "bending" else shear
        dom_util = _float_or_none(row.get("util"))
        if dom_util is not None:
            return dom_util
        return _float_or_none(overview_utils.get(name))
...
def _check_card_sanity_before(snapshot: dict[str, Any], state: dict[str, Any], expect: dict[str, Any]) -> list[str]:
    ...
    failures.extend(_blocked_publication_visible_contract_failures(snapshot, state))
...
def _run_case(page, case: dict[str, Any], base_url: str) -> dict[str, Any]:
    ...
    if bool(case.get("skip_live_input_edit")):
        _write_case_progress(case_id, "after_seeded_inputs_no_live_edit", page)
    else:
    ...
    before_failures.extend(_validate_visible_summary(before, expect, state_before))
