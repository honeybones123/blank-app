"""Prove the legacy combined low-util blocker/best-safe assembler can be deleted."""
...
ASSEMBLER = "_assemble_final_visible_combined_low_util_blocker_or_best_safe_result"
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    assembler = capture.get("assembler") or {}
    lines = [
        "# Design Guide Combined Low-Util Blocker/Best-Safe Legacy Assembler Deletion Proof",
        "",
    ...
    lines.extend(
        [
            ...
            "## Recommendation",
            "",
            "Delete only `_assemble_final_visible_combined_low_util_blocker_or_best_safe_result(...)` "
...
def main() -> int:
    ...
    json_path = (
        ARTIFACT_DIR
        / f"design_guide_combined_low_util_blocker_or_best_safe_legacy_assembler_deletion_{stamp}.json"
    )
    report_path = (
        AUDIT_DIR
        / f"design_guide_combined_low_util_blocker_or_best_safe_legacy_assembler_deletion_{stamp}.md"
    )
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_combined_low_util_blocker_or_best_safe_legacy_assembler_deletion {status}")
