"""Verify resolution metadata compatibility proof consumes DesignGuideController."""
...
HELPER_NAME = "_stamp_final_publication_resolution_metadata_compatibility_proof"
...
def _controller_sample() -> dict[str, Any]:
    ...
    response = run_design_guide_controller_publication_authority(
        DesignGuideControllerRequest(
            item={
                "title": "Resolution metadata compatibility",
                "family": "BENDING_FAIL_GOVERNS",
                "action_type": "apply",
                "candidate_id": "candidate-resolution-metadata",
                "button_contract": {"enabled": True, "action_type": "apply"},
            },
            debug={"final_publication_verifier_payload": {}},
            verifier_payload={},
            final_visible_resolution={
                "render_reason": "resolution_metadata_cutover",
                "presentation": {"tone": "action", "status": "ACTION"},
            },
            guidance_debug={},
            publication_reason="resolution_metadata_cutover",
            source="resolution_metadata_cutover",
        )
    )
    ...
    resolver_projection = dict(proof.get("resolver_projection") or {})
    metadata = {
        "render_reason": response.final_visible_resolution.get("render_reason"),
        "display_hash": _stable_hash(display),
        "resolver_projection_hash": _stable_hash(resolver_projection),
...
def _build_snapshot() -> dict[str, Any]:
    ...
    checks = {
        ...
        "compatibility_stamp_preserved": "final_publication_resolution_metadata_compatibility_proofs"
        in helper_source,
        ...
        "metadata_hash_stable": sample_a["metadata_hash"] == sample_b["metadata_hash"],
    ...
    return {
        "schema": "design_guide_controller_resolution_metadata_compatibility_cutover.v1",
        ...
        "errors": errors,
        "next_slice": "Move safe-low-util replacement compatibility proof behind DesignGuideController.",
    }


def _write(snapshot: dict[str, Any]) -> tuple[Path, Path]:
    stamp = snapshot["created_at"]
    json_path = ARTIFACT_DIR / f"design_guide_controller_resolution_metadata_compatibility_cutover_{stamp}.json"
    md_path = AUDIT_DIR / f"design_guide_controller_resolution_metadata_compatibility_cutover_{stamp}.md"
    lines = [
        "# Design Guide Controller Resolution Metadata Compatibility Cutover",
        "",
...
def main() -> int:
    ...
    print(f"design_guide_controller_resolution_metadata_compatibility_cutover {snapshot['status']}")
