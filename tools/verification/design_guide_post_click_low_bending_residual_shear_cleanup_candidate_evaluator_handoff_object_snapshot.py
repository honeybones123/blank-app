"""Object snapshot for residual shear cleanup candidate evaluator handoff."""
...
def _candidate_boundary() -> dict[str, Any]:
    ...
    return build_design_guide_controller_post_click_low_bending_residual_shear_cleanup_candidate_boundary(
...
def _case(
    *,
    ...
    first = build_design_guide_controller_post_click_low_bending_residual_shear_cleanup_candidate_evaluator_handoff(
        candidate_boundary=_candidate_boundary(),
        evaluation_inputs=inputs,
        evaluation_output_summary=output,
        dependency_status=dependency_status,
    )
    second = build_design_guide_controller_post_click_low_bending_residual_shear_cleanup_candidate_evaluator_handoff(
        candidate_boundary=_candidate_boundary(),
        evaluation_inputs=inputs,
        evaluation_output_summary=output,
        dependency_status=dependency_status,
    ...
    return {
        ...
        "evaluator_cutover_ready": bool(first.get("evaluator_cutover_ready")),
        "stable_hash_repeat": first.get("candidate_evaluator_handoff_hash")
        == second.get("candidate_evaluator_handoff_hash"),
...
def _capture() -> dict[str, Any]:
    ...
    cases = [
        _case(name="page_live_complete_handoff", expected_ready=False),
        _case(
            name="missing_evaluation_source",
            input_updates={"evaluation_source": ""},
            expected_ready=False,
        ),
        _case(
            name="missing_sequence_hash",
            output_updates={"stable_sequence_hash": ""},
            expected_ready=False,
        ),
        _case(
    ...
    return {
        "decision": "RESIDUAL_SHEAR_CLEANUP_CANDIDATE_EVALUATOR_HANDOFF_OBJECT_PROVEN",
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "page_live_complete_shape_ready_not_cutover": any(
            case.get("name") == "page_live_complete_handoff"
            and case.get("output_shape_ready") is True
            and case.get("behavior_cutover_ready") is False
            and "candidate_evaluation_execution" in case.get("page_must_keep_for_now")
            for case in cases
        ),
        ...
        "candidate_evaluation_not_moved": all(
            "candidate_evaluation_execution" in case.get("not_moved") for case in cases
        ),
        "non_driving": all(
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Residual Shear Cleanup Candidate Evaluator Handoff Object Snapshot",
        "",
    ...
    for case in capture.get("cases") or []:
        lines.append(
            ...
            + str(case.get("name"))
            + ": output_shape_ready=`"
            + str(case.get("output_shape_ready"))
    ...
    lines.extend(
        [
            ...
            "## Next",
            "",
            "Trace-wire this evaluator handoff beside the live fallback candidate evaluation loop. Do not replace `_evaluate_auto_design_candidate(...)` yet.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    payload = {
        "schema": "design_guide_post_click_low_bending_residual_shear_cleanup_candidate_evaluator_handoff_object_snapshot.v1",
        "created_at": _stamp(),
    ...
    json_path = (
        ARTIFACT_DIR
        / f"design_guide_post_click_low_bending_residual_shear_cleanup_candidate_evaluator_handoff_object_{stamp}.json"
    )
    audit_path = (
        AUDIT_DIR
        / f"design_guide_post_click_low_bending_residual_shear_cleanup_candidate_evaluator_handoff_object_{stamp}.md"
    )
    report_path = (
        REPORT_DIR
        / f"design_brain_physical_extraction_residual_shear_cleanup_candidate_evaluator_handoff_object_{stamp}.md"
    )
    ...
    print(
        "design_guide_post_click_low_bending_residual_shear_cleanup_candidate_evaluator_handoff_object "
        + payload["status"]
    )
