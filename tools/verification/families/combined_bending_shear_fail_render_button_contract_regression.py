...
from inputs_page_modules.guidance_compute import (  # noqa: E402
...
def main() -> int:
    ...
    projected_items = _design_guide_apply_button_contracts_to_items(
        [_active_repair_item()],
        state={},
    )
    ...
    checks = {
        ...
        "active_repair_render_projection_uses_current_bridge": bool(
            callable(_design_guide_apply_button_contracts_to_items)
        ),
        "active_repair_empty_state_projection_blocks_preview": _design_guide_button_contract_enabled(
            projection_contract
        )
        is not True
        and projection_contract.get("blocking_reason")
        in {
            "preview_exception",
