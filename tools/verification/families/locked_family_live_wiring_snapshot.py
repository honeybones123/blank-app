...
def _family_source_snapshot(family: LockedFamily) -> dict[str, Any]:
    inputs_source = (
        ...
        + "\n"
        + _read("inputs_page_modules/design_guide/family_ladder_guidance.py")
        + "\n"
        + _read("inputs_page_modules/design_guide/current_coordinators.py")
    )
    recognition_hits = {
...
def _shared_ownership_snapshot() -> dict[str, bool]:
    shared_source = (
        _read("inputs_page.py")
        + "\n"
        + _read("inputs_page_modules/guidance_compute.py")
        + "\n"
        + _read("inputs_page_modules/apply_routing.py")
        + "\n"
        + _read("inputs_page_modules/design_guide/family_ladder_guidance.py")
        + "\n"
        + _read("inputs_page_modules/design_guide/current_coordinators.py")
        + "\n"
        + _read("inputs_page_modules/design_guide/primary_button_queue.py")
        + "\n"
        + _read("design_brain/design_guide_controller.py")
    )
    publication_source = _read("design_brain/publication.py")
    return {
        "candidate_evaluation_loop_has_current_owner": "def _evaluate_auto_design_candidate(" in shared_source
        and "_evaluate_auto_design_candidate(" in shared_source,
        "cta_queue_has_current_owner": "def _queue_primary_design_guide_button_action(" in shared_source,
        "publication_import_has_current_owner": "from design_brain.final_publication import" in shared_source,
        "publication_gate_exists": "build_final_design_guide_publication(" in shared_source
        or "build_final_design_guide_publication(" in _read("design_brain/design_guide_controller.py"),
        "apply_routing_exists": "handle_inputs_apply_buttons" in shared_source,
        "one_click_orchestration_exists": "one_click" in shared_source.lower(),
        "ui_session_debug_exists": "st.session_state" in shared_source and "debug" in shared_source.lower(),
