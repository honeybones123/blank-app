...
PROOF_PACK: tuple[tuple[str, str], ...] = (
    ...
    ("runtime_snapshot", "tools/verification/families/bending_overdesign_governs_runtime_snapshot.py"),
    ("publication_regression", "tools/verification/families/bending_overdesign_governs_publication_regression.py"),
    ("lock_verifier", "tools/verification/families/bending_overdesign_governs_lock_verifier.py"),
)
...
def _run_script(name: str, script: str, *, timeout_s: int = 300) -> dict[str, Any]:
    ...
    status_line = next(
        (
            ...
            if line.strip().startswith(("PASS:", "FAIL:", "BENDING_OVERDESIGN"))
        ),
        "",
    )
    return {
...
def _write_markdown_report(output: dict[str, Any], report_path: Path) -> None:
    lines = [
        "# BENDING_OVERDESIGN_GOVERNS Locked Regression",
        "",
...
def main() -> int:
    ...
    failures: list[str] = []
    if FAMILY_ID != "BENDING_OVERDESIGN_GOVERNS":
    ...
    artifact_path = ARTIFACT_DIR / f"bending_overdesign_governs_locked_regression_{stamp}.json"
    report_path = AUDIT_DIR / f"bending_overdesign_governs_locked_regression_{stamp}.md"
    output = {
        "schema": "bending_overdesign_governs_locked_regression.v1",
        "status": status,
