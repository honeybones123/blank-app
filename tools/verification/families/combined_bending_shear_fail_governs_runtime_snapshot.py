...
def main() -> int:
    ...
    candidate_rules = [
        str(row.get("merge_rule_id") or "")
        for row in list(payload.get("candidate_repairs") or [])
        if isinstance(row, dict)
    ]
    phase_rank = {
        "BENDING_REINFORCEMENT_ONLY": 0,
        "SHEAR_REINFORCEMENT_ONLY": 1,
        "COMBINED_ADJUSTMENT": 2,
        "GEOMETRY_FALLBACK": 3,
        "APPROVED_COMBINED_MERGE_RULE": 3,
    }
    checks = {
        ...
        "family_ladder_phase_order": [
            phase_rank.get(rule, 999)
            for rule in candidate_rules
        ]
        == sorted(phase_rank.get(rule, 999) for rule in candidate_rules),
        "source_only_phase_precedes_combined_or_geometry": bool(
            candidate_rules
            and candidate_rules[0]
            in {
                "BENDING_REINFORCEMENT_ONLY",
                "SHEAR_REINFORCEMENT_ONLY",
            }
            and any(
                rule in {
                    "COMBINED_ADJUSTMENT",
