...
def _read_inputs_composition_surface() -> str:
    return "\n".join(
        ...
        for path in (
            ...
            "inputs_page_modules/design_guide/shear_tightening.py",
            "design_brain/shear_overdesign_candidate_evaluation.py",
        )
    )
...
def _current_live_evidence() -> dict[str, Any]:
    ...
    anchors = {
        ...
        "detail_update_purity": "ALLOWED_SHEAR_DETAILING_UPDATE_KEYS" in source
        and "shear_detailing_updates_pure" in source,
    }
    observed_surfaces = []
    if anchors["compute_tightening"]:
        observed_surfaces.append("EXTRACTED_SHEAR_TIGHTENING")
    if anchors["remove_links"]:
        observed_surfaces.append("EXTRACTED_LIGATURE_REMOVAL")
    if anchors["density_variants"]:
        observed_surfaces.append("EXTRACTED_DENSITY_REDUCTION_VARIANTS")
    return {
        "source": "current extracted cleanup source anchors",
...
def main() -> int:
    ...
    checks = {
        ...
        "replacement_evidence_found_in_current_owners": all(
            bool(value) for value in (old_evidence.get("anchors") or {}).values()
        ),
