...
def _read_inputs_composition_surface() -> str:
    """Read the current Inputs application ownership graph.

    The retired route coordinator and app-contract bridge are deliberately not
    part of this proof. Shared evaluation, publication, Apply, one-click, and
    UI/session responsibilities now compose through ``inputs_application``.
    """
...
def _inputs_page_ownership_snapshot() -> dict[str, bool]:
    ...
    return {
        ...
        "candidate_evaluation_calls": (
            ...
            or "_evaluate_candidate_fast(" in source
...
def main() -> int:
    ...
    checks = {
        ...
        "inputs_application_composition_owns_shared_plumbing": all(
            inputs_page.values()
        ),
    ...
    snapshot = {
        "schema": "shear_fail_governs_lock_verifier.v2",
        "status": status,
        ...
        "inputs_application_composition_ownership": inputs_page,
