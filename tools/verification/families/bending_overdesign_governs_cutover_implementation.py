...
def main() -> int:
    ...
    checks = {
        ...
        == bending_overdesign_contract_lane_order(),
        "inputs_page_still_owns_shared_plumbing": "from design_brain.final_publication import" in inputs_source
        and "build_final_design_guide_publication" in inputs_source
        and "handle_inputs_apply_buttons" in inputs_source,
        "family_first_live_evaluator_wired": "build_bending_overdesign_live_evaluator"
        in (ROOT / "inputs_page_modules" / "design_guide" / "family_ladder_guidance.py").read_text(
            encoding="utf-8",
            errors="replace",
        ),
