...
def _validate_ladder(snapshot: dict[str, Any], expected: dict[str, Any]) -> list[str]:
    ...
    def _expected_step_sequence(field: str) -> list[float]:
        contract = dict(expected.get(f"{field}_contract") or {})
        if not contract:
            return [float(value) for value in list(expected.get(field) or [])]
        start = int(float(contract.get("start_mm") or 0.0))
        step = int(float(contract.get("step_mm") or 0.0))
        stop = int(float(contract.get("stop_mm") or 0.0))
        if start <= 0 or step <= 0 or stop < start:
            failures.append(f"{field}_contract_invalid")
            return []
        return [float(value) for value in range(start, stop + 1, step)]

    expected_depth_steps = _expected_step_sequence("depth_steps_mm")
    expected_width_steps = _expected_step_sequence("width_steps_mm")
    ...
    indexes_are_positive_increasing = bool(indexes) and indexes[0] == 1 and all(
        isinstance(left, int)
    ...
    if not indexes_are_positive_increasing:
        failures.append("candidate_indexes_not_positive_strictly_increasing")
