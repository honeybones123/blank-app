...
from tools.verification.helpers.current_source_surfaces import read_current_inputs_composition_surface  # noqa: E402
...
def _read_inputs_composition_surface() -> str:
    return read_current_inputs_composition_surface()
...
def main() -> int:
    ...
    first_combined = next(
        (
            spec
            for spec in specs
            if set(spec.get("source_family_ids") or ())
    ...
    observed_phases = [str(spec.get("contract_step") or "") for spec in specs]
    ...
    checks = {
        ...
        "family_boundary_emits_canonical_updates": {"bot_row_1_bars", "bot_row_1_dia"} <= set(dict(first_combined.get("updates") or {}))
        and "bot1_count" not in set(dict(first_combined.get("updates") or {})),
        "source_family_ids_preserved": set(first_combined.get("source_family_ids") or ())
        == {"BENDING_FAIL_GOVERNS", "SHEAR_FAIL_GOVERNS"},
        "family_ladder_phase_order": [
            phase_rank.get(phase, 999)
            for phase in observed_phases
        ]
        == sorted(phase_rank.get(phase, 999) for phase in observed_phases),
    ...
    snapshot = {
        ...
        "first_spec": first,
        "first_combined_spec": first_combined,
