...
from tools.verification.run_family_10_fuzz_audit import _audit_family  # noqa: E402
...
def _publication_probe_failures(family: str, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def _cta_family_matches_selected(expected: str, actual: str) -> bool:
        if actual == expected:
            return True
        if actual in selected_publication_aliases.get(expected, set()):
            return True
        return actual.lower() in executor_family_aliases.get(expected, set())
    ...
    for row in rows:
        ...
        if row_is_action:
            ...
            if row.get("solver_state_timeout") and not (
                ...
                and bool(green_contract.get("passes_contract"))
            ):
