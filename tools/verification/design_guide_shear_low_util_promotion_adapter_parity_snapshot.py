...
def _capture() -> dict[str, Any]:
    ...
    inputs_source = "\n".join(
        ...
        for path in (
            ...
            ROOT / "inputs_page_modules" / "design_guide" / "efficiency_executor_promotion.py",
            ROOT / "inputs_page_modules" / "design_guide" / "resolved_candidate_guidance_item.py",
        )
        if path.exists()
    )
    return {
        ...
        "source_checks": {
            ...
            "live_page_promotion_bridge_still_present": (
                ...
                or "build_design_guide_shear_low_util_promoted_item(" in inputs_source
            ),
        },
        "proof_only": True,
