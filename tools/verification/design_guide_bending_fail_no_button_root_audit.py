        ...
        page.evaluate(
            ...
            () => {
              ...
              const guideNodes = all
                ...
                .slice(0, 80)
                .map(payloadFor);
              const visibleGuidanceItems = Array.from(document.querySelectorAll(".fast-guidance-item"))
                .filter(visible)
                .map(payloadFor);
              ...
              return {
                ...
                bodyHasVisibleGuidanceCard: visibleGuidanceItems.length > 0,
                visibleGuidanceItemCount: visibleGuidanceItems.length,
...
def _capture_root(page, *, scenario_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "dom": {
            ...
            "body_has_visible_guidance_card": dom.get("bodyHasVisibleGuidanceCard"),
            "visible_guidance_item_count": dom.get("visibleGuidanceItemCount"),
            "visible_guidance_items": dom.get("visibleGuidanceItems"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    for row in scenarios:
        root_class = str(row.get("no_button_root_class") or "").strip().upper()
        active_failures = {
            str(value or "").strip().lower()
            for value in (row.get("active_failures") or [])
        ...
        disabled_contract = not bool(contract.get("enabled") or contract.get("actionable"))
        effectively_enabled_contract = bool(
            (contract.get("enabled") or contract.get("actionable"))
            and contract.get("updates")
            and contract.get("preview_pass") is True
            and not contract.get("blocking_reason")
        )
        dom = dict(row.get("dom") or row.get("post_ready_wait", {}).get("dom") or {})
        applied_recipe = str(row.get("browser_recipe") or "").strip()
        requested_recipe = str(args.recipe or "").strip()
        if not row.get("browser_state_available"):
            blocking_failures.append("browser_state_probe_unavailable")
        if requested_recipe and applied_recipe != requested_recipe:
            blocking_failures.append(
                f"requested_recipe_not_applied:requested={requested_recipe}:applied={applied_recipe or '<empty>'}"
            )
        if not bool(dom.get("body_has_visible_guidance_card")):
            blocking_failures.append("no_visible_design_guide_card")
        visible_apply = any(
            "Run one-click" in str(button.get("text") or "")
            or "Apply" in str(button.get("text") or "")
            for button in (dom.get("actionButtons") or [])
            if not bool(button.get("disabled"))
        )
        if root_class in {"", "UNKNOWN"}:
            blocking_failures.append("unknown_no_button_root_class")
        if (
            active_failures == {"bending"}
            and disabled_contract
            and contract_family == "BENDING_FAIL_SHEAR_OVERDESIGN_GOVERNS"
        ):
            blocking_failures.append(
                "bending_only_disabled_contract_stamped_as_bending_fail_shear_overdesign"
            )
        if effectively_enabled_contract and not bool(contract.get("enabled")):
            blocking_failures.append("effectively_enabled_contract_missing_enabled_field")
        if visible_apply and not effectively_enabled_contract:
            blocking_failures.append("visible_apply_button_without_effective_contract")
    if status == "PASS" and blocking_failures:
        status = "FAIL"
    payload = {
    ...
    if blocking_failures:
        print("blocking_failures=" + json.dumps(blocking_failures))
