...
CALLSITE = "render_guidance_secondary_items.pre_card_binding"
...
def _call_window(source: str, function_name: str, callsite_id: str, *, before: int = 1800, after: int = 3000) -> tuple[int | None, str]:
    pattern = re.compile(
...
def _token_window(source: str, token: str, *, before: int = 1800, after: int = 3000) -> tuple[int | None, str]:
...
def main() -> int:
    ...
    direct_line, direct_window = _token_window(
        source, "pre_card_direct_pass_through_after_adapter_identity_proof"
    )
    ...
    post_deletion_shape = bool(
        ...
        and direct_window
        and "_pre_card_bound_item = dict(_pre_card_input_item)" in direct_window
    )
    if not pre_deletion_shape and not post_deletion_shape:
        failures.append("pre_card_callsite_not_ready_or_deleted_shape")
    ...
    callsite_direct_input_available = "_pre_card_input_item = dict(item)" in active_window
    callsite_bypass_identity = (
        "_pre_card_bound_item = dict(_pre_card_input_item)" in bypass_window
        or "_pre_card_bound_item = dict(_pre_card_input_item)" in direct_window
    )
    callsite_adapter_output_only_bound_item = (
        "_pre_card_bound_item = _final_visible_restamper_default_rebuild_adapter_cutover("
        in adapter_window
    ...
    consumers_after_binding_present = all(
        ...
        for token in (
            "_pre_card_bound_contract",
            "_pre_card_bound_is_terminal_blocker",
    ...
    for name, passed in checks.items():
        if name == "pre_deletion_shape" and post_deletion_shape:
            continue
        if name == "post_deletion_shape" and pre_deletion_shape:
            continue
        if not passed:
    ...
    support = {
        ...
        "pre_render_pass_through": _latest(
            "design_guide_pre_render_default_adapter_pass_through_deletion_readiness"
        ),
    }
    ...
    snapshot = {
        "schema": "design_guide_pre_card_default_adapter_pass_through_deletion_readiness.v1",
        "status": status,
        ...
        "checks": checks,
        "decision": (
            "PRE_CARD_BYPASS_AND_ADAPTER_CALLSITE_DELETED"
            if post_deletion_shape and status == "PASS"
            else (
                "SAFE_TO_DELETE_PRE_CARD_BYPASS_AND_ADAPTER_CALLSITE"
                if status == "PASS"
                else "NOT_READY"
            )
        ),
        "replacement": "_pre_card_bound_item = dict(_pre_card_input_item)",
    ...
    json_path = VERIFICATION / f"design_guide_pre_card_default_adapter_pass_through_deletion_readiness_{timestamp}.json"
    report_path = AUDITS / f"design_guide_pre_card_default_adapter_pass_through_deletion_readiness_{timestamp}.md"
    json_path.write_text(json.dumps(snapshot, indent=2, sort_keys=True), encoding="utf-8")

    check_rows = [f"| `{name}` | `{passed}` |" for name, passed in checks.items()]
    report = [
        "# Design Guide Pre-Card Default Adapter Pass-Through Deletion Readiness",
        "",
        f"## Summary\n{status}",
        "",
        "## Decision",
    ...
    print(f"design_guide_pre_card_default_adapter_pass_through_deletion_readiness {status}")
