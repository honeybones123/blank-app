"""Proof-only snapshot for the pre-card final-visible output bridge proof.

This verifier does not prove deletion or bypass readiness. It proves the
pre-card final-visible output bridge is now traced by the same DesignGuideController
restamper proof surface while the existing live restamper behavior remains the
...
def _capture() -> dict[str, Any]:
    ...
    restamper_index = inputs_source.find(RESTAMPER_CALL)
    stamp_index = inputs_source.find(f'callsite_id="{CALLSITE_ID}"', restamper_index)
    bound_contract_index = inputs_source.find("_pre_card_bound_contract =", restamper_index)
    input_copy_index = inputs_source.find("_pre_card_input_item = dict(item)")
    pending_rec_index = inputs_source.find("_pre_card_pending_rec = dict(")
    window = inputs_source[max(0, restamper_index - 700) : restamper_index + 1400] if restamper_index >= 0 else ""
    return {
        ...
        "stamp_after_pre_card_restamper": restamper_index >= 0 and stamp_index > restamper_index,
        "stamp_before_bound_contract_read": stamp_index >= 0 and bound_contract_index > stamp_index,
        "pre_card_input_copied_before_restamper": 0 <= input_copy_index < restamper_index,
        "pre_card_pending_rec_copied_before_restamper": 0 <= pending_rec_index < restamper_index,
        "restamper_uses_copied_input": "item=dict(_pre_card_input_item)" in window,
        "restamper_uses_copied_rec": "rec=dict(_pre_card_pending_rec)" in window,
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "restamper_call_present": bool(capture.get("restamper_call_present")),
        "callsite_id_present": bool(capture.get("callsite_id_present")),
        "stamp_after_pre_card_restamper": bool(capture.get("stamp_after_pre_card_restamper")),
        "stamp_before_bound_contract_read": bool(capture.get("stamp_before_bound_contract_read")),
        "pre_card_input_copied_before_restamper": bool(capture.get("pre_card_input_copied_before_restamper")),
        "pre_card_pending_rec_copied_before_restamper": bool(capture.get("pre_card_pending_rec_copied_before_restamper")),
        "restamper_uses_copied_input": bool(capture.get("restamper_uses_copied_input")),
        "restamper_uses_copied_rec": bool(capture.get("restamper_uses_copied_rec")),
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Design Guide Pre-Card Restamper Bridge Proof Snapshot",
        "",
        f"Status: `{payload.get('status')}`",
        "",
        "## Summary",
        "",
        "- Wired the pre-card restamper callsite as a proof-only DesignGuideController trace.",
        "- Existing restamper behavior is still live and unchanged.",
        "- This is not a bypass, deletion, or authority move.",
        "",
        "## Key Fields",
        "",
        ...
        "## Next Safe Step",
        "",
        "Run a cutover-readiness or guarded-bypass readiness proof for this pre-card bridge before narrowing or deleting it.",
    ]
...
def main() -> int:
    ...
    payload = {
        "schema": "design_guide_pre_card_final_visible_output_bridge_proof_snapshot.v1",
        "status": status,
        "created_at": stamp,
        "capture": capture,
        "checks": checks,
    ...
    json_path = ARTIFACT_DIR / f"design_guide_pre_card_final_visible_output_bridge_proof_{stamp}.json"
    report_path = AUDIT_DIR / f"design_guide_pre_card_final_visible_output_bridge_proof_{stamp}.md"
