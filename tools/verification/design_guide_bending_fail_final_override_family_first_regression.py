...
def _line_number_after(source: str, needle: str, after: int | None) -> int | None:
    for index, line in enumerate(source.splitlines(), start=1):
...
def _build_payload() -> dict[str, Any]:
    ...
    publication_source = (ROOT / "design_brain" / "publication.py").read_text(
        encoding="utf-8",
        errors="replace",
    )
    route_policy_window = _slice_between(
        controller_source,
    ...
    ladder_dispatch_window = _slice_between(
        controller_source,
        "def build_design_guide_controller_active_fail_executor_family_ladder_dispatch",
        "def _active_fail_executor_candidate_family_utils",
    )
    publication_family_window = _slice_between(
        publication_source,
        "def _selected_family_id_from_publication",
        "def _candidate_family_id_from_publication",
    )
    bending_policy = resolve_design_guide_controller_direct_target_active_failure_route_policy(
        strengthening=True,
        active_failure_keys=["bending"],
    )
    shear_policy = resolve_design_guide_controller_direct_target_active_failure_route_policy(
        strengthening=True,
        active_failure_keys=["shear"],
    )
    combined_policy = resolve_design_guide_controller_direct_target_active_failure_route_policy(
        strengthening=True,
    ...
    direct_target_fallback_line = _line_number(
        controller_source,
        "direct_target_band_bypassed_by_family_owner",
    )
    ladder_dispatch = build_design_guide_controller_active_fail_executor_family_ladder_dispatch(
        family_id="BENDING_FAIL_GOVERNS",
        base_state=_severe_bending_fail_state(),
        width_key="b",
    ...
    return {
        ...
        "final_resolver": {
            ...
            "direct_target_fallback_line": _line_number_after(
                controller_source,
                "direct_target_band_bypassed_by_family_owner",
                bending_branch_line,
            )
            or direct_target_fallback_line,
            "bending_route_policy": bending_policy,
            "shear_route_policy": shear_policy,
            "combined_route_policy": combined_policy,
            "active_failure_key_fallback_from_visible_truth": (
                bending_policy.get("should_dispatch") is True
                and bending_policy.get("active_failure_keys") == ["bending"]
                and bending_policy.get("family_id") == "BENDING_FAIL_GOVERNS"
                and 'if "bending" in active:' in publication_family_window
                and 'return "BENDING_FAIL_GOVERNS", "active_failures"' in publication_family_window
            ),
            ...
            "bending_branch_calls_family_repair_route": (
                bending_policy.get("route_kind") == "bending"
                and bending_policy.get("family_route_owner")
                == "design_brain.families.bending_fail.BendingFailFamily"
                and "family_strategy_for(family)" in ladder_dispatch_window
                and "strategy.contracted_repair_ladder_specs(base, **kwargs)" in ladder_dispatch_window
                and dict(ladder_dispatch.get("ladder") or {}).get("specs")
            ),
            "bending_branch_not_gated_by_stale_owner_metadata": (
                "_final_selected_family_id" not in route_policy_window
                and "_final_primary_owner" not in route_policy_window
                and "bending_fail_family_early_dispatch" not in route_policy_window
            ),
            "bending_branch_stamps_family_authority": (
                bending_policy.get("family_id") == "BENDING_FAIL_GOVERNS"
                and bending_policy.get("family_route_owner")
                == "design_brain.families.bending_fail.BendingFailFamily"
                and dict(bending_policy.get("route_adapter_kwargs") or {}).get("family_id")
                == "BENDING_FAIL_GOVERNS"
            ),
            "generic_target_band_remains_fallback_only": (
                "direct_target_band_bypassed_by_family_owner" in controller_source
                and "generic_target_band_search_skipped" in controller_source
                and bending_policy.get("skipped_reason") == "selected_family_bending_fail_governs"
            ),
            "shear_family_first_branch_still_present": (
                shear_policy.get("family_id") == "SHEAR_FAIL_GOVERNS"
                and shear_policy.get("family_route_owner")
                == "design_brain.families.shear_fail.ShearFailFamily"
                and combined_policy.get("family_id") == "COMBINED_BENDING_SHEAR_FAIL"
                and combined_policy.get("family_route_owner")
