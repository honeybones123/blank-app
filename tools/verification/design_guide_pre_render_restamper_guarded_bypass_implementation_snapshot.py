"""Implementation snapshot for the pre-render restamper guarded bypass.

This verifier proves the first pre-render restamper callsite keeps the old
restamper as the default path and only bypasses after a strict no-mutation
DesignGuideController proof.
"""
...
CALLSITE = 'callsite_id="render_guidance_secondary_items.pre_render_binding"'
...
def _capture() -> dict[str, Any]:
    ...
    callsite_window = source[callsite_start : callsite_start + 1800] if callsite_start >= 0 else ""
    required_helper_tokens = {
        "previous_proof_required": 'previous.get("proof_hash")',
        "same_debug_hash_required": '"debug_hash"',
        "same_projection_hashes_required": '"cta_projection_hash"' in helper_body
        and '"display_projection_hash"' in helper_body
        and '"evidence_projection_hash"' in helper_body,
    ...
    required_callsite_tokens = {
        ...
        "bypass_returns_input_item_only": "_pre_render_bound_item = dict(_pre_render_input_item)" in callsite_window,
        "proof_stamp_still_runs": "_stamp_final_visible_final_visible_output_bridge_proof(" in callsite_window,
        "diagnostics_non_authority": "final_visible_restamper_bridge_pre_render_bypassed" in callsite_window,
    }
    return {
        ...
        "latest": {
            "readiness": _latest("design_guide_pre_render_restamper_guarded_bypass_readiness"),
            "projection": _latest("design_guide_restamper_bridge_output_projection"),
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "projection_latest_pass": (latest.get("projection") or {}).get("status") == "PASS",
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Design Guide Pre-Render Restamper Guarded Bypass Implementation Snapshot",
        "",
    ...
    lines.extend(
        [
            ...
            "## Decision",
            "",
            "The first pre-render final-visible output bridge now has a guarded no-op bypass. The old restamper remains the default rebuild path.",
...
def main() -> int:
    ...
    compile_run = _run(
        [
            ...
            "py_compile",
            "inputs_page.py",
            "tools/verification/design_guide_pre_render_restamper_guarded_bypass_implementation_snapshot.py",
        ]
    )
    capture = _capture()
    ...
    stamp = _stamp()
    payload = {
        "schema": "design_guide_pre_render_restamper_guarded_bypass_implementation_snapshot.v1",
        "status": status,
        "created_at": stamp,
        "capture": capture,
        "checks": checks,
    ...
    json_path = ARTIFACT_DIR / f"design_guide_pre_render_restamper_guarded_bypass_implementation_{stamp}.json"
    report_path = AUDIT_DIR / f"design_guide_pre_render_restamper_guarded_bypass_implementation_{stamp}.md"
