"""Verify guarded source-summary cutover for residual button-contract fields."""
...
def _capture() -> dict[str, Any]:
    readiness_run = _run(
        [
            sys.executable,
            "tools/verification/design_guide_post_click_low_bending_residual_shear_cleanup_button_contract_execution_boundary_cutover_readiness.py",
        ]
    )
    ...
    summary_tokens = {
        "source_summary_flag": '"button_contract_source_summary_cutover": True' in route,
        "boundary_hash_in_binding_inputs": '"button_contract_execution_boundary_hash": (' in route,
        ...
        "button_contract_expected_util_from_boundary": (
            'residual_button_contract_execution_boundary or {}\n                                    ).get("button_contract_expected_util")'
            in route
        ),
        "button_contract_enabled_from_boundary": (
            'residual_button_contract_execution_boundary or {}\n                                    ).get("button_contract_enabled")'
            in route
        ),
        "button_contract_actionable_from_boundary": (
        ...
        "raw_fallbacks_retained": all(
            ...
            for token in (
                ...
                "residual_button_contract.get(\"expected_util\")",
                "residual_button_contract.get(\"enabled\")",
                "residual_button_contract.get(\"actionable\")",
            )
        ),
    ...
    implemented = all(summary_tokens.values())
    return {
        "decision": (
            "BUTTON_CONTRACT_SOURCE_SUMMARY_CUTOVER_IMPLEMENTED_READY_FOR_DEADNESS_RECHECK"
            if implemented
            else "BUTTON_CONTRACT_SOURCE_SUMMARY_CUTOVER_INCOMPLETE"
        ),
        "readiness_run": readiness_run,
        "summary_tokens": summary_tokens,
        "source_summary_cutover_implemented": implemented,
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Residual Shear Cleanup Button Contract Source Summary Cutover Implementation",
        "",
...
def main() -> int:
    ...
    payload = {
        "schema": "design_guide_post_click_low_bending_residual_shear_cleanup_button_contract_execution_boundary_cutover_implementation.v1",
        "created_at": _stamp(),
    ...
    json_path = (
        ARTIFACT_DIR
        / f"design_guide_post_click_low_bending_residual_shear_cleanup_button_contract_execution_boundary_cutover_implementation_{stamp}.json"
    )
    audit_path = (
        AUDIT_DIR
        / f"design_guide_post_click_low_bending_residual_shear_cleanup_button_contract_execution_boundary_cutover_implementation_{stamp}.md"
    )
    report_path = (
        REPORT_DIR
        / f"design_brain_physical_extraction_residual_shear_cleanup_button_contract_execution_boundary_cutover_implementation_{stamp}.md"
    )
