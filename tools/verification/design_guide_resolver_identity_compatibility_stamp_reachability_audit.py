"""Audit reachability for resolver identity compatibility stamp rows.

Proof-only. This verifier classifies all current references to the resolver
identity compatibility stamp surface before any deletion or collapse.
"""
...
HELPER = "_stamp_final_publication_resolver_identity_compatibility_proof"
TOKENS = (
    HELPER,
    "final_publication_resolver_identity_compatibility_proofs",
    "final_publication_resolver_identity_compatibility_proof_hash",
    "final_publication_resolver_identity_rows_compatibility_only",
    "final_publication_resolver_identity_remaining_truth_narrowed",
)
...
def _capture() -> dict[str, Any]:
    ...
    return {
        ...
        "next_safe_step": (
            "Delete the product resolver identity stamp helper/calls if product_stamp_surface_count "
            "is nonzero and no external product consumers remain; otherwise keep historical verifier "
            "references as proof-only references."
        ),
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Resolver Identity Compatibility Stamp Reachability Audit",
        "",
...
def main() -> int:
    ...
    json_path = (
        ARTIFACT_DIR
        / f"design_guide_resolver_identity_compatibility_stamp_reachability_{stamp}.json"
    )
    report_path = (
        AUDIT_DIR
        / f"design_guide_resolver_identity_compatibility_stamp_reachability_{stamp}.md"
    )
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_resolver_identity_compatibility_stamp_reachability {status}")
