...
def _runtime(trace: list[dict[str, Any]]):
    ...
    return bottom_recommendation_selector.BottomRecommendationSelectorRuntime(
        shallow_geometry_score_tie_eps=5.0,
        candidate_ductility_governs=lambda candidate: False,
        candidate_ductility_util=lambda candidate: None,
        geometry_trial_axis=axis,
        strictly_rejectable_band_winner=lambda candidate, **kwargs: (False, None),
        legacy_local_rejection_reason=lambda *args, **kwargs: None,
        log_candidate_rank=lambda **kwargs: None,
        merge_rank_trace=lambda payload: trace.append(dict(payload)),
        score_candidate=score,
        select_best_candidate=select_best,
        updates_match_state=lambda state, updates: False,
    )
...
def main() -> int:
    ...
    checks = {
        ...
        "bridge_binds_bottom_selector_dependencies": "runtime=_build_bottom_selector_runtime()" in bridge_helper,
