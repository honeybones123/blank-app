"""Verify shear reinforcement wording cutover for _describe_guidance_step."""
...
def _row(
    ...
    result = build_design_guide_pure_guidance_step_description(
        ...
        before_shear_reo_label=before_label,
        after_shear_reo_label=after_label,
    )
    expected = (
        f"Updated shear reinforcement from {before_label} to {after_label}."
        if should_handle
        else None
    )
    return {
...
def _rows() -> list[dict[str, Any]]:
    return [
        _row("spacing_update", {"s_lig": 250.0}, "2-leg N10 @ 200", "2-leg N10 @ 250", True),
        _row("legs_removed", {"lig_legs": 0}, "2-leg N10 @ 250", "No ligs", True),
        _row("diameter_update", {"lig_d": 12}, "2-leg N10 @ 200", "2-leg N12 @ 200", True),
        _row("missing_label_declined", {"s_lig": 250.0}, "", "2-leg N10 @ 250", False),
        _row("bottom_update_declined", {"bot1_count": 5}, "2-leg N10 @ 200", "2-leg N10 @ 250", False),
    ]
...
def _capture() -> dict[str, Any]:
    ...
    rows = _rows()
    source_checks = {
        "page_computes_shear_labels": "before_shear_reo_label = _shear_state_label(before_state)" in segment
        and "after_shear_reo_label = _shear_state_label(after_state)" in segment,
        "page_passes_shear_labels": "before_shear_reo_label=before_shear_reo_label" in segment
        and "after_shear_reo_label=after_shear_reo_label" in segment,
        "page_shear_fallback_branch_deleted": "Updated shear reinforcement from {_shear_state_label(before_state)}" not in segment,
        "controller_helper_accepts_shear_labels": "before_shear_reo_label: str | None = None" in controller_source
        and "after_shear_reo_label: str | None = None" in controller_source,
    ...
    return {
        "schema": "design_guide_describe_guidance_step_shear_wording_cutover.v1",
...
def _checks(payload: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "page_computes_shear_labels": bool(source_checks.get("page_computes_shear_labels")),
        "page_passes_shear_labels": bool(source_checks.get("page_passes_shear_labels")),
        "page_shear_fallback_branch_deleted": bool(source_checks.get("page_shear_fallback_branch_deleted")),
        "controller_helper_accepts_shear_labels": bool(source_checks.get("controller_helper_accepts_shear_labels")),
...
def _write(payload: dict[str, Any], checks: dict[str, bool]) -> tuple[Path, Path]:
    ...
    json_path = ARTIFACT_DIR / f"design_guide_describe_guidance_step_shear_wording_cutover_{suffix}.json"
    report_path = AUDIT_DIR / f"design_guide_describe_guidance_step_shear_wording_cutover_{suffix}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    lines = [
        "# Design Guide Describe Guidance Step Shear Wording Cutover",
        "",
...
def main() -> int:
    ...
    print(f"design_guide_describe_guidance_step_shear_wording_cutover {payload['status']}")
