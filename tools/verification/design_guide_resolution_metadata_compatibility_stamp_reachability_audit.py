"""Audit reachability for resolution metadata compatibility stamp rows.

Proof-only. This verifier classifies all current references to the resolution
metadata compatibility stamp surface before any deletion or collapse.
"""
...
HELPER = "_stamp_final_publication_resolution_metadata_compatibility_proof"
TOKENS = (
    HELPER,
    "final_publication_resolution_metadata_compatibility_proofs",
    "final_publication_resolution_metadata_compatibility_proof_hash",
    "final_publication_resolution_metadata_rows_compatibility_only",
    "final_publication_resolution_metadata_remaining_truth_narrowed",
)
...
def _capture() -> dict[str, Any]:
    ...
    return {
        ...
        "next_safe_step": (
            "Delete the product resolution metadata stamp helper/calls if product_stamp_surface_count "
            "is nonzero and no external product consumers remain; otherwise keep verifier references "
            "as proof-only history."
        ),
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Resolution Metadata Compatibility Stamp Reachability Audit",
        "",
...
def main() -> int:
    ...
    json_path = (
        ARTIFACT_DIR
        / f"design_guide_resolution_metadata_compatibility_stamp_reachability_{stamp}.json"
    )
    report_path = (
        AUDIT_DIR
        / f"design_guide_resolution_metadata_compatibility_stamp_reachability_{stamp}.md"
    )
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_resolution_metadata_compatibility_stamp_reachability {status}")
