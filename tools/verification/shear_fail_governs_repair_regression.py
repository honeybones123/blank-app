...
def main(argv: list[str] | None = None) -> int:
    ...
    design_guide_shear_ctas = [
        ...
        or "apply recommendation" in str(value).lower()
    ]
    ...
    current_compatibility_owner_proof = bool(
        evidence.get("family_match_passed") is True
        and current_family_ids.intersection(ACCEPTED_PUBLICATION_FAMILIES)
        and evidence.get("family_selection_source")
        and evidence.get("family_selection_contract") == "family_selection_contract"
        and evidence.get("family_chooser_contract") == "family_chooser_contract"
    )
    has_repair_action = bool(evidence.get("has_repair_action"))
    has_no_repair_evidence = bool(evidence.get("has_no_repair_evidence"))
    has_apply_payload = bool(
        evidence.get("has_apply_payload")
        or apply_payload_family_id
    )
    positive_checks = {
        ...
        "candidate_source_family_is_shear_fail": candidate_family_id in ACCEPTED_PUBLICATION_FAMILIES,
        ...
        "family_owner_visible": (
            "design_brain.families.shear_fail.ShearFailFamily" in family_route_owner
            or "design_brain.families.shear_fail_bending_overdesign.ShearFailBendingOverdesignFamily"
            in family_route_owner
            or current_compatibility_owner_proof
        ),
        "repair_action_or_no_repair_evidence": bool(
            evidence.get("has_repair_action") or evidence.get("has_no_repair_evidence")
        ),
        "apply_payload_matches_outcome": (
            has_apply_payload if has_repair_action else not has_apply_payload
        ...
        "payload_id_is_shear_fail_repair": (
            (
                ...
                and ("repair" in render_cta_payload_id_lower or ":candidate_" in render_cta_payload_id_lower)
            )
            if has_repair_action
            else "family_ladder_exhausted" in render_cta_payload_id_lower
            if has_no_repair_evidence
            else False
        ),
        "single_design_guide_shear_cta_if_actionable": (
            len(design_guide_shear_ctas) == 1
            if has_repair_action
            else len(design_guide_shear_ctas) == 0
            if has_no_repair_evidence
            else False
        ),
    }
    negative_checks = {
        ...
        "no_false_pass_terminal": not (
            "pass" in lower_text
            and "fail" not in lower_text
            and not has_repair_action
        ),
