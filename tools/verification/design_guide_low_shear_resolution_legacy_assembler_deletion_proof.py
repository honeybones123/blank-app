"""Prove the legacy low-shear final-visible assembler can be deleted."""
...
ASSEMBLER = "_assemble_final_visible_low_shear_resolution_result"
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    assembler = capture.get("assembler") or {}
    lines = [
        "# Design Guide Low-Shear Resolution Legacy Assembler Deletion Proof",
        "",
    ...
    lines.extend(
        [
            ...
            "## Recommendation",
            "",
            "Delete only `_assemble_final_visible_low_shear_resolution_result(...)` when decision is `READY_TO_DELETE`.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    stamp = _stamp()
    json_path = (
        ARTIFACT_DIR / f"design_guide_low_shear_resolution_legacy_assembler_deletion_{stamp}.json"
    )
    report_path = (
        AUDIT_DIR / f"design_guide_low_shear_resolution_legacy_assembler_deletion_{stamp}.md"
    )
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_low_shear_resolution_legacy_assembler_deletion {status}")
