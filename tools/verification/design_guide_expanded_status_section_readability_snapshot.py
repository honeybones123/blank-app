...
def _base_model(
    *,
    status_rows: tuple[dict, ...],
    summary: str,
    title: str = "Bending cleanup blocked",
    outcome_state: str = "BLOCKED",
    ...
    return FinalDesignGuideCardFormat(
        ...
        summary=summary,
        blocker_explanation=(
            summary if blocker_explanation is None else blocker_explanation
        ),
...
def _build_payload() -> dict:
    ...
    generic_status_duplicate_model = _base_model(
        title="Design is efficient - further reductions would weaken capacity",
        outcome_state="PASS",
        blocker_explanation="missing_action_type",
        summary=(
            "Further reductions would lower reserve capacity or stiffness; "
            "the guide did not select a material one-click change for the "
        ...
        status_rows=(
            {
                "label": "Status",
                "text": (
                    "Further reductions would lower reserve capacity or "
                    "stiffness; the guide did not select a material one-click "
                    "change for the published current state."
                ),
            },
        ),
    )
    exact_stop_evidence_model = _base_model(
        title="Design is efficient - further reductions would weaken capacity",
        outcome_state="PASS",
        summary=(
            "Further reductions would lower reserve capacity or stiffness; "
        ...
        status_rows=(
            {
                "label": "Why",
                "text": (
                    "The solver did not find a smaller practical option that "
                    "kept the "
                    "governing checks acceptable. Further reductions can "
                    "reduce bending capacity, shear capacity, and stiffness."
                ),
            },
        ),
    )
    ...
    generic_status_duplicate_html = render_final_design_guide_card_html(
        generic_status_duplicate_model
    )
    exact_stop_evidence_html = render_final_design_guide_card_html(
        exact_stop_evidence_model
    )
    ...
    checks = {
        ...
        "generic_status_label_duplicate_section_omitted": (
            "data-testid='design-guide-main-explanation'"
            not in generic_status_duplicate_html
        ),
        "pass_exact_stop_evidence_uses_cleanup_heading": (
            "Why no further cleanup?" in exact_stop_evidence_html
            and ">Status<" not in exact_stop_evidence_html
            and "The solver did not find a smaller practical option"
            in exact_stop_evidence_html
        ),
