"""Verify auto-design commit-blocked response coordinator extraction."""
...
def _run_case(module: Any) -> dict[str, Any]:
    ...
    try:
        ...
        dbg = {
            "current_fail_fingerprint": {"bending": "FAIL", "shear": "FAIL"},
            "current_fail_keys": ["bending", "shear"],
            "candidate_covered_fail_keys": ["shear"],
            "candidate_remaining_fail_keys": ["bending"],
            "shear_fail_status_used": "FAIL",
            "shear_fail_util_used": 1.21,
            "current_shear_status": "FAIL",
            "current_shear_util": 1.21,
            "current_shear_selection_origin": "overview",
        }
        result = module._return_auto_design_commit_blocked_response_coordinator(
...
def build_payload() -> dict[str, Any]:
    ...
    helper_start, helper_end, helper = _function_segment(
        source,
        "_return_auto_design_commit_blocked_response_coordinator",
    )
    ...
    runtime_checks = {
        "trace_run_end_preserved": runtime["traces"][0]["status"] == "no_actionable_full_coverage_candidate"
        ...
        and runtime["traces"][0]["kwargs"]["final_updates"] == {},
        "feedback_payload_preserved": feedback["status"] == "blocked"
        and feedback["reason"] == "partial_failure_coverage"
        ...
        "recommendation_envelope_preserved": envelope["status"] == "blocked"
        and envelope["commit_audit"] == {"audit": "yes"}
        and envelope["updates"] == {"D": 650}
        and envelope["blocked_reason"] == "partial_failure_coverage"
        and envelope["commit_eligible"] is False,
        "latch_reason_preserved": runtime["result"]["latch_reason"] == "run_one_click_auto_design:partial_failure_coverage",
        "returned_payload_shape_preserved": payload["status"] == "no_actionable_full_coverage_candidate"
        and payload["stop_reason"] == "partial_failure_coverage"
        ...
        and payload["current_fail_keys"] == ["bending", "shear"]
        and payload["candidate_covered_fail_keys"] == ["shear"]
        and payload["candidate_remaining_fail_keys"] == ["bending"]
        and payload["one_click_commit_blocked_reason"] == "partial_failure_coverage"
        and "No single one-click update" in payload["user_visible_no_action_reason"],
    }
    static_checks = {
        "helper_present": "def _return_auto_design_commit_blocked_response_coordinator(" in source,
        "helper_preserves_latch_reason": "run_one_click_auto_design:partial_failure_coverage" in helper,
        "helper_preserves_visible_no_action": "No single one-click update currently covers all failing checks" in helper,
        ...
        "dispatch_delegates_commit_blocked_response_dispatch": (
            "_dispatch_auto_design_commit_blocked_response_from_final_response_coordinator("
            in dispatch_body
        ),
        "commit_blocked_response_dispatch_delegates_response": (
            "_return_auto_design_commit_blocked_response_coordinator("
            in commit_blocked_dispatch
            and "final_response_scope[" in commit_blocked_dispatch
        ),
        "run_delegates_final_tail": "_finish_auto_design_post_current_eval_and_dispatch_coordinator(" in run_body,
        "dispatch_preserves_other_return_branches": "if commit_rejected:" in dispatch_body
        and 'if out_status == "blocked":' in dispatch_body
    ...
    return {
        "status": status,
        "surface": "run_one_click_auto_design_commit_blocked_response_coordinator",
        "helper_segment": {
            "function": "_return_auto_design_commit_blocked_response_coordinator",
        ...
        "next_safe_slice": "extract blocked status final return branch from run_one_click_auto_design",
    }
...
def write_artifacts(payload: dict[str, Any]) -> tuple[Path, Path]:
    ...
    json_path = ARTIFACT_DIR / f"inputs_page_auto_design_commit_blocked_response_coordinator_extraction_{stamp}.json"
    md_path = AUDIT_DIR / f"inputs_page_auto_design_commit_blocked_response_coordinator_extraction_{stamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    lines = [
        "# Auto Design Commit-Blocked Response Coordinator Extraction",
        "",
