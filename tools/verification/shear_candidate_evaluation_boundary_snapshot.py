"""Proof-only snapshot for the shear candidate evaluation boundary."""
...
MODULE_PATH = ROOT / "design_brain" / "shear_candidate_evaluation.py"
...
def _write_artifacts(snapshot: dict[str, Any]) -> tuple[Path, Path]:
    ...
    json_path = ARTIFACT_DIR / f"shear_candidate_evaluation_boundary_{stamp}.json"
    report_path = AUDIT_DIR / f"shear_candidate_evaluation_boundary_{stamp}.md"
    json_path.write_text(json.dumps(snapshot, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    report_path.write_text(
        "\n".join(
            [
                "# Shear Candidate Evaluation Boundary Snapshot",
                "",
                ...
                "It does not prove SHEAR_FAIL_GOVERNS lock.",
                "",
                "## Checks",
                "",
...
def main() -> int:
    ...
    base_state = {
        ...
        "actions": {
            "design_shear_kn": 360.0,
            "design_moment_knm": 520.0,
        },
        "constraints": {
    ...
    candidate_update = {
        "reinforcement": {
            ...
            "ligature_diameter_mm": 12,
        }
    }

    boundary_input = ShearCandidateInput(base_state=base_state)
    boundary_update = ShearCandidateUpdate(updates=candidate_update)
    ...
    repeated_evaluation = ShearCandidateEvaluation(
        input_hash=boundary_input.input_hash,
        update_hash=boundary_update.update_hash,
        candidate_state_hash=candidate_state_hash,
        **realistic_result,
    ...
    input_fields = {field.name for field in fields(ShearCandidateInput)}
    update_fields = {field.name for field in fields(ShearCandidateUpdate)}
    evaluation_fields = {field.name for field in fields(ShearCandidateEvaluation)}
    missing_fields = {
        "ShearCandidateInput": sorted(REQUIRED_INPUT_FIELDS - input_fields),
        "ShearCandidateUpdate": sorted(REQUIRED_UPDATE_FIELDS - update_fields),
        "ShearCandidateEvaluation": sorted(REQUIRED_EVALUATION_FIELDS - evaluation_fields),
    }

    evaluation_payload = evaluation.to_dict()
    stable_hashes = (
        boundary_input.input_hash == stable_shear_candidate_hash(base_state)
        and boundary_update.update_hash == stable_shear_candidate_hash(candidate_update)
        and candidate_state_hash == build_shear_candidate_state_hash(base_state, candidate_update)
    ...
    snapshot = {
        "schema": "shear_candidate_evaluation_boundary_snapshot.v1",
        "result": "PASS" if all(checks.values()) else "FAIL",
        "purpose": (
            "Prove the permanent shear candidate evaluation boundary can represent "
            "real evaluated shear candidates without importing page-owned logic."
        ),
        "scope_limits": {
            "proves_boundary_shape_only": True,
            "proves_lane_decisions": False,
            "proves_runtime_parity": False,
            "proves_shear_fail_governs_lock": False,
        },
    ...
    if snapshot["result"] != "PASS":
        print("shear candidate evaluation boundary FAIL")
        print(f"JSON: {json_path}")
        print(f"Report: {report_path}")
        print(json.dumps(snapshot, indent=2, sort_keys=True))
        return 1

    print("shear candidate evaluation boundary PASS")
    print(f"JSON: {json_path}")
    print(f"Report: {report_path}")
    return 0
