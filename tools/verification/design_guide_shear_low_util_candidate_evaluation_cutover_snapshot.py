"""Proof snapshot for shear low-util candidate evaluation boundary cutover."""
...
def _exercise_helper() -> dict[str, Any]:
    ...
    helper = module.evaluate_design_guide_shear_low_util_cleanup_candidate
    base_state = {"b": 300.0, "D": 500.0, "lig_d": 10, "lig_legs": 2, "s_lig": 200.0}
    ...
    return {
        ...
        "defaulted": defaulted,
        "calls": calls,
        "accepted_hash_repeat": _stable_hash(accepted) == _stable_hash(
            helper(
                evaluator=accepting_evaluator,
                base_state=base_state,
                updates=updates,
...
def _capture() -> dict[str, Any]:
    ...
    return {
        "decision": "SHEAR_LOW_UTIL_CANDIDATE_EVALUATION_CUTOVER_PASS",
        "function": {
            "name": FUNCTION_NAME,
        ...
        "source_checks": {
            "controller_helper_exported": (
                '"evaluate_design_guide_shear_low_util_cleanup_candidate"' in controller_source
            ),
            ...
            "legacy_direct_evaluator_try_block_removed": (
                "candidate = _evaluate_auto_design_candidate(" not in target_source
            ),
            "candidate_service_evaluator_injected_in_target": (
                "evaluator=_evaluate_shear_low_util_candidate_with_service" in target_source
            ),
            "page_evaluator_not_injected_in_target": (
                "evaluator=_evaluate_auto_design_candidate" not in target_source
            ),
            "page_evaluator_function_retained": "def _evaluate_auto_design_candidate(" in inputs_source,
            "candidate_service_wrapper_retained": (
                "def _evaluate_shear_low_util_candidate_with_service(" in inputs_source
                and "_evaluate_shear_low_util_candidate_with_updates(" in inputs_source
            ),
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    ...
    first_call = dict(calls[0]) if calls else {}
    first_kwargs = dict(first_call.get("kwargs") or {})
    ...
    defaulted_proof = dict((exercise.get("defaulted") or {}).get("evaluation_proof") or {})
    ...
    return {
        ...
        "evaluator_called_with_same_source": (
            first_kwargs.get("source") == "low_util_shear_target_cleanup_action"
        ),
        "evaluator_called_with_same_label": (
            first_kwargs.get("label") == "Shear cleanup - one-click reduction"
        ),
        "evaluator_called_with_same_action_type": (
            first_kwargs.get("action_type") == "apply_resolved_candidate"
        ),
        "evaluator_called_with_updates": dict(first_kwargs.get("updates") or {}) == {
            "lig_d": 0,
            "lig_legs": 0,
            "s_lig": 200.0,
        },
        ...
        "default_metadata_preserves_legacy_values": (
            defaulted_proof.get("evaluation_source") == "low_util_shear_target_cleanup_action"
            and defaulted_proof.get("evaluation_label") == "Shear cleanup - one-click reduction"
            and defaulted_proof.get("evaluation_action_type") == "apply_resolved_candidate"
        ),
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Design Guide Shear Low-Util Candidate Evaluation Cutover",
        "",
    ...
    lines.extend(
        [
            ...
            "## Finding",
            "",
            "The selected shear low-util cleanup loop now calls a controller-owned evaluation boundary with a candidate-evaluation service handoff. The actual engineering evaluator remains unchanged behind the page compatibility wrapper.",
            "",
            "## Remaining Boundary",
            "",
            "The selected loop still depends on the page-owned current overview collection input used by failure-coverage comparison.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    stamp = _stamp()
    json_path = ARTIFACT_DIR / f"design_guide_shear_low_util_candidate_evaluation_cutover_{stamp}.json"
    report_path = AUDIT_DIR / f"design_guide_shear_low_util_candidate_evaluation_cutover_{stamp}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(report_path, payload)
    print(f"design_guide_shear_low_util_candidate_evaluation_cutover {status}")
