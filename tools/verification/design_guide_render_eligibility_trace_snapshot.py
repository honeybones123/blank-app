...
from inputs_page_modules.design_guide.render_eligibility import (  # noqa: E402
...
TRACE_SOURCE_FILES = (
    ...
    ROOT / "inputs_page_modules" / "design_guide" / "render_eligibility.py",
)
...
def _line_map_for_patterns(paths: tuple[Path, ...], patterns: dict[str, str]) -> dict[str, Any]:
    sources = [
    ...
    for key, pattern in patterns.items():
        ...
        if match_line is not None:
            for line_no in range(max(1, match_line - 5), min(len(match_lines), match_line + 8) + 1):
                snippet.append({"line": line_no, "text": match_lines[line_no - 1]})
        out[key] = {
            "found": match_line is not None,
            "path": str(match_path) if match_path is not None else None,
            "line": match_line,
            "pattern": pattern,
            "snippet": snippet,
        }
...
def _classify_case(case: dict[str, Any]) -> dict[str, Any]:
    publication = {
        "selected_family_id": case.get("selected_family_id"),
        "outcome_state": case.get("final_publication_outcome_state"),
    }
    decision = should_render_design_guide_slot_from_publication_eligibility(
        inputs_has_design_actions_or_loads=bool(case.get("inputs_has_design_actions_or_loads")),
        browser_test_mode=bool(case.get("browser_test_mode")),
        selected_family_id=case.get("selected_family_id"),
        active_failures=case.get("active_failures"),
        invalid_input_state=bool(case.get("invalid_input_state")),
        blocker_state=bool(case.get("blocker_state")),
        final_publication=publication,
        debug_bundle={},
    )
    page_gate_allows = bool(decision.get("current_page_gate"))
    contract_required = bool(decision.get("contract_required_design_brain_eligibility"))
    adapter_should_render = bool(decision.get("should_render_design_guide_slot"))
    classification = str(decision.get("render_eligibility_classification") or "")
    reason = str(decision.get("render_eligibility_reason") or "")
    return {
        **case,
        "current_page_gate_allows": page_gate_allows,
        "contract_required_design_brain_eligibility": contract_required,
        "adapter_should_render_design_guide_slot": adapter_should_render,
        "classification": classification,
        "classification_matches_expected": classification == case.get("expected_classification"),
        "render_eligibility_reason": reason,
        "contract_gap": bool(contract_required and not adapter_should_render),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    trace_static_map = _line_map_for_patterns(TRACE_SOURCE_FILES, TRACE_STATIC_PATTERNS)
