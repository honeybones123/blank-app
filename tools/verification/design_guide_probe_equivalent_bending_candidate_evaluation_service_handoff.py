...
SERVICE_WRAPPER = "_evaluate_probe_equivalent_bending_candidate_with_service"
...
def _sample_parity() -> dict[str, Any]:
    ...
    candidate_id = "cleanup:bending:bottom_count_5"
    ...
    def evaluator_fn(state: dict[str, Any], **kwargs: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "candidate_search_evidence": {
                ...
                "search_scope": "synthetic_probe_equivalent_bending_cleanup_search",
            ...
            "title_main": "Bending cleanup - best safe one-click reduction",
            "guidance_change_lines": ["Reduce bottom reinforcement."],
        }

    old_direct = evaluator_fn(
        dict(candidate_state),
        source=source,
        label=None,
        action_type=None,
    )
    old_service_boundary = evaluate_design_candidate_with_updates(
        candidate_state,
        updates=None,
        source=source,
        label=None,
        action_type=None,
        state_snapshot_fn=passthrough,
        evaluator_fn=evaluator_fn,
    )
    new_service_boundary = evaluate_probe_equivalent_bending_candidate_with_updates(
        candidate_state,
        updates=None,
        source=source,
        label=None,
    ...
    return {
        ...
        "old_service_boundary": old_service_boundary,
        "new_service_boundary": new_service_boundary,
        "old_direct_hash": _stable_hash(old_direct),
        "old_service_boundary_hash": _stable_hash(old_service_boundary),
        "new_service_boundary_hash": _stable_hash(new_service_boundary),
        "candidate_overview_hashes": {
            "old": _stable_hash(dict(old_direct.get("overview") or {})),
            "new": _stable_hash(dict(new_service_boundary.get("overview") or {})),
        },
        "candidate_evidence_hashes": {
            "old": _stable_hash(dict(old_direct.get("candidate_search_evidence") or {})),
            "new": _stable_hash(dict(new_service_boundary.get("candidate_search_evidence") or {})),
        },
        "updates_hashes": {
            "old": _stable_hash(dict(old_direct.get("updates") or {})),
            "new": _stable_hash(dict(new_service_boundary.get("updates") or {})),
        },
        "button_contract_hashes": {
            "old": _stable_hash(dict(old_direct.get("button_contract") or {})),
            "new": _stable_hash(dict(new_service_boundary.get("button_contract") or {})),
        },
        "action_payload_hashes": {
            "old": _stable_hash(dict(old_direct.get("action_payload") or {})),
            "new": _stable_hash(dict(new_service_boundary.get("action_payload") or {})),
        },
        "visible_wording_hashes": {
            "old": _stable_hash(
                {
                    "title_main": old_direct.get("title_main"),
                    "guidance_change_lines": old_direct.get("guidance_change_lines"),
                }
            ),
            "new": _stable_hash(
                {
                    "title_main": new_service_boundary.get("title_main"),
                    "guidance_change_lines": new_service_boundary.get("guidance_change_lines"),
                }
            ),
        },
        "debug_proof_hashes": {
            "old": _stable_hash(
                {
                    "candidate_search_evidence": old_direct.get("candidate_search_evidence"),
                    "source": old_direct.get("source"),
                    "candidate_id": old_direct.get("candidate_id"),
                }
            ),
            "new": _stable_hash(
                {
                    "candidate_search_evidence": new_service_boundary.get("candidate_search_evidence"),
                    "source": new_service_boundary.get("source"),
                    "candidate_id": new_service_boundary.get("candidate_id"),
                }
            ),
        },
        "calls": calls,
...
def _capture() -> dict[str, Any]:
    ...
    shim_start, shim_end, shim_source = _function_source(inputs_source, "_evaluate_auto_design_candidate")
    parity = _sample_parity()
    return {
        "schema": "design_guide_probe_equivalent_bending_candidate_evaluation_service_handoff.v1",
        "target": {
            "name": TARGET,
        ...
        "compatibility_shim": {
            "name": "_evaluate_auto_design_candidate",
            "line_start": shim_start,
            "line_end": shim_end,
            "line_count": max(0, shim_end - shim_start + 1),
            "retained": bool(shim_source),
        },
        "parity": parity,
        "source_checks": {
            "target_no_longer_calls_evaluate_candidate_full_directly": "evaluate_candidate_full(" not in target_source,
            "target_uses_service_wrapper": "_evaluate_probe_equivalent_bending_candidate_with_service(" in target_source,
            ...
            "service_wrapper_uses_candidate_evaluation_helper": "_evaluate_probe_equivalent_bending_candidate_with_updates(" in wrapper_source,
            "service_wrapper_uses_passthrough_state": "_candidate_evaluation_state_passthrough" in wrapper_source,
            "candidate_evaluation_helper_exists": "def evaluate_probe_equivalent_bending_candidate_with_updates(" in candidate_source,
            "candidate_evaluation_exports_helper": '"evaluate_probe_equivalent_bending_candidate_with_updates"' in candidate_source,
            ...
            "target_packaging_paths_still_present": all(
                ...
                for token in (
                    ...
                    "debug_sink[\"probe_equivalent_bending_cleanup_candidate_rows\"]",
                )
            ),
...
def _checks(capture: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        "old_direct_and_service_boundary_equivalent": (
            parity.get("old_direct_hash") == parity.get("old_service_boundary_hash")
        ),
        "old_direct_and_new_service_equivalent": (
            parity.get("old_direct_hash") == parity.get("new_service_boundary_hash")
        ),
        ...
        "selected_candidate_id_unchanged": (
            (parity.get("old_direct") or {}).get("candidate_id")
            == (parity.get("new_service_boundary") or {}).get("candidate_id")
        ),
        "updates_unchanged": (
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Probe-Equivalent Bending Candidate Evaluation Service Handoff",
        "",
        "## Executive Summary",
    ...
    lines.extend(
        [
            ...
            "",
            "## Next",
            "Keep `_direct_target_band_guidance_item(...)` untouched. The next safe target is the next small candidate-evaluation handoff from the target-band boundary audit.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def _append_progress(payload: dict[str, Any], report_path: Path) -> None:
    ...
    lines.extend(
        [
            f"## {payload.get('created_at')} - Probe-equivalent bending candidate evaluation service handoff",
            "",
...
def main() -> int:
    ...
    payload = {
        ...
        "decision": "PROBE_EQUIVALENT_BENDING_CANDIDATE_EVALUATION_SERVICE_HANDOFF_PASS"
        if passed
        else "PROBE_EQUIVALENT_BENDING_CANDIDATE_EVALUATION_SERVICE_HANDOFF_FAIL",
        "capture": capture,
        "checks": checks,
    }
    suffix = created_at.replace(":", "-")
    json_path = ARTIFACT_DIR / f"design_guide_probe_equivalent_bending_candidate_evaluation_service_handoff_{suffix}.json"
    report_path = AUDIT_DIR / f"design_guide_probe_equivalent_bending_candidate_evaluation_service_handoff_{suffix}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")
    _write_report(report_path, payload)
    _append_progress(payload, report_path)
    print(f"design_guide_probe_equivalent_bending_candidate_evaluation_service_handoff {payload['status']}")
