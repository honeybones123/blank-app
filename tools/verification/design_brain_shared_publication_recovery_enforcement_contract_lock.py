...
def _build_source_checks() -> dict[str, Any]:
    ...
    design_brain_init_source = _read(DESIGN_BRAIN_INIT)
    ...
    return {
        ...
        "public_enforcers_defined_by_publication_module": all(
            f"def {name}(" in publication_source for name in public_enforcers
        ),
        "shell_does_not_own_publication_enforcement": all(
            name not in inputs_source for name in public_enforcers
        ),
        "public_publication_contract_export_is_narrow": (
            "enforce_design_brain_publication_contract" in design_brain_init_source
            and "enforce_family_selection_publication_contract" not in design_brain_init_source
            and "enforce_underdesign_repair_publication_boundary" not in design_brain_init_source
        ),
