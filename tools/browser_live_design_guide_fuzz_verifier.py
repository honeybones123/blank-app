...
from tools.verification.helpers.browser_helpers import (  # noqa: E402
    ...
    _same_value,
    _with_fragment_browser_state_overlay,
...
FAILED_DESIGN_LOCK_BLOCKER_TERMS = (
    ...
    "section depth is locked",
    "section width locked",
    "section width is locked",
    "depth locked",
    "depth is locked",
    "width locked",
    "width is locked",
...
def parse_visible_design_guide(page, browser_state: dict[str, Any]) -> dict[str, Any]:
    ...
    for index in range(count):
        ...
        try:
            ...
            selected_family_id = str(
                loc.get_attribute("data-selected-family-id") or ""
            ).strip()
            try:
                canonical_title = str(
                    loc.locator("[data-testid='design-guide-title']")
            ...
            try:
                canonical_status = str(
                    loc.locator("[data-testid='design-guide-status-pill']")
                    .first.inner_text(timeout=1_000)
                    or ""
                ).strip().upper()
            except Exception:
    ...
    canonical_final_publication = dict(
        proof.get("final_design_guide_publication") or {}
    )
    canonical_final_publication_sections = dict(
        dict(canonical_final_publication.get("display") or {}).get(
    ...
    contract = dict(
        browser_state.get("design_guide_primary_button_contract")
        or browser_state.get("primary_button_contract")
        or guidance.get("primary_button_contract")
        or {}
    )
    executor_backed_proof = dict(contract.get("executor_backed_proof") or {})
    if "preview_pass" not in contract and "preview_pass" in executor_backed_proof:
        contract["preview_pass"] = bool(executor_backed_proof.get("preview_pass"))
    ...
    canonical_family = str(
        (cards[0] if cards else {}).get("selected_family_id") or ""
    ).strip()
    if canonical_family:
        family = canonical_family
    elif "bending and shear" in title_l or "combined" in title_l or (
    ...
    for attempts_source in (
        ...
        canonical_final_publication_sections.get(
        ...
        for family_key, attempt_payload in attempts_source.items():
            if not isinstance(attempt_payload, dict):
                continue
            attempt_family = str(family_key or "").strip().lower()
            if not attempt_family:
                continue
            existing_attempt = dict(
                blocker_attempts_by_family.get(attempt_family) or {}
            )
            existing_attempt.update(dict(attempt_payload))
            blocker_attempts_by_family[attempt_family] = existing_attempt
...
def _assert_design_guide_layout_contract_sync(page, case_context: dict[str, Any] | None = None, *, allow_transient: bool = False) -> dict[str, Any]:
        ...
        placement_info = dict(
            page.evaluate(
                """() => {
                    ...
                    let batchDesignNode = null;
                    let widgetInputNode = null;
                    ...
                    while (walker.nextNode()) {
                        ...
                        if (visible && !batchDesignNode && /^Batch design$/i.test(text)) {
                            batchDesignNode = walker.currentNode;
                        }
                        if (
                            visible &&
                            !widgetInputNode &&
                            (
                                /^Positive design moment/i.test(text) ||
                                /^Bottom cover \\(mm\\)$/i.test(text) ||
                                /^Geometry & Materials$/i.test(text)
                            )
                        ) {
                            widgetInputNode = walker.currentNode;
                        }
                    }
                    const summaryNode = document.querySelector(
                        '.summary-card-stack, [data-testid="inputs-summary-row"], [data-testid="summary-card-stack"]'
                    );
                    const designGuideIndex = indexOf(card);
                    const batchDesignIndex = indexOf(batchDesignNode);
                    const widgetInputIndex = indexOf(widgetInputNode);
                    const summaryIndex = indexOf(summaryNode);
                    return {
                        design_guide_index: designGuideIndex,
                        batch_design_index: batchDesignIndex,
                        batch_design_found: Boolean(batchDesignNode),
                        widget_input_index: widgetInputIndex,
                        widget_input_found: Boolean(widgetInputNode),
                        summary_index: summaryIndex,
                        summary_found: Boolean(summaryNode),
                        design_guide_card_found: Boolean(card),
                        batch_design_before_design_guide:
                            designGuideIndex >= 0 && batchDesignIndex >= 0
                                ? batchDesignIndex < designGuideIndex
                                : null,
                        summary_before_design_guide:
                            designGuideIndex >= 0 && summaryIndex >= 0
                                ? summaryIndex < designGuideIndex
                                : null,
                        design_guide_before_widget_inputs:
                            designGuideIndex >= 0 && widgetInputIndex >= 0
                                ? designGuideIndex < widgetInputIndex
                                : null,
                    };
                }"""
            )
    ...
    if (
        card_count == 1
        and placement_info.get("batch_design_found")
        and placement_info.get("batch_design_before_design_guide") is False
    ):
        failures.append("design_guide_wrong_page_order: Design Guide card must appear below Batch design")
    if (
        card_count == 1
        and placement_info.get("summary_found")
        and placement_info.get("summary_before_design_guide") is False
    ):
        failures.append("design_guide_wrong_page_order: Design Guide card must appear below summary tables")
    if (
        card_count == 1
        and placement_info.get("widget_input_found")
        and placement_info.get("design_guide_before_widget_inputs") is False
    ):
        failures.append("design_guide_wrong_page_order: Design Guide card must appear above widget inputs and diagrams")
...
def _ladder_stop_evidence_required(step: dict[str, Any]) -> bool:
    ...
    card_type_value = _effective_card_type(summary, card, state)
...
def _exact_cleanup_terminal_blocker(row: dict[str, Any]) -> bool:
    ...
    search_ran = bool(
        ...
        or (family and row.get(f"{family}_cleanup_search_ran"))
        or (family and row.get(f"post_click_{family}_cleanup_search_ran"))
    )
    search_exhaustive = bool(
        ...
        or (family and row.get(f"{family}_cleanup_search_exhaustive"))
        or (family and row.get(f"post_click_{family}_cleanup_search_exhaustive"))
    )
    executable_count = _deep_get_count(
        row,
        keys=(
            ...
            f"post_click_executable_{family}_cleanup_count",
        ),
    )
...
def _effective_card_type(
    summary: dict[str, Any],
    card: dict[str, Any],
    browser_state: dict[str, Any] | None = None,
) -> str:
    """Classify evidence-backed no-action cleanup publications as terminal."""
...
def colour_alignment(
    ...
    ctype = _effective_card_type(summary, card, browser_state)
    contract = dict(card.get("button_contract") or {})
    ...
    if active:
        ...
        if set(active) == {"bending", "shear"} and (
            colour != "red" or not _family_matches(str(card.get("family") or ""), "combined")
        ):
...
_FAMILY_ID_COVERAGE: dict[str, set[str]] = {
    ...
    "combined_overdesign_governs": {"bending", "shear", "combined"},
}
    ...
    for family in required_families:
        ...
        required = {
            ...
            "geometry_strengthening_searched": bool(
                combined.get("geometry_strengthening_searched")
                or combined.get("geometry_locked")
            ),
...
def _blocker_text_is_specific(card: dict[str, Any], state: dict[str, Any], families: list[str]) -> tuple[bool, str]:
    structured_attempt_text = " ".join(
        str(
            dict(row or {}).get("family_specific_reason")
            or dict(row or {}).get("reason")
            or ""
        )
        for row in dict(card.get("blocker_attempts_by_family") or {}).values()
        if isinstance(row, dict)
    )
    text = (
        f"{card.get('title') or ''} {card.get('text') or ''} "
        f"{structured_attempt_text}"
    ).lower()
    first_card = {}
    try:
        first_card = dict(list(card.get("cards") or [{}])[0] or {})
    ...
    def _has_family_reason(family: str) -> bool:
        structured_attempt = dict(
            dict(card.get("blocker_attempts_by_family") or {}).get(family)
            or {}
        )
        return (
            bool(
                structured_attempt.get("family_specific_reason")
                or structured_attempt.get("reason")
            )
            or
...
def _post_click_final_state_analysis(step: dict[str, Any], previous_step: dict[str, Any] | None) -> dict[str, Any]:
    ...
    card = dict(step.get("visible_design_guide") or {})
    state = dict(step.get("browser_state") or {})
    ctype = _effective_card_type(summary, card, state)
    active = active_fail_families(summary)
...
def build_optimisation_audit(summary: dict[str, Any], card: dict[str, Any], state: dict[str, Any]) -> dict[str, Any]:
    ...
    if proof_family is None and opt_family in {"bending", "shear"}:
        proof_family = opt_family
    proof = blocker_proof_analysis(card, state, proof_family)
    return {
        "optimisation_family": opt_family,
        "optimisation_type": opt_type,
        "card_type": _effective_card_type(summary, card, state),
...
def assert_optimisation_contract(step: dict[str, Any]) -> None:
    ...
    terminal_family_proven = bool(
        ctype == "TERMINAL"
        and opt_family in {"bending", "shear"}
        and has_exact_blocker(state, opt_family)
    )
    if (
        opt_family in {"bending", "shear"}
        and card_family
        and not _family_matches(card_family, opt_family)
        and not terminal_family_proven
    ):
        _fail("optimisation_wrong_family", f"Visible card family {card_family} does not match optimisation family {opt_family}.", step)
    if opt_family == "combined" and not (
        _family_matches(card_family, "combined") or card_family in {"bending", "shear"}
    ):
...
def _assert_focused_replay_acceptance_contract(
    ...
    *,
    ...
    audit = {
        ...
        "design_guide_card_type": _effective_card_type(
            summary,
            card,
...
def assert_visible_contract(
    ...
    *,
    ...
    if after_mutation and previous_step:
        ...
        prev_state = dict(previous_step.get("browser_state") or {})
        prev_publication_hash = str(
            dict(prev_state.get("final_publication_hashes") or {}).get(
                "publication_hash"
            )
            or ""
        )
        current_publication_hash = str(
            dict(state.get("final_publication_hashes") or {}).get(
                "publication_hash"
            )
            or ""
        )
        same_card = (
            str(prev_card.get("title") or "") == str(card.get("title") or "")
            and str(prev_card.get("text") or "") == str(card.get("text") or "")
            and dict(prev_card.get("family_status_current") or {})
            == dict(card.get("family_status_current") or {})
            and dict(prev_card.get("family_status_preview") or {})
            == dict(card.get("family_status_preview") or {})
            and prev_publication_hash == current_publication_hash
        )
        ...
        try:
            ...
            for candidate in dom_candidates:
                ...
                try:
                    parsed = json.loads(raw)
                    if isinstance(parsed, dict):
                        merged = _with_fragment_browser_state_overlay(
                            page,
                            parsed,
                            timeout_ms=1_000,
                        )
                        return merged, {
                            ...
                            "source": source,
                            "raw_length": len(raw),
                            "fragment_overlay_merged": merged is not parsed,
...
# Input edits run inside the Inputs fragment.  Its fragment-local browser-state
# overlay is newer than the outer app probe until the next full-app rerun, so
# keep the shared helper's overlay-aware loader for the apply transaction.
try:
    _apply_live_inputs.__globals__["_load_browser_state"] = _load_browser_state_by_label
except Exception:
    pass
...
def _wait_for_stable_editable_input(page, label: str, *, timeout_s: float = 30.0) -> tuple[Any, dict[str, Any]]:
    ...
    while time.time() < deadline:
        ...
        try:
            fresh_locator = page.locator(selector).first
            if fresh_locator.count() == 1:
                fresh_locator.evaluate(
