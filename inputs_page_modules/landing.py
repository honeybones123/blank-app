"""Inputs landing-state and landing-card render helpers."""
...
<div class="inputs-landing-wrap">
  ...
  <ul class="inputs-landing-options">
    <li>Enter design actions (M*, V*, Ïƒ)</li>
...
""",
    ...
    b1, b2 = st_module.columns(2, gap="small")
    with b1:
        if st_module.button("Go to Design Inputs", key="inputs_landing_go_design_inputs", use_container_width=True):
            st_module.session_state[scroll_design_actions_flag] = True
            st_module.rerun()
    with b2:
        if st_module.button("Open Design Mode", key="inputs_landing_open_detailed", use_container_width=True):
