...
def _build_final_solver_return_coordinator(
    *,
    ...
    dbg = {
        "iteration_count": len(step_trace),
        "initial_worst_util": init_worst,
        "final_worst_util": final_worst,
        "target_band": {"min": t_lo, "max": t_hi},
        "stop_reason": stop_reason,
        "reached_target_band": bool(final_in_band and final_pass),
        "step_candidate_labels": [str(s.get("label") or "") for s in step_trace],
        "all_key_pass": final_pass,
        "status": status,
        "trace_run_id": rid,
        **_coherence_debug_fields(initial_coherence),
        ...
        "governing_family_exists_after_domain_fix": bool(governing_family_exists_after_domain_fix),
        "pruned_non_shear_family_count": int(pruned_non_shear_family_count),
        "governing_domain": final_governing_domain,
        ...
        "target_domains_for_band": list(target_domains_for_band),
        "final_target_domains_eval": list(final_target_domains),
        "final_eval_band_trace": (
            _one_click_trace_eval_domain_payload(final_eval, mode_config)
            if isinstance(final_eval, dict)
            else {}
        ),
        "step_committable_eval_trace": list(step_committable_eval_trace),
        "final_eval_internal_worst_util": final_eval_internal_worst_util_dbg,
        "final_eval_committable_worst_util": final_eval_committable_worst_util_dbg,
        "final_eval_used_source": final_eval_used_source_dbg,
        "final_eval_committable_updates": dict(final_eval_committable_updates_dbg or {}),
        "final_objective_util": final_objective_util,
        "shear_remove_links_candidate_seen": bool(shear_remove_links_candidate_seen),
        "shear_remove_links_candidate_truth_ok": bool(shear_remove_links_candidate_truth_ok),
        ...
        "early_in_band_exit_available_tightening_paths": list(early_in_band_exit_available_tightening_paths),
        "early_in_band_exit_reason": early_in_band_exit_reason,
        "partial_failing_final_updates_blocked": bool(partial_failing_final_updates_blocked),
        "partial_failing_final_updates_raw": dict(partial_failing_final_updates_raw),
        "best_available_out_of_band_retained": bool(best_available_out_of_band_retained),
    }
...
def _handle_one_click_solver_candidate_shear_truth_and_preview_gate_coordinator(
    *,
    ...
    return {
        ...
        "rejected_as_web_crushing_marginal": rejected_as_web_crushing_marginal,
        "rejected_as_impractical_shear_layout": rejected_as_impractical_shear_layout,
        "should_continue": shear_preview_rejection_gate_state["should_continue"],
    }
...
def _prepare_one_click_solver_candidate_scoring_state_coordinator(
    *,
    ...
    if has_target_domains and okp and not mixed_direction_mode:
        ...
        required_fail_count = int(domain_progress.get("required_fail_count", 0) or 0)
        required_unsatisfied_count = int(domain_progress.get("required_unsatisfied_count", 0) or 0)
    mixed_rank = _one_click_mixed_direction_rank_adjustment(cur_eval, peval, mixed_direction_mode, mode_config)
    mixed_sort_prefix = (
        (
...
def _run_one_click_solver_single_candidate_pre_metric_gate_flow_coordinator(
    *,
    ...
    if domain_prune_state["should_continue"]:
        return _state(should_continue=True)
    ...
    if preview_eval_state["should_continue"]:
        return _state(peval=peval, preview=preview, should_continue=True)
    ...
    if duplicate_signature_state["should_continue"]:
        return _state(peval=peval, preview=preview, psig=psig, should_continue=True)

    return _state(peval=peval, preview=preview, psig=psig)
...
def _run_one_click_solver_single_candidate_post_metric_scoring_flow_coordinator(
    *,
    ...
    scalar_metric_state = _prepare_one_click_solver_candidate_scalar_metric_state_coordinator(
        peval=peval,
        cur_eval=cur_eval,
        working=working,
        norm_u=norm_u,
...
def _run_one_click_solver_candidate_scoring_loop_coordinator(
    *,
    ...
    for entry in prepared:
        ...
        scored = single_candidate_scoring_flow_state["scored"]
        rejected_as_non_governing_cleanup = single_candidate_scoring_flow_state[
            "rejected_as_non_governing_cleanup"
        ]
        rejected_as_non_governing_shear_strengthening = (
            single_candidate_scoring_flow_state[
        ...
        rejected_as_evaluation_failed = single_candidate_scoring_flow_state[
            "rejected_as_evaluation_failed"
        ]
        rejected_as_duplicate_signature = single_candidate_scoring_flow_state[
            "rejected_as_duplicate_signature"
        ]
        rejected_as_non_material_improvement = single_candidate_scoring_flow_state[
            "rejected_as_non_material_improvement"
        ]
        growth_candidates_rejected_in_tightening = single_candidate_scoring_flow_state[
            "growth_candidates_rejected_in_tightening"
        ]
        shear_remove_links_candidate_seen = single_candidate_scoring_flow_state[
            "shear_remove_links_candidate_seen"
        ]
        shear_remove_links_candidate_truth_ok = single_candidate_scoring_flow_state[
            "shear_remove_links_candidate_truth_ok"
        ]
        shear_remove_links_candidate_dropped_reason = single_candidate_scoring_flow_state[
            "shear_remove_links_candidate_dropped_reason"
        ]
        shear_remove_links_candidate_materiality = single_candidate_scoring_flow_state[
            "shear_remove_links_candidate_materiality"
        ]
        rejected_as_spacing_too_weak = single_candidate_scoring_flow_state[
            "rejected_as_spacing_too_weak"
        ]
        rejected_as_web_crushing_marginal = single_candidate_scoring_flow_state[
            "rejected_as_web_crushing_marginal"
        ]
        rejected_as_impractical_shear_layout = single_candidate_scoring_flow_state[
            "rejected_as_impractical_shear_layout"
        ]
        web_crushing_penalty_applied = single_candidate_scoring_flow_state[
            "web_crushing_penalty_applied"
        ]
...
def _resolve_one_click_solver_scored_candidate_selection_state_coordinator(
    *,
    ...
    candidate_fallback_pool_trace_state = _handle_one_click_solver_candidate_fallback_pool_trace_coordinator(
        scored=scored,
        cur_eval=cur_eval,
        working=working,
        mode_config=mode_config,
    ...
    scored = candidate_fallback_pool_trace_state["scored"]
    fallback_next_hop_injected = candidate_fallback_pool_trace_state[
        "fallback_next_hop_injected"
    ]
    fallback_next_hop_reason = candidate_fallback_pool_trace_state[
        "fallback_next_hop_reason"
    ]
...
def _prepare_one_click_solver_candidate_pipeline_state_coordinator(
    *,
    ...
    raw_candidates = candidate_collection_state["raw_candidates"]
    raw_n = candidate_collection_state["raw_n"]
    use_governing_domain_candidates = candidate_collection_state["use_governing_domain_candidates"]
    tightening_meta = candidate_collection_state["tightening_meta"]
    candidate_family_depth_reached = candidate_collection_state["candidate_family_depth_reached"]
    shear_governing_mode_active = candidate_collection_state["shear_governing_mode_active"]
    shear_severity_band = candidate_collection_state["shear_severity_band"]
    shear_candidate_family_order = candidate_collection_state["shear_candidate_family_order"]
    spacing_candidates_considered = candidate_collection_state["spacing_candidates_considered"]
    leg_candidates_considered = candidate_collection_state["leg_candidates_considered"]
    dia_candidates_considered = candidate_collection_state["dia_candidates_considered"]
    geometry_candidates_considered_for_shear = candidate_collection_state["geometry_candidates_considered_for_shear"]
    combined_candidates_considered_for_shear = candidate_collection_state["combined_candidates_considered_for_shear"]
    ...
    pool_labels = pre_scoring_state["pool_labels"]
    prepared = pre_scoring_state["prepared"]
    prepared_samples = pre_scoring_state["prepared_samples"]
    reduction_candidates_considered = pre_scoring_state["reduction_candidates_considered"]
    governing_family_exists = pre_scoring_state["governing_family_exists"]
    shear_governing_family_detected = pre_scoring_state["shear_governing_family_detected"]
    governing_family_exists_after_domain_fix = pre_scoring_state["governing_family_exists_after_domain_fix"]
    shear_domain_prune_active = pre_scoring_state["shear_domain_prune_active"]
    should_apply_domain_prune = pre_scoring_state["should_apply_domain_prune"]
    mixed_direction_mode = pre_scoring_state["mixed_direction_mode"]
    growth_candidates_rejected_in_tightening = pre_scoring_state["growth_candidates_rejected_in_tightening"]
    rejected_as_non_governing_cleanup = pre_scoring_state["rejected_as_non_governing_cleanup"]
    rejected_as_non_governing_shear_strengthening = pre_scoring_state[
        "rejected_as_non_governing_shear_strengthening"
    ]
    rejected_as_non_material_improvement = pre_scoring_state["rejected_as_non_material_improvement"]
    rejected_as_no_real_change = pre_scoring_state["rejected_as_no_real_change"]
    rejected_as_duplicate_signature = pre_scoring_state["rejected_as_duplicate_signature"]
    rejected_as_evaluation_failed = pre_scoring_state["rejected_as_evaluation_failed"]
    shear_remove_links_candidate_seen = pre_scoring_state["shear_remove_links_candidate_seen"]
    shear_remove_links_candidate_dropped_reason = pre_scoring_state[
        "shear_remove_links_candidate_dropped_reason"
    ]
    pruned_non_shear_family_count = pre_scoring_state["pruned_non_shear_family_count"]
    domain_match_prune_used = pre_scoring_state["domain_match_prune_used"]
    shear_prune_rule_source = pre_scoring_state["shear_prune_rule_source"]

    iteration_scoring_start_state = _prepare_one_click_solver_iteration_scoring_start_state_coordinator(
        step_idx=step_idx, cur_sig=cur_sig, working=working, cur_eval=cur_eval,
        t_lo=t_lo, t_hi=t_hi,
        tightening_mode_active=tightening_mode_active,
        governing_domain=governing_domain,
        material_improvement_threshold=material_improvement_threshold,
        ...
        no_actionable_after_full_tightening_search=no_actionable_after_full_tightening_search,
        shear_governing_mode_active=shear_governing_mode_active,
        shear_severity_band=shear_severity_band, shear_candidate_family_order=shear_candidate_family_order,
        shear_governing_family_detected=shear_governing_family_detected,
        governing_family_exists_after_domain_fix=governing_family_exists_after_domain_fix,
        mixed_direction_mode=mixed_direction_mode,
        pruned_non_shear_family_count=pruned_non_shear_family_count,
    ...
    shear_prune_rule_source = iteration_scoring_start_state["shear_prune_rule_source"]
    cur_has_td = iteration_scoring_start_state["cur_has_td"]
    cur_required_fail_count = iteration_scoring_start_state["cur_required_fail_count"]
    cur_required_unsatisfied_count = iteration_scoring_start_state["cur_required_unsatisfied_count"]
    scored: list[dict] = iteration_scoring_start_state["scored"]
...
def _prepare_one_click_solver_runtime_setup_state_coordinator(
    *,
    ...
    initial_coherence_should_block = bool(solver_initial_state["initial_coherence_should_block"])
    initial_stop_reason = solver_initial_state["initial_stop_reason"]
    if (not initial_pack_valid) or initial_coherence_should_block:
        return {
            "should_return": True,
    ...
    working = solver_mode_budget_state["working"]
    mode_config = solver_mode_budget_state["mode_config"]
    t_lo = solver_mode_budget_state["t_lo"]
    t_hi = solver_mode_budget_state["t_hi"]
    max_tightening_steps = solver_mode_budget_state["max_tightening_steps"]
    tightening_budget_extensions_used = solver_mode_budget_state["tightening_budget_extensions_used"]
    tightening_budget_extension_cap = solver_mode_budget_state["tightening_budget_extension_cap"]
    tightening_step_count = solver_mode_budget_state["tightening_step_count"]
    no_actionable_after_full_tightening_search = solver_mode_budget_state[
        "no_actionable_after_full_tightening_search"
    ]
    candidate_family_depth_reached = solver_mode_budget_state["candidate_family_depth_reached"]
    final_distance_to_band = solver_mode_budget_state["final_distance_to_band"]
    shear_governing_mode_active = solver_mode_budget_state["shear_governing_mode_active"]
    shear_severity_band = solver_mode_budget_state["shear_severity_band"]
    shear_candidate_family_order = solver_mode_budget_state["shear_candidate_family_order"]
    spacing_candidates_considered = solver_mode_budget_state["spacing_candidates_considered"]
    leg_candidates_considered = solver_mode_budget_state["leg_candidates_considered"]
    dia_candidates_considered = solver_mode_budget_state["dia_candidates_considered"]
    geometry_candidates_considered_for_shear = solver_mode_budget_state["geometry_candidates_considered_for_shear"]
    combined_candidates_considered_for_shear = solver_mode_budget_state["combined_candidates_considered_for_shear"]
    web_crushing_penalty_applied = solver_mode_budget_state["web_crushing_penalty_applied"]
    rejected_as_spacing_too_weak = solver_mode_budget_state["rejected_as_spacing_too_weak"]
    rejected_as_web_crushing_marginal = solver_mode_budget_state["rejected_as_web_crushing_marginal"]
    rejected_as_impractical_shear_layout = solver_mode_budget_state["rejected_as_impractical_shear_layout"]
    final_resolved_shear_util = solver_mode_budget_state["final_resolved_shear_util"]
    final_resolved_web_util = solver_mode_budget_state["final_resolved_web_util"]
    step_committable_eval_trace = solver_mode_budget_state["step_committable_eval_trace"]
    ...
    target_domains_for_band = solver_initial_eval_state["target_domains_for_band"]
    ...
    early_in_band_exit_reason = early_in_band_gate_state["early_in_band_exit_reason"]
    ...
    solver_iteration_state = _prepare_one_click_solver_iteration_state_coordinator(init_eval=init_eval)
...
def _handle_one_click_solver_accepted_candidate_post_step_coordinator(
    *,
    ...
    if apply_selected_candidate_state["should_break"]:
        return _build_one_click_solver_apply_selected_candidate_break_state_coordinator(
...
def _dispatch_one_click_solver_post_selection_candidate_selection_state_coordinator(
    *,
    ...
    return _resolve_one_click_solver_scored_candidate_selection_state_coordinator(
        ...
        step_committable_eval_trace=post_selection_scope["step_committable_eval_trace"],
...
def _run_one_click_solver_iteration_loop_coordinator(
    *,
    ...
    for step_idx in range(max_steps):
        iteration_gate_state = _prepare_one_click_solver_iteration_gate_state_coordinator(
            step_idx=step_idx,
            working=working,
            mode_config=mode_config,
            target_band_domain=target_band_domain,
            ...
            max_tightening_steps=max_tightening_steps,
            tightening_budget_extensions_used=tightening_budget_extensions_used,
            tightening_budget_extension_cap=tightening_budget_extension_cap,
            candidate_family_depth_reached=candidate_family_depth_reached,
            trace_callback=trace_callback,
        )
...
def _finalize_one_click_solver_result_coordinator(
    *,
    ...
    final_eval_internal = final_evaluation_state["final_eval_internal"]
    final_updates = final_evaluation_state["final_updates"]
    final_eval = final_evaluation_state["final_eval"]
    final_eval_internal_worst_util_dbg = final_evaluation_state[
        "final_eval_internal_worst_util_dbg"
    ]
    final_eval_committable_worst_util_dbg = final_evaluation_state[
        "final_eval_committable_worst_util_dbg"
    ]
    final_eval_used_source_dbg = final_evaluation_state["final_eval_used_source_dbg"]
    final_eval_committable_updates_dbg = final_evaluation_state[
        "final_eval_committable_updates_dbg"
    ]
    final_target_domains = final_evaluation_state["final_target_domains"]
    final_worst = final_evaluation_state["final_worst"]
    final_pass = final_evaluation_state["final_pass"]
    final_ok = final_evaluation_state["final_ok"]
    final_spacing_fail = final_evaluation_state["final_spacing_fail"]
    final_in_band = final_evaluation_state["final_in_band"]
    final_band_hit = final_evaluation_state["final_band_hit"]
    final_objective_util = final_evaluation_state["final_objective_util"]
    final_distance_to_band = final_evaluation_state["final_distance_to_band"]
    final_resolved_shear_util = final_evaluation_state["final_resolved_shear_util"]
    final_resolved_web_util = final_evaluation_state["final_resolved_web_util"]
    ...
    winning_action_type = partial_failing_final_updates_guard_state["winning_action_type"]
    partial_failing_final_updates_blocked = partial_failing_final_updates_guard_state[
        "partial_failing_final_updates_blocked"
    ]
    partial_failing_final_updates_raw = partial_failing_final_updates_guard_state[
        "partial_failing_final_updates_raw"
    ]
    ...
    should_return_rescue_result, rescue_result = _dispatch_one_click_solver_rescue_seed_result_from_finalization_coordinator(finalization_scope=locals())
    if should_return_rescue_result:
        return rescue_result

    return _dispatch_one_click_solver_final_trace_return_coordinator(finalization_scope=locals())
...
def _handle_auto_design_blocked_incoherent_state_coordinator(
    *,
    ...
    blocked_dbg["one_click_run_feedback_cleared_at_entry"] = bool(
        one_click_run_feedback_cleared_at_entry
    )
...
def _run_auto_design_commit_orchestration_coordinator(
    *,
    ...
    if final_updates:
        commit_start_context = _prepare_auto_design_commit_start_coordinator(
            current_state=current_state,
            final_updates=final_updates,
            dbg=dbg,
            win_l=win_l,
