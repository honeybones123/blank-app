...
def _expand_longitudinal_reo_alias_updates(updates: dict, prior_state: dict | None = None) -> dict:
    expanded = dict(updates or {})
    ...
    if "bot1_count" in expanded and "bot_row_1_bars" not in expanded:
        expanded["bot_row_1_bars"] = expanded.get("bot1_count")
    if "bot_row_1_bars" in expanded and "bot1_count" not in expanded:
        expanded["bot1_count"] = expanded.get("bot_row_1_bars")
    if "db_bot_1" in expanded and "bot_row_1_dia" not in expanded:
        expanded["bot_row_1_dia"] = expanded.get("db_bot_1")
    if "bot_row_1_dia" in expanded and "db_bot_1" not in expanded:
        expanded["db_bot_1"] = expanded.get("bot_row_1_dia")
    if "bot1_layout_mode" in expanded and "bot_row_1_mode" not in expanded:
        expanded["bot_row_1_mode"] = expanded.get("bot1_layout_mode")
    if "bot_row_1_mode" in expanded and "bot1_layout_mode" not in expanded:
        expanded["bot1_layout_mode"] = expanded.get("bot_row_1_mode")
    if "bot1_spacing" in expanded and "bot_row_1_spacing" not in expanded:
        expanded["bot_row_1_spacing"] = expanded.get("bot1_spacing")
    if "bot_row_1_spacing" in expanded and "bot1_spacing" not in expanded:
        expanded["bot1_spacing"] = expanded.get("bot_row_1_spacing")

    if "bot2_count" in expanded and "bot_row_2_bars" not in expanded:
        expanded["bot_row_2_bars"] = expanded.get("bot2_count")
    if "bot_row_2_bars" in expanded and "bot2_count" not in expanded:
        expanded["bot2_count"] = expanded.get("bot_row_2_bars")
    if "db_bot_2" in expanded and "bot_row_2_dia" not in expanded:
        expanded["bot_row_2_dia"] = expanded.get("db_bot_2")
    if "bot_row_2_dia" in expanded and "db_bot_2" not in expanded:
        expanded["db_bot_2"] = expanded.get("bot_row_2_dia")
    if "bot2_layout_mode" in expanded and "bot_row_2_mode" not in expanded:
        expanded["bot_row_2_mode"] = expanded.get("bot2_layout_mode")
    if "bot_row_2_mode" in expanded and "bot2_layout_mode" not in expanded:
        expanded["bot2_layout_mode"] = expanded.get("bot_row_2_mode")
    if "bot2_spacing" in expanded and "bot_row_2_spacing" not in expanded:
        expanded["bot_row_2_spacing"] = expanded.get("bot2_spacing")
    if "bot_row_2_spacing" in expanded and "bot2_spacing" not in expanded:
        expanded["bot2_spacing"] = expanded.get("bot_row_2_spacing")

    if any(str(key).startswith("bot") or key in {"db_bot_1", "db_bot_2"} for key in expanded):
        row_1_bars = first("bot_row_1_bars", "bot1_count", default=0)
        row_2_bars = first("bot_row_2_bars", "bot2_count", default=0)
        try:
            row_count = 2 if int(float(row_2_bars or 0)) > 0 else 1 if int(float(row_1_bars or 0)) > 0 else 0
        except Exception:
            row_count = 2 if row_2_bars not in (None, "", 0, "0") else 1
        if "bot_row_count" not in expanded:
            expanded["bot_row_count"] = row_count
        if "bot1_layout_mode" in expanded and "bot_row_1_mode" not in expanded:
            expanded["bot_row_1_mode"] = expanded["bot1_layout_mode"]
        if "bot2_layout_mode" in expanded and "bot_row_2_mode" not in expanded:
            expanded["bot_row_2_mode"] = expanded["bot2_layout_mode"]
    return expanded
...
def apply_resolved_candidate_payload(
    *,
    ...
    st = st_module
    apply_store = ApplyTransactionStore(st.session_state)
    acceptance_store = PostApplyAcceptanceStore(st.session_state)
    ...
    DESIGN_GUIDE_COMPONENT_APPLY_IN_FLIGHT_KEY = getattr(
        legacy_page,
    ...
    def _clear_design_guide_component_apply_in_flight(reason: str) -> None:
        ...
        st.session_state["_design_guide_component_apply_in_flight_cleared_reason"] = str(reason)
    ...
    if not updates:
        ...
        _clear_design_guide_component_apply_in_flight("apply_resolved_candidate_missing_updates")
        return False
    ...
    updates = _expand_longitudinal_reo_alias_updates(updates, prior_state)
    ...
    if isinstance(applied_candidate, dict):
        try:
            ...
            if bool(preview_overview.get("all_key_pass")) and not bool(preview_overview.get("any_fail")):
                ...
                acceptance_store.enable(accepted_fp)
        except Exception:
            pass
    acceptance_store.enable()
    ...
    if not current_apply_guard.get("pass"):
        ...
        _clear_design_guide_component_apply_in_flight("apply_resolved_candidate_current_state_guard_blocked")
        return False
    ...
    try:
        ...
        pass
    post_overview: dict[str, Any] = {}
    post_apply_all_key_pass = False
    post_apply_any_fail = False
    post_apply_required_checks_pass = False
    ...
    if isinstance(post_apply_candidate, dict):
        ...
        post_apply_all_key_pass = bool(post_overview.get("all_key_pass"))
        post_apply_any_fail = bool(post_overview.get("any_fail"))
        post_apply_required_checks_pass = bool(post_apply_all_key_pass and not post_apply_any_fail)
        if post_apply_required_checks_pass:
            try:
    ...
    restore_post_cleanup_acceptance = bool(
        isinstance(post_apply_candidate, dict)
        and bool(dict(post_apply_candidate.get("overview") or {}).get("all_key_pass"))
        and not bool(dict(post_apply_candidate.get("overview") or {}).get("any_fail"))
    )
    st.session_state[DESIGN_GUIDE_LAST_APPLY_ROUTE_KEY] = {
        ...
        "post_apply_resolved_candidate_label": label,
        "post_apply_resolved_candidate_expected_util": expected_post_util,
        "post_apply_overview": dict(post_overview),
        "post_apply_all_key_pass": bool(post_apply_all_key_pass),
        "post_apply_any_fail": bool(post_apply_any_fail),
        "post_apply_required_checks_pass": bool(post_apply_required_checks_pass),
    }
    ...
    try:
        ...
        pass
    if restore_post_cleanup_acceptance:
        try:
            accepted_fp = _local_cleanup_acceptance_fingerprint(_shared_state_snapshot())
            acceptance_store.enable(accepted_fp)
            _DESIGN_GUIDE_POST_CLEANUP_ACCEPTED_FPS.add(accepted_fp)
    ...
    _set_design_guide_primary_payload_binding_audit(
        ...
        canonical_primary_payload_exists=bool(payload_dict.get("resolved_candidate_updates") or updates),
