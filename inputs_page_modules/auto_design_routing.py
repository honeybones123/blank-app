...
def handle_inputs_auto_design(
    *,
    ...
    legacy_page._set_design_guide_live_breadcrumb("DG HANDLE AUTO DESIGN ENTRY")
    print(
    ...
    def _auto_design_entry_probe(stage: str) -> dict:
        shared_probe = legacy_page._shared_state_snapshot()
        resolved_actions = legacy_page._resolve_design_actions_from_state(shared_probe)
        try:
            summary_state_probe, summary_state_debug = legacy_page._resolved_inputs_summary_state()
    ...
    try:
        legacy_page._append_design_guide_trace(
        ...
        pass
    reconciled_action_keys = legacy_page._reconcile_design_action_widgets_with_shared(selected_prefix)
    reconciled_shear_keys = legacy_page._reconcile_inputs_shear_widgets_with_shared()
    ...
    if session_state.get("_solver_running", False):
        ...
        if direct_request and not bool(session_state.get("_compute_in_progress", False)):
            if not latch_owner or latch_owner == "handle_auto_design":
                clear_payload = legacy_page._clear_auto_design_runtime_latches(
    ...
    try:
        ...
        if clicked:
            session_state.pop("_inputs_action_run_auto_design", None)
        legacy_page._set_design_guide_live_breadcrumb("DG CALLING CANONICAL ONE CLICK")
        print(
        ...
        result = legacy_page.run_one_click_auto_design(entry_source="inputs_handle_auto_design")
        ...
        if result:
            ...
            if isinstance(rr, dict) and isinstance(rr.get("updates"), dict) and rr.get("updates"):
                session_state["pending_recommendation"] = legacy_page._attach_recommendation_envelope(
                    {
                        **rr,
                        "_source": "auto_design",
                    },
                ...
                if not isinstance(preserved.get("recommendation_envelope"), dict):
                    preserved = legacy_page._attach_recommendation_envelope(
                        preserved,
