...
def _one_click_collect_actionable_guidance_candidates(
    ...
    *,
    ...
    payload = _compute_design_guidance_items(
        working_state,
    ...
    for item in raw_items:
        ...
        _ensure_guidance_item_resolved_candidate_payload(work, state=working_state)
        pl = dict(work.get("action_payload") or {})
        updates: dict | None = None
        try:
            resolved = _guidance_action_updates(action_type, pl, state=working_state)
        ...
        if not updates:
            try:
                resolved2 = _guidance_action_updates(action_type, work, state=working_state)
