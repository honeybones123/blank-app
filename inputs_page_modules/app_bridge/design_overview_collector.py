...
class DesignOverviewRuntime:
    build_bending_pack: Callable[[dict], dict]
    build_crack_pack: Callable[[dict], dict]
    build_deflection_pack: Callable[[dict], dict]
    build_actions_context: Callable[[dict], dict]
    build_shear_pack: Callable[[dict], dict]
    overall_status: Callable[[list[dict]], tuple[str, Any]]
    parse_util: Callable[[Any], float | None]
    resolve_actions: Callable[[dict], dict]
    stage3_shear_truth: Callable[[dict], dict]
    stage3_remaining_issue: Callable[[dict, dict], Any]
    state_with_actions: Callable[[dict], dict]
...
def _collect_design_overview(
    ...
    *,
    ...
    _build_bending_check_rows_from_state_for_app_bridge = runtime.build_bending_pack
    _build_crack_pack_from_state_for_app_bridge = runtime.build_crack_pack
    _build_deflection_pack_from_state_for_app_bridge = runtime.build_deflection_pack
    _build_design_actions_context_for_app_bridge = runtime.build_actions_context
    _build_shear_check_rows_from_state_for_app_bridge = runtime.build_shear_pack
    _overall_status_from_rows = runtime.overall_status
    _parse_util_value = runtime.parse_util
    _resolve_design_actions_from_state = runtime.resolve_actions
    _stage3_final_published_shear_truth_bundle_for_app_bridge = runtime.stage3_shear_truth
    _stage3_remaining_issue_class_from_overview_state = runtime.stage3_remaining_issue
    _state_with_resolved_design_actions_for_app_bridge = runtime.state_with_actions
    ...
    ux_probe_record = runtime.probe_record
    ...
    overview_fp = stable_fingerprint_for_payload(
        {
            "state": overview_state,
            "actions": actions,
        }
    )
    cached_overview = get_rerun_pure_cache("collect_design_overview", overview_fp)
    if isinstance(cached_overview, dict):
        ux_probe_record(
            "inputs_page.summary_overview_build",
            fingerprint=overview_fp,
            cache_hit=True,
        )
    ...
    set_rerun_pure_cache("collect_design_overview", overview_fp, overview_out)
    return overview_out
