...
_APPLY_RECOMMENDATION_RESULT_NAMES: tuple[str, ...] = (
    ...
    "_recommendation_envelope_from_pending",
...
def apply_recommendation_result(rec: dict) -> str:
    ...
    envelope = _recommendation_envelope_from_pending(rec)
    ...
    if mode:
        try:
            PostApplyAcceptanceStore(st.session_state).enable()
        except Exception:
            pass
    if mode and isinstance(payload, dict):
        if apply_guidance_action(mode, payload):
            OneClickSessionStore(st.session_state).clear_run_feedback()
            return "dispatch_ok"
    ...
    if apply_design_candidate(st.session_state, fallback_candidate):
        try:
            acceptance = PostApplyAcceptanceStore(st.session_state)
            acceptance.enable(
        ...
        OneClickSessionStore(st.session_state).clear_run_feedback()
        ApplyTransactionStore(st.session_state).update_route(
