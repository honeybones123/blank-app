...
class EfficiencyTighteningStateRuntime:
    ...
    candidate_is_growth_move: Callable[..., Any]
    collect_design_overview: Callable[..., Any]
    combined_underdesign_shear_strengthening_truth_gate_payload: Callable[..., Any]
    compute_bottom_reo_tightening_recommendation: Callable[..., Any]
    compute_geometry_tightening_recommendation: Callable[..., Any]
    compute_mode_guidance_recommendation: Callable[..., Any]
    ...
    shear_change_is_reinforcement_growth: Callable[..., Any]
    ...
    shear_overdesign_reserve_guidance_predicate: Callable[..., Any]
    state_with_resolved_design_actions: Callable[..., Any]
    updates_match_state: Callable[..., Any]
    float_from_state: Callable[..., Any]
    evaluate_candidate_full: Callable[..., Any]


def _bind_efficiency_tightening_runtime(
    runtime: EfficiencyTighteningStateRuntime,
) -> None:
    globals().update(
        {
            "GUIDANCE_INEFFICIENT_UTIL_THRESHOLD": runtime.guidance_inefficient_util_threshold,
            "GUIDANCE_NEAR_LIMIT_UTIL_THRESHOLD": runtime.guidance_near_limit_util_threshold,
            "GUIDANCE_STRONGLY_UNDERUTILISED_UTIL": runtime.guidance_strongly_underutilised_util,
            "GUIDANCE_UNDERSIZED_DONE_BLOCK_UTIL": runtime.guidance_undersized_done_block_util,
            "TARGET_BAND_EPS": runtime.target_band_eps,
            "VERY_LOW_DEMAND_UTIL_THRESHOLD": runtime.very_low_demand_util_threshold,
        }
    )
    for field_name in tuple(runtime.__dataclass_fields__)[6:]:
        globals()[
            field_name
            if field_name == "evaluate_candidate_full"
            else f"_{field_name}"
...
def compute_efficiency_tightening_state(
    ...
    *,
    runtime: EfficiencyTighteningStateRuntime | None = None,
) -> dict:
    if runtime is not None:
        _bind_efficiency_tightening_runtime(runtime)
