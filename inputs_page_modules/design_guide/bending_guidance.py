...
class BendingGuidanceRuntime:
    bending_item_from_geometry_trial: Callable[..., Any]
    bending_near_limit_specific_title: Callable[..., Any]
    ...
    reinforcement_options_remain: Callable[..., Any]


@dataclass(frozen=True)
class BendingGeometryTrialRuntime:
...
def _bending_item_from_geometry_trial(
    ...
    *,
    ...
    if util is None:
        return None
    trial = runtime.choose_geometry_trial_for_metric(
        state,
        metric="bending",
    ...
    if not trial:
        return None
    util_after = float(trial.get("util_after", 0.0) or 0.0)
    reasoning = (
        "Why: flexural demand is high. The suggested geometry step "
        f"({trial['label']}) targets bending utilisation "
        f"from {float(util):.2f} toward {util_after:.2f}."
    )
    chosen_title = runtime.geometry_trial_title_for_choice(
        title,
        trial,
        state,
    )
    updates = dict(trial.get("updates") or {})
    change_lines = runtime.guidance_change_lines_for_updates(
        state,
        updates,
    )
    return runtime.guidance_item(
        "bending",
        chosen_title,
        str(trial.get("label") or "Apply geometry trial"),
        secondary,
        reasoning,
        levers,
        str(trial.get("action_type") or "increase_depth"),
        dict(trial.get("payload") or {}),
        status=status,
        util=util,
        guidance_before_after=(
            str(trial.get("before_after") or "") or None
        ),
...
def _bending_guidance_item(
    ...
    pack: dict,
    *,
    runtime: BendingGuidanceRuntime | None = None,
) -> dict | None:
    if runtime is not None:
        _bending_item_from_geometry_trial = (
            runtime.bending_item_from_geometry_trial
        )
        _bending_near_limit_specific_title = (
            runtime.bending_near_limit_specific_title
        )
        ...
        _reinforcement_options_remain = (
            runtime.reinforcement_options_remain
        )
    else:
        namespace = globals()
        _bending_item_from_geometry_trial = namespace[
            "_bending_item_from_geometry_trial"
        ]
        _bending_near_limit_specific_title = namespace[
            "_bending_near_limit_specific_title"
        ]
        ...
        _reinforcement_options_remain = namespace[
            "_reinforcement_options_remain"
        ]
...
__all__ = [
    "BendingGeometryTrialRuntime",
    "BendingGuidanceRuntime",
