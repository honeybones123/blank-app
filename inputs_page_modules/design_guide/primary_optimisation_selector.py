...
class PrimaryOptimisationSelectorRuntime:
    ...
    optimisation_candidate_family: Callable[..., Any]
    resolved_efficiency_target_band: Callable[..., Any]
    state_with_overrides: Callable[..., Any]
    updates_match_state: Callable[..., Any]


def _bind_primary_selector_runtime(
    runtime: PrimaryOptimisationSelectorRuntime,
) -> None:
    globals().update(
        {
            "TARGET_BAND_EPS": runtime.target_band_eps,
            "_build_candidate_search_evidence": (
                runtime.build_candidate_search_evidence
            ),
            "_build_design_actions_context": (
            ...
            "_family_ladder_guidance_item": (
                runtime.family_ladder_guidance_item
            ),
            ...
            "_optimisation_candidate_family": (
                runtime.optimisation_candidate_family
            ),
            "_resolved_efficiency_target_band": (
                runtime.resolved_efficiency_target_band
            ),
            "_state_with_overrides": runtime.state_with_overrides,
...
def _select_primary_optimisation_candidate(
    *,
    ...
    runtime: PrimaryOptimisationSelectorRuntime | None = None,
) -> dict:
    if runtime is not None:
        _bind_primary_selector_runtime(runtime)
    selector_debug = {
    ...
    if (
        ...
        direct_item = _family_ladder_guidance_item(
            state,
            overview,
            mode_cfg,
            strengthening=False,
