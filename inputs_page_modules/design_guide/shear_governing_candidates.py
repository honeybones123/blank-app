...
class ShearGoverningCandidateRuntime:
    generate_escalated_shear_states: Callable[..., Any]
    guidance_state_snapshot: Callable[..., Any]
    one_click_diff_accumulated_updates: Callable[..., Any]
    shear_severity_band: Callable[..., Any]
...
def _generate_shear_governing_candidates(
    ...
    mode_config: dict,
    *,
    runtime: ShearGoverningCandidateRuntime | None = None,
) -> tuple[list[dict], dict]:
    """Shear-governing one-click orchestration with staged severity escalation."""
    if runtime is not None:
        _generate_escalated_shear_states = (
            runtime.generate_escalated_shear_states
        )
        _guidance_state_snapshot = runtime.guidance_state_snapshot
        _one_click_diff_accumulated_updates = (
            runtime.one_click_diff_accumulated_updates
        )
        _shear_severity_band = runtime.shear_severity_band
    else:
        namespace = globals()
        _generate_escalated_shear_states = namespace[
            "_generate_escalated_shear_states"
        ]
        _guidance_state_snapshot = namespace["_guidance_state_snapshot"]
        _one_click_diff_accumulated_updates = namespace[
            "_one_click_diff_accumulated_updates"
        ]
        _shear_severity_band = namespace["_shear_severity_band"]
