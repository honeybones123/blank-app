"""Design Guide / auto-design candidate identity helpers."""
...
def _make_auto_design_candidate_key(state: dict) -> tuple:
    ...
    key_parts = [(key, str(state.get(key))) for key in AUTO_DESIGN_CANDIDATE_KEY_TRACKED_FIELDS]
    key_parts.extend(
        [
...
def _candidate_cache_key(state: dict) -> tuple:
    # Reuse the canonical candidate key builder so cache identity stays aligned
    # with all variables already treated as design-governing.
