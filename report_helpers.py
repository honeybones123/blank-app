...
def render_active_beam_report_preview(report_data: dict):
    beam = report_data.get("beam_info", {})
    metadata = report_data.get("metadata", {})
    branding = report_data.get("branding", {})
    checks = report_data.get("checks", {})
...
def build_active_beam_report_pdf(report_data: dict) -> bytes:
    """
    ...
    temp_paths = []
    try:
        from inputs_page import make_beam_3d_figure, make_summary_cross_section_figure

        sec_diag = export_box_diagram_png(
            fig_or_callable=make_summary_cross_section_figure,
        ...
        if (report_data.get("diagrams") or {}).get("include_beam_elevation"):
            beam_diag = export_box_diagram_png(
                fig_or_callable=make_beam_3d_figure,
