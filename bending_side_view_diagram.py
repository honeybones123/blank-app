...
from shear_visuals import (
    ...
    _build_side_view_tension_reo,
    _side_view_display_state,
    _side_view_display_x,
)
    ...
    *,
    ...
    if x_m is None:
        ...
        fig.add_annotation(
            ...
            borderwidth=1,
            font=dict(size=11, color="rgba(90,90,90,1.0)"),
        )
        return None
    ...
    fig.add_shape(
        ...
        x1=xd,
        y1=y1 + 0.08 * D_m,
        line=dict(color="rgba(190, 55, 35, 0.98)", width=2.2, dash="dash"),
    )
    fig.add_annotation(
        x=xd,
        y=y1 + 0.10 * D_m,
        ...
        bordercolor="rgba(190,55,35,0.35)",
        borderwidth=1,
        borderpad=3,
        font=dict(size=11, color="rgba(115,30,20,1.0)"),
        xanchor="center",
    )
    return xd
...
def _make_bending_beam_fill_white(fig: go.Figure) -> None:
    """Keep the Crack side-view outline while using a white beam interior."""
...
def _add_undeformed_beam_reference(fig: go.Figure, *, model: dict[str, Any], D_m: float) -> None:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            line=dict(color="rgba(60,60,60,0.55)", width=1.5, dash="dot"),
            hoverinfo="skip",
            showlegend=False,
        )
    )
...
def _add_creep_secondary_reo(
    ...
    *,
    ...
    ann_start = len(fig.layout.annotations or [])
    ...
    for tr in list(fig.data)[trace_start:]:
        ...
        if line is None:
            continue
        line.color = "rgba(35, 70, 120, 0.38)"
        line.width = min(float(getattr(line, "width", 1.2) or 1.2), 1.4)

    kept = []
    for idx, ann in enumerate(list(fig.layout.annotations or [])):
        text = str(getattr(ann, "text", "") or "").strip().lower()
        if idx >= ann_start and text in {"tension reo", "top reo", "top face"}:
            continue
        kept.append(ann)
    ...
    *,
    ...
    if multiplier <= 1.01:
        fig.add_annotation(
            ...
            borderwidth=1,
            borderpad=3,
            font=dict(size=12, color="rgba(35,35,35,0.98)"),
        )
        return True
    ...
    _style_recent_deflected_beam_trace(
        fig,
        start_idx=before,
        fillcolor="rgba(70, 120, 185, 0.08)",
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=w_i,
            mode="lines",
            line=dict(color="rgba(55,55,55,0.48)", width=1.7, dash="dash"),
            hoverinfo="skip",
            showlegend=False,
        )
    )
    fig.add_trace(
        go.Scatter(
            ...
            y=w_lt,
            mode="lines",
            line=dict(color="rgba(20, 88, 190, 0.96)", width=2.5, dash="dash"),
            hoverinfo="skip",
            showlegend=False,
        )
    )
    ...
    fig.add_annotation(
        ...
        borderwidth=1,
        borderpad=4,
        font=dict(size=14, color="rgba(30,30,30,1.0)"),
    )
    fig.add_annotation(
        ...
        bordercolor="rgba(30,75,150,0.28)",
        borderwidth=1,
        borderpad=4,
        font=dict(size=14, color="rgba(20,75,170,1.0)"),
    )
    ...
    fig.add_annotation(
        ...
        arrowsize=1.18,
        arrowwidth=2.75,
        arrowcolor="rgba(185, 45, 35, 0.98)",
    )
    fig.add_annotation(
        x=xd,
        y=0.5 * (yi + ylt),
        ...
        bordercolor="rgba(185,45,35,0.42)",
        borderwidth=1,
        borderpad=5,
        font=dict(size=16, color="rgba(150,35,28,1.0)"),
    )
    return True
...
def _compression_zone_span_from_stress_figure(
    ...
    *,
    ...
    for shape in list(source_fig.layout.shapes or []):
        ...
        if "red" not in fill and "255,200,200" not in fill and "red" not in line_color:
            continue
    ...
    for trace in source_fig.data:
        ...
        if fill != "toself" and "red" not in fillcolor and "red" not in line_color:
            continue
        raw_y = getattr(trace, "y", None)
...
def _add_immediate_deflection_label(fig: go.Figure, *, defl: dict[str, Any], D_m: float) -> bool:
    ...
    fig.add_annotation(
        ...
        borderwidth=1,
        borderpad=4,
        font=dict(size=12, color="rgba(30,30,30,1.0)"),
    )
    return True
...
def _add_creep_flexural_crack_label(
    ...
    *,
    ...
    color = "rgba(70,70,70,0.72)" if muted else "rgba(45,45,45,0.95)"
    border = "rgba(70,70,70,0.18)" if muted else "rgba(0,0,0,0.14)"
    fig.add_annotation(
