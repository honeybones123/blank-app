...
def _diagram_kind(support_type: str | None) -> str:
    from deflection import _support_props
...
def compute_crack_diagram_deflection_mesh(
    ...
    from deflection import deflected_longitudinal_profile_mm
    ...
    if max_abs_defl <= 1e-15:
        ...
        scale_factor = float(min(max(raw_sf, 1.0), 40.0))
...
def render_crack_moment_tab_plotly() -> None:
    """
    SLS bending moment diagram from shared ``moment_x`` / ``moment_values`` (fallback: ``shear_x`` / ``shear_M_sls_kNm``).

    Ordinate = −M so **sagging (M > 0) is below** the baseline and **hogging (M < 0) is above**, matching the
    Beam Actions & Diagrams sign display.
    """
    ...
    if not isinstance(wx, np.ndarray) or not isinstance(wM, np.ndarray) or wx.size < 2 or wM.size != wx.size:
        ...
        return
    x_plot = np.asarray(wx.tolist(), dtype=float)
    M_adj = np.asarray(wM.tolist(), dtype=float).copy()
    ...
    fig = go.Figure()
    # 1) Baseline (behind fill): visually pinned to supports at y = 0
    fig.add_trace(
        go.Scatter(
    ...
    for sx in sup_x:
        fig.add_vline(x=sx, line_width=1, line_color="rgba(0,0,0,0.06)")
    ...
    # 3) Peak |M| marker (hover only for value)
    try:
    ...
    try:
        from sfd_bmd_page import _add_plotly_support_markers_aligned

        fb = str(get_param("support_type", "simply_supported") or "simply_supported").strip().lower()
        _add_plotly_support_markers_aligned(
            fig,
            support_positions_plot=sup_x,
            support_types_plot=sup_ty,
            y_min=float(y_range[0]),
            y_max=float(y_range[1]),
            L=L,
            support_type_fallback=fb,
        )
    except Exception:
        pass
