...
def render_final_design_guide_card_html(model: FinalDesignGuideCardFormat) -> str:
    ...
    show_status_section = bool(reason_rows) and not _status_rows_repeat_collapsed_card_text(model, reason_rows)
    other_sections = [
        section
        for section in model.sections
        if section.visible
        and str(section.title or "").strip() not in {"Current", "Preview after proposed change", "Status"}
    ]
    other_section_html = []
    ...
    return "".join(
        [
            ...
            preview_section,
            (
                "<div class='dg-section-title'>Status</div>"
                f"<div class='dg-reason-list' data-testid='design-guide-main-explanation'>{_reason_rows_html(reason_rows)}</div>"
