"""Matplotlib torsion prism diagram builders."""
...
def draw_face_label_debug(
    ...
    if show_cracks and n_cracks > 0:
        theta_rad = np.radians(theta_deg * 45.0)
    ...
    if show_corners:
        ...
        for v, label in zip(vertices, corner_labels):
    ...
    elev = 20.0 + cam_b * 40.0
    azim = 45.0 + cam_a * 60.0
...
def plot_torsion_prism_3d(
    ...
    NOTE: theta_multiplier is the wall angle multiplier (proportional to roof angle).
    """
    fig = draw_face_label_debug(
        ...
        theta_deg=float(theta_multiplier),
    )
    return fig
