...
def minimum_longitudinal_row_count_for_spacing(
    *,
    width: float,
    bar_dia: float,
    cover_side: float,
    ...
    """Return the minimum evenly spaced row count for the shared c/c rule."""
    ...
    try:
        ...
        spacing_limit = float(max_cc_spacing_mm)
    except (TypeError, ValueError):
        return count
    if centre_span <= 0.0 or spacing_limit <= 0.0:
        return count
    required_for_spacing = int(math.ceil(centre_span / spacing_limit)) + 1
