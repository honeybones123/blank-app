"""Data boundary for bending-fail plus shear-overdesign mixed candidate merging."""
...
ALLOWED_BENDING_FAIL_SHEAR_OVERDESIGN_SOURCE_FAMILIES = frozenset(
...
def stable_bending_fail_shear_overdesign_hash(value: Any) -> str:
...
def source_family_allowed(value: str | None) -> bool:
    return str(value or "") in ALLOWED_BENDING_FAIL_SHEAR_OVERDESIGN_SOURCE_FAMILIES
...
def mixed_candidate_state_hash(
    ...
    return stable_bending_fail_shear_overdesign_hash(state)


@dataclass(frozen=True)
class BendingFailShearOverdesignInputs:
    ...
    def input_hash(self) -> str:
        return stable_bending_fail_shear_overdesign_hash(self.to_dict())

    @property
    def selection_boundary_satisfied(self) -> bool:
        return self.selected_family_id == "BENDING_FAIL_SHEAR_OVERDESIGN_GOVERNS"
...
class MixedSourceCandidate:
    ...
    def update_hash(self) -> str:
        return stable_bending_fail_shear_overdesign_hash(self.updates)
...
class MixedMergedCandidate:
    ...
    updates: dict[str, Any] = field(default_factory=dict)
    merge_rule_id: str = "PAIRWISE_BENDING_FAIL_SHEAR_OVERDESIGN_MERGE"
    ...
    @property
    def has_mandatory_bending_source(self) -> bool:
        return "BENDING_FAIL_GOVERNS" in self.source_families

    @property
    def has_opportunistic_shear_source(self) -> bool:
        return "SHEAR_OVERDESIGN_GOVERNS" in self.source_families
...
class MixedCandidateEvaluation:
    ...
    bending_repaired: bool | None = None
    ...
    def with_evaluation_hash(self) -> "MixedCandidateEvaluation":
        ...
        return MixedCandidateEvaluation(
            **{
                **payload,
                "evaluation_hash": stable_bending_fail_shear_overdesign_hash(payload),
            }
        )


__all__ = [
    "ALLOWED_BENDING_FAIL_SHEAR_OVERDESIGN_SOURCE_FAMILIES",
