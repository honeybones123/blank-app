"""Contract-owned selection of the family ladder entry point.

This module owns *routing only*.  It deliberately does not generate candidates,
evaluate engineering results, publish cards, or mutate session state.  Those
operations remain with the selected locked family runtime.
"""
...
LADDER_METHOD_BY_FAMILY: dict[str, str | None] = {
    ...
    # Mixed families consume candidates from their two source-family ladders
    # and own merge/ranking only; their contracts prohibit duplicating either
    # source ladder internally.
    ...
    # Minimum-reinforcement governing states are exact-stop families. Their
    # family contracts disable reduction rather than inventing repair search.
...
TERMINAL_FAMILIES = frozenset(
    family_id
    for family_id, method_name in LADDER_METHOD_BY_FAMILY.items()
...
class FamilyLadderDispatchDecision:
    ...
    dispatch_owner: str = "design_brain.family_ladder_dispatch"
...
def resolve_family_ladder_dispatch(
    ...
    *,
    strategy_lookup: Callable[[str], Any] = family_strategy_for,
) -> FamilyLadderDispatchDecision:
    """Resolve the only legal first search route for a classified design.

    Legacy fallback is intentionally fail-closed: it is allowed only while a
    classified non-terminal family has no callable contracted ladder entry
    point.  A family ladder returning no candidate is *not* permission to use
    the legacy engine; that decision belongs to the family's exact-stop or
    no-valid-repair contract.
    """

    classified = dict(classification or {})
    selected_raw = str(
        classified.get("selected_family_id")
        or classified.get("selected_family")
        or ""
    ).strip()
    ...
    method_available = bool(
        ...
        and callable(getattr(strategy, ladder_method, None))
    )
    should_run = bool(passed and not terminal and method_available)
    candidate_contract_id = (
        f"{selected}.{ladder_method or 'terminal_no_candidates'}.v1"
        if passed
        else None
    )
    generation_policy_id = (
        "terminal_no_candidates.v1"
        if terminal
        else ("ordered_family_contract.v1" if passed else None)
    )
    evaluation_policy_id = (
        "terminal_no_evaluation.v1"
        if terminal
        else ("authoritative_engineering_executor.v1" if passed else None)
    )
    selection_policy_id = (
        "terminal_typed_outcome.v1"
        if terminal
        else ("family_contract_rank_key.v1" if passed else None)
    )
