"""Restamp a selected guidance item from whole-beam family evidence.

This is a pure pre-publication transformation. It does not render, mutate
session state, evaluate engineering candidates, or execute Apply actions.
"""
...
from design_brain.family_classification_runtime import (
...
def restamp_primary_guidance_family_from_whole_beam(
    ...
    """Return guidance items/debug with an eligible mixed family restamped."""
    ...
    def not_applicable(domain: str) -> bool:
        ...
        return status in {"CAPACITY", "INFO", "NOT RUN", "NOT_RUN", "NOT SUPPLIED"} or (
            util(domain) is None and not status
        )
    ...
    whole_beam = {
        ...
        if not_applicable("bending")
        else "OVERDESIGNED"
        if bending_util is not None and bending_util < target_low
        else "TARGET",
        "shear_state": "FAIL"
        if statuses.get("shear") == "FAIL"
        else "TARGET"
        if not_applicable("shear")
        else "OVERDESIGNED"
        if shear_util is not None and shear_util < target_low
        else "TARGET",
        ...
        "can_optimise_shear_without_hurting_bending": bool(
            not not_applicable("shear")
            and shear_util is not None
            and shear_util < target_low
        ),
        "can_optimise_bending_without_hurting_shear": bool(
            not not_applicable("bending")
            and bending_util is not None
            and bending_util < target_low
        ),
    }
    ...
    item["family_selection_source"] = (
        "design_brain.whole_beam_family_restamp."
