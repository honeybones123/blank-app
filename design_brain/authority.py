"""Authoritative Design Brain state and result models.

This module is intentionally pure. It does not calculate candidates, publish
cards, render UI, read session state, or apply updates. It defines the stable
identity boundary that live compute, publication, rendering, CTA, and Apply
must converge on during production certification.
"""
...
class EngineeringInputSnapshot:
    """Complete committed engineering input state.

    The fields are deliberately engineering-only. UI presentation state must
    stay outside this model so it cannot perturb calculation identity.
    """
...
class AuthoritativeDesignResult:
    """Single immutable result that downstream Design Guide surfaces consume."""
    ...
    def with_publication_authority_hash(self) -> "AuthoritativeDesignResult":
        return replace(
            self,
            publication_authority_hash=stable_authority_hash(self.publication_authority_payload()),
        )
...
def build_authoritative_design_result(
    *,
    ...
    result = AuthoritativeDesignResult(
        ...
        selected_candidate=dict(selected_candidate) if selected_candidate is not None else None,
        selected_candidate_absence=(
            dict(selected_candidate_absence) if selected_candidate_absence is not None else None
        ),
