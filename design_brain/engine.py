...
def _build_presentation(context: dict, display_truth: dict, button_contract: dict) -> dict:
    ...
    family_tokens = {
        ...
        for value in (
            ...
            button_contract.get("published_family_id"),
            button_contract.get("cta_family_id"),
            button_contract.get("apply_payload_family_id"),
            context.get("selected_family_id"),
            context.get("published_family_id"),
            context.get("cta_family_id"),
            context.get("candidate_family_id"),
            context.get("card_family_id"),
            candidate_search_evidence.get("selected_family_id"),
            candidate_search_evidence.get("published_family_id"),
            candidate_search_evidence.get("cta_family_id"),
        )
        if str(value or "").strip()
    }
    geometry_detailing_action = bool(
        "GEOMETRY_DETAILING_GOVERNS" in family_tokens
        or "GEOMETRY_DETAILING" in family_tokens
    )
    if geometry_detailing_action:
        button_label = "Apply geometry correction"
    ...
    if guidance_intent == "required_fix" and geometry_detailing_action:
        return out(
            theme="fail",
            css_bucket="fail",
            use_success_style=False,
            subtext=(
                "A geometry/detailing correction is available before normal engineering "
                "recommendation ladders run."
            ),
            button_theme="fail",
            critical_status="ACTION",
            headline_override="Geometry needs correction",
