...
def _explicit_locked_no_repair_signal_from_publication(
    primary: dict,
    ...
    *,
    ...
    contract_d = _as_dict(contract)
    evidence = _candidate_search_evidence_from_publication(primary, debug_d)
    action_payload = _as_dict(primary.get("action_payload"))
    resolved = _as_dict(primary.get("resolved_candidate"))
    result_evidence = _as_dict(result_d.get("evidence"))
    sources = (
        payload_d,
        primary,
        debug_d,
    ...
    if any(bool(_as_dict(source).get(key)) for source in sources for key in strong_keys):
        return True
    text = _publication_text(primary, debug_d)
    return bool(
        "no valid repair" in text
        or "no legal repair" in text
        or "locked/no valid repair" in text
        or "all_repair_paths_locked" in text
    )
...
def _canonical_cleanup_family_from_publication_updates(
    ...
    intent_text = " ".join(
        ...
        for value in (
            ...
            debug.get("guidance_intent"),
            _as_dict(debug.get("presentation")).get("guidance_intent"),
            publication_text,
        )
    ).lower()
    ...
    if cleanup_publication:
        mechanical = _normalise_failure_family(family_id)
...
def _raw_state_flags_from_publication(
    ...
    contract = button_contract_from_payload(primary, debug)
    explicit_locked_no_repair_signal = _explicit_locked_no_repair_signal_from_publication(
        primary,
        debug,
        payload=payload,
        result=result,
    ...
    if explicit_locked_no_repair_signal:
        repair_payload = {}
    ...
    locked_repair_blocked = bool((explicit_lock_signal or explicit_locked_no_repair_signal) and not repair_payload)
    raw_flags = {
        ...
        "legal_repair_exists": bool(repair_payload) and not explicit_locked_no_repair_signal,
...
def _repair_action_payload_from_publication(primary: dict, debug: dict, active_failures: list[str]) -> dict:
    if _explicit_locked_no_repair_signal_from_publication(primary, debug):
        return {}
...
def _locked_no_repair_publication_item(primary: dict, diagnostics: dict) -> dict:
    selected = "LOCKED_NO_REPAIR"
    ...
    item.update(
        {
            "title_main": "No legal repair is available",
            "title": "No legal repair is available",
            "headline": "No legal repair is available",
            "governing_label": "No valid repair",
            "summary_line": reason,
            ...
            "card_family_id": selected,
            "apply_payload_family_id": selected,
            "family_selection_source": diagnostics.get("family_selection_source"),
            "family_selection_contract": FAMILY_SELECTION_CONTRACT_ID,
            "family_chooser_contract": FAMILY_CHOOSER_CONTRACT_ID,
            "rejected_families": dict(diagnostics.get("rejected_families") or {}),
...
def enforce_family_selection_publication_contract(payload: dict) -> dict:
    ...
    if selected_family_id == "LOCKED_NO_REPAIR":
        locked_item = _locked_no_repair_publication_item(primary, diagnostics)
        locked_contract = _as_dict(locked_item.get("button_contract"))
        locked_evidence = _as_dict(locked_item.get("candidate_search_evidence"))
        debug.update(
            {
                **diagnostics,
                ...
                "family_match_passed": True,
                "family_match_violation_reason": None,
                "locked_no_repair_publication_route_used": True,
                "candidate_search_evidence": dict(locked_evidence),
                "primary_button_contract": dict(locked_contract),
                "button_contract": dict(locked_contract),
                "button_contract_enabled": False,
            }
        )
        out["guidance_items"] = [locked_item] + items[1:]
    ...
    if not violation:
        ...
        if repair_payload and (not contract_enabled(current_contract) or not current_updates):
            ...
            geometry_detailing_recovery = bool(
                selected_family_id == "GEOMETRY_DETAILING_GOVERNS"
                or str(owner_family or "").strip().lower() == "geometry_detailing"
            )
            if geometry_detailing_recovery:
                owner_family = "geometry_detailing"
                effective_candidate_family_id = "GEOMETRY_DETAILING_GOVERNS"
            ...
            if geometry_detailing_recovery:
                recovered_title = "Geometry needs correction"
            ...
            primary.update(
                {
                    ...
                    "primary_action": (
                        ...
                        if not geometry_detailing_recovery
                        else "Apply geometry correction"
                    ),
                    "selected_family": selected_family_id,
                    "selected_family_id": selected_family_id,
                    "published_family_id": selected_family_id,
                    ...
                    "family_match_passed": True,
                    "family_match_violation_reason": None,
                    "status": "ACTION" if geometry_detailing_recovery else "FAIL",
                    "critical_status": "ACTION" if geometry_detailing_recovery else "FAIL",
                    "guidance_intent": "required_fix",
                    "display_state": "ACTION",
                    "final_state_class": "action",
                    "pill": "ACTION" if geometry_detailing_recovery else "NEXT",
                    "reasoning": (
                        "Increase width so the section satisfies the geometry/detailing contract."
                        if geometry_detailing_recovery
                        else primary.get("reasoning")
                    ),
    ...
    if violation:
        ...
        if repair_payload:
            ...
            if geometry_detailing_recovery:
                governing_label = "Geometry/detailing correction"
            recovered_contract.update(
                {
                    "enabled": True,
                    "actionable": True,
            ...
            recovered.update(
                {
                    ...
                    "summary_line": (
                        "A geometry/detailing correction is available before normal engineering recommendation ladders run."
                        if geometry_detailing_recovery
                        else recovered.get("summary_line")
                    ),
                    "action_type": "apply_resolved_candidate",
                    "updates": dict(repair_payload.get("updates") or {}),
                    ...
                    "primary_action": (
                        ...
                        if geometry_detailing_recovery
                        else recovered.get("primary_action") or "Run one-click auto design"
                    ),
                    "guidance_intent": "required_fix",
                    "status": "ACTION" if geometry_detailing_recovery else recovered.get("status"),
                    "critical_status": "ACTION" if geometry_detailing_recovery else recovered.get("critical_status"),
                    "pill": "ACTION" if geometry_detailing_recovery else recovered.get("pill"),
                    "reasoning": (
                        "Increase width so the section satisfies the geometry/detailing contract."
                        if geometry_detailing_recovery
                        else recovered.get("reasoning")
                    ),
                    "selected_family": selected_family_id,
                    "selection_reason": chooser.get("selection_reason"),
                    "selected_family_reason": chooser.get("selected_family_reason"),
