...
def build_final_design_guide_cta(
    *,
    ...
    return FinalDesignGuideCTA(
        enabled=bool(contract.get("enabled") or contract.get("actionable") or derived_executor_enabled),
        actionable=bool(contract.get("actionable") or derived_executor_enabled),
...
def build_final_design_guide_direct_shell_card_projection(
    *,
    ...
    active_strength_shell = bool((active_failure_keys & {"bending", "shear"}) or family_owned_strength_repair_shell)
    ...
    anchor_bucket = "fail" if active_strength_shell else "efficiency"
...
def _guard_final_publication_cta_identity(
    ...
    if selected_family and not _text(payload.get("family")):
        payload["family"] = selected_family
        summary = _mapping(payload.get("apply_payload_summary"))
        if not _text(summary.get("family")):
    ...
    if (
        ...
        or terminal_state in _FINAL_PUBLICATION_TERMINAL_NO_ACTION_STATES
    ):
...
def build_final_visible_contract_binding_no_second_cta_result(
    *,
    ...
    if expected_util is not None and float(expected_util) < threshold:
        if bool(evidence_d.get("no_second_cta_required")) and target_band_count <= 0:
            applies = True
