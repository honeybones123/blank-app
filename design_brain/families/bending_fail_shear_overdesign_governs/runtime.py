...
def _merged_candidates(inputs: BendingFailShearOverdesignInputs) -> tuple[MixedMergedCandidate, ...]:
    ...
    # Mandatory safety repair is an ordered ladder. Keep those candidates
    # contiguous so evaluation can stop as soon as the beam passes.
    for bend in bending:
        merged.append(
            MixedMergedCandidate(
    ...
    # Optional shear cleanup is lower priority; do not interleave it with and
    # multiply every mandatory repair step before a passing beam is found.
    for bend in bending:
...
def run_bending_fail_shear_overdesign_runtime(
    *,
    ...
    exhausted = {
        ...
        "all_bending_repair_candidates_attempted": selected is None,
        "all_shear_optimisation_candidates_attempted": selected is None,
        "all_approved_merge_candidates_attempted": selected is None,
