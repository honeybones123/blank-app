...
def load_bending_fail_shear_overdesign_governs_contract() -> dict[str, Any]:
    with CONTRACT_PATH.open("r", encoding="utf-8") as handle:
        data = json.load(handle)
    if not isinstance(data, dict):
        raise ValueError("BENDING_FAIL_SHEAR_OVERDESIGN_GOVERNS contract must be a JSON object")
    return data


def _section(name: str) -> dict[str, Any]:
    return dict(load_bending_fail_shear_overdesign_governs_contract().get(name) or {})
...
def shear_protection() -> dict[str, Any]:
    return _section("shear_protection")
...
def shared_exclusions() -> tuple[str, ...]:
    return tuple(str(value) for value in load_bending_fail_shear_overdesign_governs_contract().get("shared_exclusions") or ())
...
def contract_hash() -> str:
    payload = {
        ...
        "shear_protection": shear_protection(),
