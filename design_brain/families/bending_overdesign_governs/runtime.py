...
def _candidate_updates_from_contract(base_state: dict[str, Any]) -> tuple[dict[str, Any], ...]:
    ...
    def _bottom_update_reduces_reinforcement(
        candidate_update: dict[str, Any],
    ) -> bool:
        """Keep the reduction lane strictly lighter than the current row.

        The contract sequence is an ordering example, not proof that every
        listed arrangement is lighter than every possible live starting
        state. This prevents a small arrangement such as 4-N16 from being
        "cleaned up" to 4-N20 or 5-N24.
        """

        if current_bottom_area_key <= 0.0:
            return True
        try:
            candidate_area_key = float(
                candidate_update.get("bot1_count") or 0
            ) * float(candidate_update.get("db_bot_1") or 0) ** 2
        except (TypeError, ValueError):
            return False
        return candidate_area_key < current_bottom_area_key - 1e-9
    ...
    for sequence_item in bottom_sequence:
        ...
        if (
            _bottom_update_is_current(candidate_update)
            or not _bottom_update_reduces_reinforcement(candidate_update)
        ):
            continue
        updates.append(
            {
    ...
    if base_width is not None:
        for reduced_width in _descending_values(base_width, width_increment, min_width):
            ...
            for sequence_item in bottom_sequence:
                ...
                if (
                    _bottom_update_is_current(bottom_update)
                    or not _bottom_update_reduces_reinforcement(bottom_update)
                ):
                    continue
                updates.append(
                    {
