...
def _candidate_updates_for_lane(
    *,
                    ...
                    _candidate(
                        ...
                        update={
                            "reinforcement": {
                                ...
                                "ligature_diameter_mm": diameter,
                                "ligature_spacing_mm": spacing,
                            }
                        },
    ...
    if lane_id == "DEPTH_INCREASE_RESTART_REINFORCEMENT_SEARCH":
        ...
        for step in range(1, int(max_depth_steps) + 1):
            depth = base_depth + increment * step
            for label, diameter in _base_bar_sizes(lane_map):
                for spacing in _base_spacing_values(lane_map):
                    candidates.append(
                        _candidate(
                            lane_id=lane_id,
                            ordinal=len(candidates) + 1,
    ...
    if lane_id == "WIDTH_INCREASE_RESTART_REINFORCEMENT_SEARCH":
        ...
        for step in range(1, int(max_width_steps) + 1):
            width = base_width + increment * step
            for label, diameter in _base_bar_sizes(lane_map):
                for spacing in _base_spacing_values(lane_map):
                    candidates.append(
                        _candidate(
    ...
    *,
    ...
    for lane in sorted(internal_strategy_lanes(), key=lambda item: int(item.get("lane_index") or 0)):
        ...
        if lane_id == "EXHAUSTED":
            exhausted_reason = "all_contract_strategies_attempted"
            trace.append(
                {
                    "lane_id": lane_id,
                    "lane_index": int(lane.get("lane_index") or 0),
