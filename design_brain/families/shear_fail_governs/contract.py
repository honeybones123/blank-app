...
def expected_ladder_snapshots() -> dict[str, dict[str, Any]]:
    ...
    for key, value in snapshots.items():
        if not isinstance(value, dict):
            continue
        snapshot = dict(value)
        width_progression = snapshot.pop("width_progression", None)
        if isinstance(width_progression, dict):
            start = float(width_progression["start"])
            stop = float(width_progression["stop"])
            step = float(width_progression["step"])
            if step <= 0.0 or stop < start:
                raise ValueError("width_progression must have positive step and stop >= start")
            count = int(round((stop - start) / step))
            widths = [start + step * index for index in range(count + 1)]
            if not widths or abs(widths[-1] - stop) > 1e-9:
                raise ValueError("width_progression must terminate exactly at stop")
            snapshot["widths_tried"] = widths
        expanded[str(key)] = snapshot
    return expanded
