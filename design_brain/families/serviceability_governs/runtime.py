        ...
        for increment in (1, 2):
            candidates.append(
                _candidate(
                    ...
                    update={"reinforcement": {"bottom_bar_count": bottom_count + increment}},
                )
            )
        return tuple(candidates)

    if lane_id == "DEPTH_INCREASE_RESTART_REINFORCEMENT_SEARCH":
        for step in range(1, max_geometry_steps + 1):
            for increment in (0, 1, 2):
                candidates.append(
                    _candidate(
                        ...
                        ordinal=len(candidates) + 1,
                        update={
                            "geometry": {"beam_depth_mm": depth + 25.0 * step},
                            "reinforcement": {"bottom_bar_count": bottom_count + increment},
                        },
            ...
            for increment in (0, 1, 2):
                candidates.append(
                    _candidate(
                        ...
                        ordinal=len(candidates) + 1,
                        update={
                            "geometry": {"beam_width_mm": width + 25.0 * step},
                            "reinforcement": {"bottom_bar_count": bottom_count + increment},
                        },
                        restart_lanes=("BOTTOM_REINFORCEMENT_INCREASE",),
                    )
        ...
        for step in range(1, max_geometry_steps + 1):
            candidates.append(
                _candidate(
                    ...
                    update={
                        "geometry": {
                            "beam_depth_mm": depth + 25.0 * step,
                            "beam_width_mm": width + 25.0 * step,
                        },
                        "reinforcement": {"bottom_bar_count": bottom_count + step},
                    },
                )
            )
        return tuple(candidates)
    ...
    *,
    ...
    for index, lane_id in enumerate(lane_order, start=1):
        ...
        if lane_id == "EXHAUSTED":
            blockers = _specific_blockers(base_state, candidate_repairs)
