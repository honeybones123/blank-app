...
CONTRACT_RUNTIME_DEPTH_SCAN_STEPS_MM = tuple(float(step) for step in range(25, 551, 25))
...
def _top_row_spacing_updates(
    ...
    *,
    ...
    """Preserve the shared longitudinal spacing rule after a width increase."""

    base_count = max(
        2,
        _as_int(
            base.get("top_row_1_bars"),
            _as_int(base.get("top1_count"), 2),
        ),
    )
    base_dia = max(
        10,
        _as_int(
            ...
            _as_int(base.get("db_top_1"), 10),
        ),
    )
    required_count = minimum_longitudinal_row_count_for_spacing(
        width=float(width),
        bar_dia=float(base_dia),
        cover_side=float(cover_side),
        lig_d=float(lig_d),
        minimum_count=int(base_count),
    )
    if required_count <= base_count:
        return {}
    return {
        "top1_count": int(required_count),
        "top_row_1_bars": int(required_count),
    }
...
def _order_and_reindex_bending_specs(
    specs: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Apply the family contract's reinforcement-first progression."""

    phase_by_lane = {
        "SINGLE_LAYER_BOTTOM_REO": 0,
        "LARGER_BAR": 1,
        "MULTI_LAYER_REO": 2,
        "DEPTH_INCREASE": 3,
        "WIDTH_INCREASE": 4,
    ...
    def _phase(spec: dict[str, Any]) -> int:
        stage_name = str(spec.get("stage_name") or "").lower()
        if spec.get("reinforcement_retry_after_width") or "after_width" in stage_name:
            return 5
        if "rescue" in stage_name:
            return 6
        return phase_by_lane.get(
            str(spec.get("contract_runtime_lane_id") or ""),
            7,
        )

    def _sort_key(row: tuple[int, dict[str, Any]]) -> tuple:
        original_index, spec = row
        phase = _phase(spec)
        if phase >= 5:
            return (
                phase,
                _as_float(spec.get("b"), float("inf")),
                phase_by_lane.get(
                    str(spec.get("contract_runtime_lane_id") or ""),
                    7,
                ),
        ...
        return (phase, 0.0, 0, original_index)

    ordered = [
        dict(spec)
        for _, spec in sorted(enumerate(specs), key=_sort_key)
    ]
    for index, spec in enumerate(ordered, start=1):
        spec["ladder_index"] = index
        strategy = str(spec.get("strategy") or "contract ladder candidate")
        spec["label"] = (
            f"BENDING_FAIL_GOVERNS contract runtime {index}: {strategy}"
        )
        spec["bending_ladder_phase"] = _phase(spec)
    return ordered
...
def _canonical_unlocked_geometry_continuation_specs(
    specs: list[dict[str, Any]],
    *,
    base: dict[str, Any],
    width_key: str,
    ...
    """Carry the first legal multi-layer repair to the project geometry limit."""

    reinforcement_rows = [
        dict(spec)
        for spec in specs
        if any(
            key in _as_dict(spec.get("updates"))
            for key in (
                "bot1_count",
                "bot_row_1_bars",
                "db_bot_1",
    ...
    template = next(
        (
            spec
            for spec in reinforcement_rows
            if int(
                _as_dict(spec.get("updates")).get("bot2_count")
                or _as_dict(spec.get("updates")).get("bot_row_2_bars")
                or 0
            )
            > 0
        ),
        (reinforcement_rows[0] if reinforcement_rows else {}),
    )
    carried = {
        key: value
        for key, value in _as_dict(template.get("updates")).items()
        if key not in {"D", "b", "bw", width_key}
    }
    top_count = max(
        2,
        _as_int(base.get("top1_count"), _as_int(base.get("top_row_1_bars"), 2)),
    )
    top_dia = max(
        10,
        _as_int(base.get("db_top_1"), _as_int(base.get("top_row_1_dia"), 10)),
    )
    row1_count = max(
        2,
        _as_int(
            carried.get("bot1_count"),
            _as_int(carried.get("bot_row_1_bars"), _as_int(base.get("bot1_count"), 2)),
        ),
    )
    row1_dia = max(
        10,
        _as_int(
            carried.get("db_bot_1"),
            _as_int(carried.get("bot_row_1_dia"), _as_int(base.get("db_bot_1"), 10)),
        ),
    )
    row2_count = max(
        0,
        _as_int(carried.get("bot2_count"), _as_int(carried.get("bot_row_2_bars"), 0)),
    )
    row2_dia = max(
        10,
        _as_int(
            carried.get("db_bot_2"),
            _as_int(carried.get("bot_row_2_dia"), row1_dia),
        ),
    )
    continuation: list[dict[str, Any]] = []
    geometry_pairs: list[tuple[float, float]] = []
    for depth in project_depth_values(base_depth):
        width = max(
            float(base_width),
            math.ceil((float(depth) / maximum_depth_width_ratio) / 50.0) * 50.0,
        )
    ...
    for width, depth in geometry_pairs:
        required_top_count = minimum_longitudinal_row_count_for_spacing(
            width=width,
            bar_dia=float(top_dia),
            cover_side=float(cover_side),
            lig_d=float(lig_d),
            minimum_count=int(top_count),
        )
        required_row1_count = minimum_longitudinal_row_count_for_spacing(
            width=width,
            bar_dia=float(row1_dia),
            cover_side=float(cover_side),
            lig_d=float(lig_d),
            minimum_count=int(row1_count),
        )
        required_row2_count = (
            minimum_longitudinal_row_count_for_spacing(
                width=width,
                bar_dia=float(row2_dia),
                cover_side=float(cover_side),
                lig_d=float(lig_d),
                minimum_count=int(row2_count),
            )
            if row2_count > 0
            else 0
        )
        updates = {
            **carried,
            "D": float(depth),
            **(_width_update(width_key, width) if width > base_width + 1e-9 else {}),
            "top1_count": int(required_top_count),
            "top_row_1_bars": int(required_top_count),
            "bot1_count": int(required_row1_count),
            "bot_row_1_bars": int(required_row1_count),
        }
        if row2_count > 0:
            updates.update(
                {
                    "bot2_count": int(required_row2_count),
                    "bot_row_2_bars": int(required_row2_count),
                }
            )
        continuation.append(
            {
                **template,
                ...
                "strategy": (
                    f"retain the legal bending reinforcement repair and increase "
                    f"geometry to {width:.0f} x {depth:.0f} mm"
                ),
                "candidate_id": (
                    f"bending_fail_unlocked_geometry_{int(round(width))}x"
                    f"{int(round(depth))}"
                ),
...
def _contract_runtime_candidate_updates(
    *,
    ...
    updates: dict[str, dict[str, Any]] = {
        ...
        "WIDTH_INCREASE": {},
        "MULTI_LAYER_REO": _bottom_updates(
            row1_count=split_row1,
            row2_count=split_row2,
            dia=next_dia,
        ),
        "EXACT_STOP": {},
        "NO_VALID_STRATEGY": {},
...
class BendingFailFamily(DiagnosticFamilyStrategy):
    ...
    def contracted_repair_ladder_specs(
        ...
        *,
        ...
        def _append_approved_diameter_candidate(
            *,
            lane_id: str,
            candidate_dia: int,
            ...
            """Append one contract-approved deterministic diameter candidate."""

            nonlocal runtime_index
            meta = CONTRACT_RUNTIME_LANE_SPEC_META[lane_id]
            if lane_id == "LARGER_BAR":
                reinforcement_updates = _bottom_updates(
                    row1_count=base_count,
                    dia=candidate_dia,
                )
            else:
                total_count = max(4, int(base_count + 1))
                row1_count = int(math.ceil(total_count / 2.0))
                row2_count = int(total_count - row1_count)
                reinforcement_updates = _bottom_updates(
            ...
            candidate_updates = {
                **(
                    _width_update(width_key, candidate_width)
                    if after_width
                    else {}
                ),
                **reinforcement_updates,
                **(
                    _top_row_spacing_updates(
                        base,
            ...
            diff = _normalised_update_diff(base, candidate_updates)
            if not diff:
                return
            (
                layout_width,
            ...
            if (
                _depth_width_ratio_blocked(
                    layout_width,
                    layout_depth,
                    maximum_depth_width_ratio,
                )
                or not bool(spacing_rule.get("valid"))
                or clear < MIN_BOTTOM_CLEAR_SPACING_MM - 1e-9
            ):
                return
            runtime_index += 1
            suffix = "_after_width" if after_width else ""
            stage_name = f"{meta['stage_name']}_diameter_{candidate_dia}{suffix}"
            strategy = (
                f"{meta['strategy']} using the approved N{candidate_dia} "
                "diameter progression"
                + (
                    f" after width increase to {candidate_width:.0f} mm"
                    if after_width
                    else ""
                )
            )
            decision = decide_bending_fail_repair_ladder_add(
                ...
                assigned_candidate_index=runtime_index,
                assigned_label=(
                    "BENDING_FAIL_GOVERNS approved diameter catalogue "
                    f"{runtime_index}: {strategy}"
                ),
                escalation=(
                    "retry_approved_diameter_after_width"
                    if after_width
                    else "approved_diameter_progression"
                ),
                width=layout_width,
                depth=layout_depth,
                row1=row1,
            ...
            if not (
                decision.should_append_spec
                and decision.spec_payload is not None
            ):
                return
            update = BeamCandidateUpdate(updates=candidate_updates)
            evaluation = _contract_runtime_evaluator(
            ...
            runtime_specs.append(
                {
                    **decision.spec_payload,
                    ...
                    "selected_strategy_lane": lane_id,
                    "ladder_hash": runtime_result.ladder_hash,
                    "bending_fail_contract_runtime_ladder_hash": (
                        runtime_result.ladder_hash
                    ),
                    "ladder_trace_evidence": {
                        "lane_id": lane_id,
                        "contract_lane_id": str(lane_id).lower(),
                        "lane_index": runtime_order.index(lane_id),
                        "evaluation_hash": evaluation.evaluation_hash,
                    },
                    "update_hash": update.update_hash,
                    "candidate_state_hash": evaluation.candidate_state_hash,
                    "approved_diameter_progression": True,
                    "reinforcement_retry_after_width": bool(after_width),
                    "candidate_generation_policy_id": (
                        "ordered_family_contract.v1"
                    ),
                }
            )

        # The family contract requires every deterministic larger-bar step to
        # be present before geometry.  The legacy implementation generated only
        # ``next_dia`` and silently skipped N24/N28/N32 and later allowed sizes.
        for candidate_dia in larger_dias[1:]:
            for lane_id in ("LARGER_BAR", "MULTI_LAYER_REO"):
                _append_approved_diameter_candidate(
                    lane_id=lane_id,
                    candidate_dia=int(candidate_dia),
                    candidate_width=float(base_width),
                    after_width=False,
                )

        if not geometry_locked:
            # The family contract requires reinforcement lanes to be retried
            # after width creates enough detailing room.  Previously a blocked
            # reinforcement lane was discarded permanently, leaving only
            # geometry-with-current-reinforcement candidates.
            for lane_id in (
                "SINGLE_LAYER_BOTTOM_REO",
                "LARGER_BAR",
                "MULTI_LAYER_REO",
                ...
                for width_step in DEFAULT_WIDTH_STEPS_MM:
                    width = float(base_width + float(width_step))
                    if lane_id == "SINGLE_LAYER_BOTTOM_REO":
                        row_count = minimum_longitudinal_row_count_for_spacing(
                            width=width,
                            bar_dia=float(base_dia),
                            cover_side=float(cover_side),
                            lig_d=float(lig_d),
                            minimum_count=int(base_count + 1),
                        )
                        reinforcement_updates = _bottom_updates(
                            row1_count=row_count,
                            dia=base_dia,
                        )
                    elif lane_id == "LARGER_BAR":
                        row_count = minimum_longitudinal_row_count_for_spacing(
                            width=width,
                            bar_dia=float(next_dia),
                            cover_side=float(cover_side),
                            lig_d=float(lig_d),
                            minimum_count=int(base_count + 1),
                        )
                        ...
                        reinforcement_updates = _bottom_updates(
                            row1_count=row_count,
                            row2_count=row_count,
                            dia=next_dia,
                        )
                    retry_updates = {
                        **_width_update(width_key, width),
                        **reinforcement_updates,
                        **_top_row_spacing_updates(
                            base,
                            width=width,
                    ...
                    diff = _normalised_update_diff(base, retry_updates)
                    if not diff:
                        continue
                    (
                        layout_width,
                    ...
                    if not bool(spacing_rule.get("valid")):
                        continue
                    decision = decide_bending_fail_repair_ladder_add(
                        step=int(runtime_order.index(lane_id)),
                        stage_name=f"{meta['stage_name']}_after_width",
                        strategy=(
                            f"{meta['strategy']} retried after width increase "
                            f"to {width:.0f} mm"
                        ),
                        updates=retry_updates,
                        diff=diff,
                        spacing_blocked=(
                            clear < MIN_BOTTOM_CLEAR_SPACING_MM - 1e-9
                        ),
                        assigned_candidate_index=runtime_index + 1,
                        assigned_label=(
                            f"BENDING_FAIL_GOVERNS contract runtime "
                            f"{runtime_index + 1}: {meta['strategy']} after width"
                        ),
                        escalation="retry_reinforcement_after_width",
                        width=layout_width,
                        depth=layout_depth,
                        row1=row1,
                        row2=row2,
                    ...
                    if (
                        ...
                        runtime_index += 1
                        retry_update = BeamCandidateUpdate(
                            updates=retry_updates
                        )
                        retry_evaluation = _contract_runtime_evaluator(
                            BeamCandidateInput(base_state=base),
                            retry_update,
                        )
                        runtime_specs.append(
                            {
                                **decision.spec_payload,
                                ...
                                "ladder_trace_evidence": {
                                    ...
                                    "lane_index": runtime_order.index(
                                        lane_id
                                    ),
                                    "evaluation_hash": (
                                        retry_evaluation.evaluation_hash
                                    ),
                                },
                                "update_hash": retry_update.update_hash,
                                "candidate_state_hash": (
                                    retry_evaluation.candidate_state_hash
                                ),
                                "reinforcement_retry_after_width": True,
                            }
                        )
            for candidate_dia in larger_dias[1:]:
                # Restart the full diameter catalogue at the nearest approved
                # width increment.  Later width/depth lanes carry that legal
                # reinforcement state forward, avoiding a Cartesian product
                # that would add no new contract decision.
                candidate_width = float(
                    base_width + DEFAULT_WIDTH_STEPS_MM[0]
                )
                for lane_id in ("LARGER_BAR", "MULTI_LAYER_REO"):
                    _append_approved_diameter_candidate(
                        lane_id=lane_id,
        ...
        runtime_depth_steps = list(CONTRACT_RUNTIME_DEPTH_SCAN_STEPS_MM)

        if not geometry_locked:
            for depth_step in CONTRACT_RUNTIME_DEPTH_SCAN_STEPS_MM:
                depth = float(base_depth + depth_step)
                depth_updates = {"D": depth}
                diff = _normalised_update_diff(base, depth_updates)
                if not diff:
                    continue
                ...
                stage_name = "contract_runtime_depth_increment_scan"
                strategy = f"contract runtime depth scan to {depth:.0f} mm with current bottom reinforcement"
                if _depth_width_ratio_blocked(layout_width, layout_depth, maximum_depth_width_ratio):
                    ...
                    continue
                longitudinal_spacing_rule = resolve_longitudinal_bar_spacing_rule(base, depth_updates)
                if not bool(longitudinal_spacing_rule.get("valid")):
                    ...
                    continue
                runtime_index += 1
                depth_update = BeamCandidateUpdate(updates=depth_updates)
                depth_evaluation = _contract_runtime_evaluator(
                    BeamCandidateInput(base_state=base),
                    depth_update,
                )
                decision = decide_bending_fail_repair_ladder_add(
                    step=int(runtime_order.index("DEPTH_INCREASE")),
                    stage_name=stage_name,
                    strategy=strategy,
                    ...
                    assigned_candidate_index=runtime_index,
                    assigned_label=(
                        f"BENDING_FAIL_GOVERNS contract runtime {runtime_index}: "
                        "contract runtime depth increment scan"
                    ),
                    escalation="contract_runtime_depth_increase",
                    width=layout_width,
                    depth=layout_depth,
                    ...
                    runtime_specs.append(
                        {
                            **decision.spec_payload,
                            "contract_runtime_authority": "run_bending_fail_governs_ladder_runtime",
                            "contract_runtime_lane_id": "DEPTH_INCREASE",
                            "selected_strategy_lane": "DEPTH_INCREASE",
                            "ladder_hash": runtime_result.ladder_hash,
                            "bending_fail_contract_runtime_ladder_hash": runtime_result.ladder_hash,
                            "ladder_trace_evidence": {
                                "lane_id": "DEPTH_INCREASE",
                                "contract_lane_id": "depth_increase",
                                "lane_index": runtime_order.index("DEPTH_INCREASE"),
                                "evaluation_hash": depth_evaluation.evaluation_hash,
                            },
                            "update_hash": depth_update.update_hash,
                            "candidate_state_hash": depth_evaluation.candidate_state_hash,
                            "depth_increment_scan_candidate": True,
                        }
                    )
            ...
            for width_step in DEFAULT_WIDTH_STEPS_MM:
                ...
                for depth_step in CONTRACT_RUNTIME_DEPTH_SCAN_STEPS_MM:
                    depth = float(base_depth + depth_step)
                    width_depth_updates = {**width_updates, "D": depth}
                    diff = _normalised_update_diff(base, width_depth_updates)
                    if not diff:
                        continue
                    (
                        layout_width,
                    ...
                    stage_name = "contract_runtime_width_depth_increment_scan"
                    strategy = (
                        f"contract runtime width/depth scan to {width:.0f} x {depth:.0f} mm "
                        "with current bottom reinforcement"
                    )
                    if _depth_width_ratio_blocked(layout_width, layout_depth, maximum_depth_width_ratio):
                        runtime_known_bad.append(
                            build_bending_fail_known_bad_depth_width_record(
                        ...
                        continue
                    longitudinal_spacing_rule = resolve_longitudinal_bar_spacing_rule(
                        base,
                        width_depth_updates,
                    )
                    if not bool(longitudinal_spacing_rule.get("valid")):
                        ...
                        continue
                    runtime_index += 1
                    width_depth_update = BeamCandidateUpdate(updates=width_depth_updates)
                    width_depth_evaluation = _contract_runtime_evaluator(
                        BeamCandidateInput(base_state=base),
                        width_depth_update,
                    )
                    decision = decide_bending_fail_repair_ladder_add(
                        step=int(runtime_order.index("WIDTH_INCREASE")),
                        stage_name=stage_name,
                        strategy=strategy,
                        updates=width_depth_updates,
                        diff=diff,
                        spacing_blocked=False,
                        assigned_candidate_index=runtime_index,
                        assigned_label=(
                            f"BENDING_FAIL_GOVERNS contract runtime {runtime_index}: "
                            "contract runtime width/depth increment scan"
                        ),
                        escalation="bounded_geometry_repair",
                        width=layout_width,
                        depth=layout_depth,
                        row1=row1,
                        row2=row2,
                        ...
                        runtime_specs.append(
                            {
                                **decision.spec_payload,
                                "contract_runtime_authority": "run_bending_fail_governs_ladder_runtime",
                                "contract_runtime_lane_id": "WIDTH_INCREASE",
                                "selected_strategy_lane": "WIDTH_INCREASE",
                                "ladder_hash": runtime_result.ladder_hash,
                                "bending_fail_contract_runtime_ladder_hash": runtime_result.ladder_hash,
                                "ladder_trace_evidence": {
                                    "lane_id": "WIDTH_INCREASE",
                                    "contract_lane_id": "width_increase",
                                    "lane_index": runtime_order.index("WIDTH_INCREASE"),
                                    "evaluation_hash": width_depth_evaluation.evaluation_hash,
                                },
                                "update_hash": width_depth_update.update_hash,
                                "candidate_state_hash": width_depth_evaluation.candidate_state_hash,
                                "width_depth_increment_scan_candidate": True,
                            }
                        )

            after_width_reinforcement_specs = [
                dict(spec)
                for spec in runtime_specs
                if spec.get("reinforcement_retry_after_width")
            ]
            for source_spec in after_width_reinforcement_specs:
                source_updates = dict(source_spec.get("updates") or {})
                lane_id = str(
                    source_spec.get("contract_runtime_lane_id") or ""
                )
                for depth_step in CONTRACT_RUNTIME_DEPTH_SCAN_STEPS_MM:
                    depth = float(base_depth + depth_step)
                    combined_updates = {**source_updates, "D": depth}
                    diff = _normalised_update_diff(base, combined_updates)
                    if not diff:
                        continue
                    (
                        layout_width,
                    ...
                    stage_name = (
                        f"{source_spec.get('stage_name')}"
                        "_depth_increment_scan"
                    )
                    strategy = (
                        f"{source_spec.get('strategy')} with depth "
                        f"increased to {depth:.0f} mm"
                    )
                    if _depth_width_ratio_blocked(
                        layout_width,
                        layout_depth,
                    ...
                    if not bool(longitudinal_spacing_rule.get("valid")):
                        ...
                        continue
                    runtime_index += 1
                    combined_update = BeamCandidateUpdate(
                        updates=combined_updates
                    )
                    combined_evaluation = _contract_runtime_evaluator(
                        BeamCandidateInput(base_state=base),
                        combined_update,
                    )
                    decision = decide_bending_fail_repair_ladder_add(
                        ...
                        assigned_candidate_index=runtime_index,
                        assigned_label=(
                            "BENDING_FAIL_GOVERNS contract runtime "
                            f"{runtime_index}: {strategy}"
                        ),
                        escalation=(
                            "retry_reinforcement_after_width_then_depth"
                        ),
                        width=layout_width,
                        depth=layout_depth,
                        row1=row1,
                        ...
                        runtime_specs.append(
                            {
                                **decision.spec_payload,
                                ...
                                "ladder_trace_evidence": {
                                    ...
                                    "evaluation_hash": (
                                        combined_evaluation.evaluation_hash
                                    ),
                                },
                                "update_hash": combined_update.update_hash,
                                "candidate_state_hash": (
                                    combined_evaluation.candidate_state_hash
                                ),
                                "reinforcement_retry_after_width": True,
                                "width_reinforcement_depth_scan_candidate": (
                                    True
                                ),
                            }
                        )
        ...
        if not geometry_locked:
            runtime_width_steps = [
                float(step)
                for step in DEFAULT_WIDTH_STEPS_MM
            ]
        ...
        stop_reason = (
            ...
            if geometry_locked
                ...
                "canonical project geometry limits reached; legal no-repair proof "
                "required after 5000 mm depth and 5000 mm width exhaustion"
            )
        )
        if not geometry_locked:
            runtime_specs.extend(
                _canonical_unlocked_geometry_continuation_specs(
                    runtime_specs,
                    base=base,
                    width_key=width_key,
                    base_width=base_width,
            ...
            runtime_width_steps = [
                ...
                for width in range(
                    int(math.ceil(base_width / 25.0) * 25 + 25),
                    int(PROJECT_MAX_BEAM_WIDTH_MM) + 1,
                    25,
                )
            ]
        deduped_runtime_specs = _order_and_reindex_bending_specs(
            _dedupe_specs(runtime_specs)
        )
        ...
        runtime_ladder = build_bending_fail_repair_ladder_result(
            ...
            bar_diameters_tried=[
                int(dia) for dia in larger_dias
            ],
