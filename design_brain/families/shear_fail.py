...
from design_brain.families.shear_fail_governs.contract import (
    internal_strategy_lanes as _contract_strategy_lanes,
)
...
_RUNTIME_CONTRACT_STEP_BY_LANE = {
    "SPACING_REDUCTION": 1,
    "LEG_COUNT_INCREASE_RESTART_REINFORCEMENT_SEARCH": 2,
    "BAR_SIZE_INCREASE": 3,
    "DEPTH_INCREASE_RESTART_REINFORCEMENT_SEARCH": 4,
    "WIDTH_INCREASE_RESTART_REINFORCEMENT_SEARCH": 5,
    "EXACT_STOP": 6,
    "EXHAUSTED": 7,
    "NO_VALID_REPAIR": 8,
}
...
def _runtime_strategy_label(record: dict) -> str:
    ...
    if lane_id == "BAR_SIZE_INCREASE":
        return (
            ...
            f"with {int(reinforcement.get('ligature_leg_count') or 0)} legs "
    ...
    if lane_id == "DEPTH_INCREASE_RESTART_REINFORCEMENT_SEARCH":
        return (
            ...
            f"and restart shear reinforcement at "
            f"{int(reinforcement.get('ligature_leg_count') or 0)} legs, "
            f"N{int(reinforcement.get('ligature_diameter_mm') or 0)}, "
            f"{float(reinforcement.get('ligature_spacing_mm') or 0):.0f} mm spacing"
        )
...
def _contract_geometry_increment_mm(lane_id: str, fallback: float) -> float:
    for lane in _contract_strategy_lanes():
        if str(lane.get("lane_id") or "") != lane_id:
            continue
        policy = _as_dict(lane.get("search_policy"))
        try:
            return max(float(policy.get("increment_mm")), 0.0)
        except (TypeError, ValueError):
            break
    ...
    def contracted_repair_ladder_specs(
        ...
        *,
        ...
        runtime_result = _run_contract_runtime(
            ...
            max_depth_steps=(
                ...
                if geometry_locked
                    ...
                    incremental_geometry_values(
                        _state_float(base, "D", 600.0),
                        maximum_mm=PROJECT_MAX_BEAM_DEPTH_MM,
                        increment_mm=_contract_geometry_increment_mm(
        ...
        return {
            ...
            "restart_rule": (
                "After every leg-count, bar-size, depth, or width change, restart the legal earlier "
                "shear-reinforcement phases in spacing, legs, then diameter order."
            ),
